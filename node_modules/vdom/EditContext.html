...
const calcTextWidth = v => ctx.measureText(v).width;
...
function showText(){
	...
	if(selectionStart != selectionEnd){
		...
		let [ax, ay] = calcXY(Math.min(selectionStart, selectionEnd))
		let [bx, by] = calcXY(Math.max(selectionStart, selectionEnd))
		ctx.fillStyle = 'yellow';
		while(ay < by){
			let lineEnd = getCurrLineEnd(offset);
			ctx.fillRect(ax, ay * FONT_SIZE, calcTextWidth(editContext.text.slice(offset, lineEnd)), FONT_SIZE);
			offset = lineEnd + 1;
			ax = 0;
			++ay;
		}
		ctx.fillRect(ax, ay * FONT_SIZE, bx-ax, FONT_SIZE);
		ctx.fillStyle = 'black';
	}
	let y = 0;
	let offset = 0;

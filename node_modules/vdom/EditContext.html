...
function wrapText(text, maxWidth){
	...
	for(let y=0; y<lineList.length; ++y){
		let line = lineList[y]
		let charWidthList = Array.from(line, calcTextWidth);
		let width = 0;
		for(let i=0, n=charWidthList.length; i<n; ++i){
			width += charWidthList[i];
			if(width > maxWidth){
				lineList.splice(y, 1, line.slice(0, i), line.slice(i));
				break;
			}
		}
	}
...
function showText(){
	...
	const lineList = splitLines(wrapText(editContext.text.replaceAll('\t', ' '.repeat(4)), canvas.width));
	for(let i=0, n=lineList.length; i<n; ++i){
		ctx.fillText(lineList[i], 0, i * FONT_SIZE);
	}
	//draw caret
	const [x, y] = calcXY(selectionEnd);
	ctx.fillRect(x, y * FONT_SIZE, 1, FONT_SIZE);
}
...
document.addEventListener('keydown', evt => {
	...
	if(keyCode == 'Tab'){
		...
		if(evt.shiftKey){
			if(selectionEnd > 0){
				editContext.updateSelection(selectionStart, selectionEnd - 1);
				showText();
			}
		}else if(selectionStart != selectionEnd){
			let offset = Math.min(selectionStart, selectionEnd);
			editContext.updateSelection(offset, offset);
			showText();
		}else if(selectionStart > 0){
			let offset = selectionStart - 1;
			editContext.updateSelection(offset, offset);
			showText();
		}
	}else if(keyCode == 'ArrowRight'){
		if(evt.shiftKey){
			if(selectionEnd < editContext.text.length){
				editContext.updateSelection(selectionStart, selectionEnd + 1);
				showText();
			}
		}else if(selectionStart != selectionEnd){
			let offset = Math.max(selectionStart, selectionEnd);
			editContext.updateSelection(offset, offset);
			showText();
		}else if(selectionStart < editContext.text.length){
			let offset = selectionStart + 1;
			editContext.updateSelection(offset, offset);
			showText();
		}
		...
		if(selectionStart != 0 || selectionEnd != editContext.text.length){
			editContext.updateSelection(0, editContext.text.length);
			showText();
		}
	}else if(keyCode == 'KeyC'){
		if(!evt.ctrlKey)return;
		navigator.clipboard.writeText(editContext.text.substring(selectionStart, selectionEnd));

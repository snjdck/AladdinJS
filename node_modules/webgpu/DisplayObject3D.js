...
const Quaternion = require('opengl/math/Quaternion');
...
class DisplayObject3D extends DisplayObject
{
	...
	*collectDrawUnits(camera){
		if(!this.visible)return;
		if(camera.canCull(this))return;
		yield* this.onCollectDrawUnits(camera);
	}

	*onCollectDrawUnits(camera){
		for(const child of this.children){
			yield* child.collectDrawUnits(camera);
		}
	}

	...
	drawDepthMap(shadowCasterList, viewCamera){
		...
		for(let commandEncoder of CommandEncoder(Vulkan.device)){
			for(let renderPassEncoder of beginRenderPass(';1+store', commandEncoder, depthStencilView)){
				Material.drawDepth(renderPassEncoder, shadowCasterList, tempBuffer);
			}
		}
	}
}
...
const drawFn = createDepthDrawFn('2d', `
	let viewDirection = vec3<f32>(0, 0, -1) * m34tom33(info.viewCameraInv.cameraMatrix);
	let factor = vec4<f32>(0, 0, 1, 1);
	let emissive = 0.0;
	let ambient = 0.1;
	let gloss = 20;//Shininess
	let diffuse = saturate(dot(normal_in_world, info.lightDirection.xyz));
	let specular = pow(saturate(dot(
		reflect(-info.lightDirection.xyz, normal_in_world),
		viewDirection
	)), gloss);
	let lightValue: f32 = dot(vec4<f32>(emissive, ambient, diffuse, specular), factor);
	return vec4<f32>(info.lightColor.rgb * lightValue * shadow_factor, 1);
`);

...
const {fetchTextures, fetchCubeDepth} = require('../Vulkan/resource');
...
const rotationList = [
	new Quaternion().fromEulerAngles(90 * RADIAN, 0,  90 * RADIAN),//+x
	new Quaternion().fromEulerAngles(90 * RADIAN, 0, -90 * RADIAN),//-x
	new Quaternion().fromEulerAngles(90 * RADIAN, 0, 180 * RADIAN),//+y
	new Quaternion().fromEulerAngles(90 * RADIAN, 0,   0 * RADIAN),//-y
	new Quaternion(),
	new Quaternion().fromEulerAngles(180 * RADIAN, 0,   0),//-z
];
...
class PointLight3D extends Light3D
{
	constructor(){
		...
		const {canvas} = Vulkan;
		this.camera = new Camera3D(new Lens.Perspective(Math.PI / 2, 1, 1, 10000, 0));
/*

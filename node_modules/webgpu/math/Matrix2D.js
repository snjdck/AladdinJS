...
class Matrix2D
{
	...
	appendRotation(angle, result=this){
		const sin = Math.sin(angle);
		const cos = Math.cos(angle);
		const {a, b, c, d, tx, ty} = this;
		
		result.a  = a  * cos - b  * sin;
		result.c  = c  * cos - d  * sin;
		result.tx = tx * cos - ty * sin;
		
		result.b  = b  * cos + a  * sin;
		result.d  = d  * cos + c  * sin;
		result.ty = ty * cos + tx * sin;

		return result;
	}

	prependRotation(angle, result=this){
		const sin = Math.sin(angle);
		const cos = Math.cos(angle);
		const {a, b, c, d, tx, ty} = this;
		...
		result.c = c * cos - a * sin;
		result.d = d * cos - b * sin;
	...
	appendScale(sx, sy=sx, result=this){
		result.a  = this.a  * sx;
		result.c  = this.c  * sx;
		result.tx = this.tx * sx;
		
		result.b  = this.b  * sy;
		result.d  = this.d  * sy;
		result.ty = this.ty * sy;
		return result;
	}

	prependScale(sx, sy=sx, result=this){
		result.a = this.a * sx;
		result.b = this.b * sx;
	...
	appendTranslation(dx, dy, result=this){
		const {a, b, c, d, tx, ty} = this;
		result.tx = tx + dx;
		result.ty = ty + dy;
		if(result == this)return this;
	...
	prependTranslation(dx, dy, result=this){
		const {a, b, c, d, tx, ty} = this;
		result.tx = dx * a + dy * c + tx;
		result.ty = dx * b + dy * d + ty;
		if(result == this)return this;
		result.a = a;
		result.b = b;
		result.c = c;
	...
	setTranslation(vx, vy, result=this){
		result.tx = vx;
		result.ty = vy;
		if(result == this)return this;
		result.a = this.a;

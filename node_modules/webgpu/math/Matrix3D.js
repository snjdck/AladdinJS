...
class Mat3x4
{
	...
	prependScale(sx, sy=sx, sz=sy, result=this){
		const [b00, b01, b02, b03, b10, b11, b12, b13, b20, b21, b22, b23] = this.rawData;
		const output = result.rawData;
	...
	setTranslation(tx, ty, tz, result=this){
		const [a00, a01, a02, a03, a10, a11, a12, a13, a20, a21, a22, a23] = this.rawData;
		const output = result.rawData;
	...
	copyRowTo(row, output, offset=0){
		const {rawData} = this;
		const index = row << 2;
		output[offset  ] = rawData[index  ];
		output[offset+1] = rawData[index+1];
		output[offset+2] = rawData[index+2];
		output[offset+3] = rawData[index+3];
	}
...
//rotation必须normalize:xx+yy+zz+ww=1
Mat3x4.decompose = function(rawData){
	const [v00, v01, v02, v03, v10, v11, v12, v13, v20, v21, v22, v23] = rawData;
	const sx = Math.hypot(v00, v10, v20);
	const sy = Math.hypot(v01, v11, v21);
	const sz = Math.hypot(v02, v12, v22);
	const tx = v03;
	const ty = v13;
	const tz = v23;
	const rotation = new Quaternion().fromMatrix([
		v00 / sx, v01 / sy, v02 / sz, 0,
		v10 / sx, v11 / sy, v12 / sz, 0,
		v20 / sx, v21 / sy, v22 / sz, 0,
	]);
	return {
		rotation,

...
const {nearEquals} = require('utils/math');
...
class Quaternion{
	...
	fromMatrix(rawData){
		const [v00, v01, v02, v03, v10, v11, v12, v13, v20, v21, v22, v23] = rawData;
		const xy = (v10 + v01) * 0.25;
		const zw = (v10 - v01) * 0.25;
		const zx = (v02 + v20) * 0.25;
		const yw = (v02 - v20) * 0.25;
		const yz = (v21 + v12) * 0.25;
		const xw = (v21 - v12) * 0.25;
		const b0 = (v11 + v22) * 0.25;//(ww-xx)/2
		const b1 = (v00 + v22) * 0.25;//(ww-yy)/2
		const b2 = (v00 + v11) * 0.25;//(ww-zz)/2
		const xx = Math.hypot(xw, b0) - b0;
		const yy = Math.hypot(yw, b1) - b1;
		const zz = Math.hypot(zw, b2) - b2;
		const ww = (v00 + v11 + v22 + xx + yy + zz) / 3;
	...
	interpolate(to, f, result){
		return Quaternion.Slerp(this, to, f, result);
	}

	equals(other, precision){
		return nearEquals(this.x, other.x, precision) && nearEquals(this.y, other.y, precision) && nearEquals(this.z, other.z, precision) && nearEquals(this.w, other.w, precision);
	}
/*

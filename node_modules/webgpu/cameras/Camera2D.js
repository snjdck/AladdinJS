...
const {fetchView3D, fetchView2D} = require('../Vulkan/resource');
...
const drawBitmap3D = function(){
	return (commandEncoder, item) => {
		...
		for(let renderPassEncoder of beginRenderPass('[0,0,0,0]+store;1+discard;0+discard', commandEncoder, colorView, depthView)){
...
const drawFilter2D = function(){
	...
	return (commandEncoder, item) => {
		...
		for(let renderPassEncoder of beginRenderPass('[0,0,0,0]+store', commandEncoder, view0))camera.draw(renderPassEncoder);
		if(item.filter.justCopyFlag){
			...
			for(let renderPassEncoder of beginRenderPass('[0,0,0,0]+store', commandEncoder, view1)){

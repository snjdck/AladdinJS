...
Camera2D.prototype.draw = function(){
	...
	return function(renderPassEncoder){
		...
		if(bitmapList.length > 0){
			...
			renderPassEncoder.setResources({'viewport': {buffer:tempBuffer}});
...
const drawBitmap3D = function(){
	return (commandEncoder, item) => {
		...
		const colorTexture = commandEncoder.fetchTempTexture(item, '2d,bgra8unorm,0x14,1');
		const depthTexture = commandEncoder.fetchTempTexture(item, '2d,depth24plus-stencil8,0x10,1');
		...
		for(let renderPassEncoder of commandEncoder.beginRenderPass('[0,0,0,0]+store;1+discard;0+discard', colorView, depthView)){
			renderPassEncoder.setViewportWH(item.width, item.height);
...
const drawFilter2D = function(){
	...
	return (commandEncoder, item) => {
		...
		const colorTexture = commandEncoder.fetchTempTexture([areaRect.width, areaRect.height, 2], '2d,bgra8unorm,0x17,1', '>=', '>=');
		...
		for(let renderPassEncoder of commandEncoder.beginRenderPass('[0,0,0,0]+store', view0))camera.draw(renderPassEncoder);
		if(item.filter.justCopyFlag){
			const clipRect = intersect(areaRect, item.filter.clipRect);
			if(!clipRect)return;
			commandEncoder.copyTextureToTexture(
				{texture:colorTexture, origin: [clipRect.x - areaRect.x, clipRect.y - areaRect.y, 0]},
				{texture:colorTexture, origin: [0, 0, 1]},
				[clipRect.width, clipRect.height, 1]
			);
			...
			for(let renderPassEncoder of commandEncoder.beginRenderPass('[0,0,0,0]+store', view1)){
				const materialType = item.filter.constructor;
				renderPassEncoder.setResources({'texture': view0});

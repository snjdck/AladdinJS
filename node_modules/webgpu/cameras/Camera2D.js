...
const drawBitmap3D = function(){
	return (commandEncoder, item) => {
		const {colorView, depthView, width, height} = fetchView3D(item, commandEncoder.device);
...
const drawFilter2D = function(){
	...
	return (commandEncoder, item) => {
		...
		const {width, height, view0, view1, copyTexture} = fetchView2D(areaRect, commandEncoder.device);

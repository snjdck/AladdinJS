...
const drawFilter2D = function(){
	const renderPass2D = RenderPass({colorAttachments:[{loadOp:'clear',clearValue:[0,0,0,0],storeOp:'store'}]});
	...
	return (commandEncoder, item) => {
		...
		for(let renderPassEncoder of renderPass2D(commandEncoder, view0))camera.draw(renderPassEncoder);
		if(item.filter.justCopyFlag){
			...
			for(let renderPassEncoder of renderPass2D(commandEncoder, view1)){

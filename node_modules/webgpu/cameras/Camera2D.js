...
Camera2D.prototype.drawPick = function(){
	...
	const fn = walkTree(function(item, result, picking){
		...
		if(!item.mouseChildren)return true;
	});
	const pickList = [];
	return function(renderPassEncoder, root, mouseX, mouseY, picking){
		pickList.length = 0;
		...
		renderPassEncoder.drawQuadWith(Material2D.Pick, {'viewport': {buffer:viewport}, 'rectInfoList': pickList, 'meshId':pickList}, pickList.length);
	}
}();

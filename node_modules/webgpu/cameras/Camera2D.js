...
Camera2D.prototype.draw = function(){
	...
	return function(renderPassEncoder){
		...
		this.pickList = bitmapList.slice();
...
Camera2D.prototype.drawPick = function(){
	...
	return function(renderPassEncoder, mouseX, mouseY, picking){
		...
		viewport.set([2, -2, -(2*mouseX+1), 2*mouseY+1]);
		this.worldTransformInvert.copyToArray(viewport, 4);
		renderPassEncoder.setResources({'viewport': {buffer:viewport}});
		pickList.forEach(v => picking.register(v));
		Material2D.Pick.draw(pickList, renderPassEncoder);
	}
}();

...
Camera2D.prototype.draw = function(){
	...
	return function(renderPassEncoder){
		...
		if(bitmapList.length > 0){
			...
			Material2D.setBindGroup(renderPassEncoder, 0, [{binding: 0, resource:{buffer:tempBuffer}}]);
	...
	return (commandEncoder, item) => {
		...
		if(item.filter.justCopyFlag){
			...
			for(let renderPassEncoder of beginRenderPass('[0,0,0,0]+store', commandEncoder, view1)){
				...
				materialType.setBindGroup(renderPassEncoder, 0, [{binding:0, resource:view0}]);

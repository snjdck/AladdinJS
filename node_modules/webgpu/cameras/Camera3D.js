...
Camera3D.prototype.draw = function(){
	...
	return function(renderPassEncoder, prevCtx){
		...
		if(drawUnitCollector.mirrorList.length > 0){
			for(let commandEncoder of renderPassEncoder.createCommandEncoder()){
				for(let {entity, material} of drawUnitCollector.mirrorList){
					...
					const ctx = fetchTextures(Vulkan.canvas, renderPassEncoder.device);
		...
		if(opaqueList.length > 0){
			const {width, height} = Vulkan.canvas;
			const ctx = fetchTextures(Vulkan.canvas, renderPassEncoder.device);//sub camera area should smaller
			for(let commandEncoder of renderPassEncoder.createCommandEncoder()){
				for(let renderPassEncoderDeferred of commandEncoder.beginRenderPass('[0,0,0,0]+store:[0,0,0,0]+store:[0,0,0,0]+store;1+store;0+store', ctx.colorView, ctx.normalView, ctx.emissiveView, ctx.depthStencilView)){
				...
				for(let _ of commandEncoder.beginRenderPass('[0,0,0,0]+store', ctx.lightView));
			}
			for(let commandEncoder of renderPassEncoder.createCommandEncoder()){
				drawBloom(commandEncoder, ctx.emissiveTexture);
				for(let light of drawUnitCollector.lightList){
					light.drawDepthMap(commandEncoder, drawUnitCollector.shadowCasterList, this);
					light.drawDeferred(commandEncoder, ctx);
					//light.drawShadow(ctx);
				}
			}
			for(let commandEncoder of renderPassEncoder.createCommandEncoder()){
				for(let renderPassEncoder of commandEncoder.beginRenderPass('[0,0,0,0]+store', prevCtx.colorView)){
					renderPassEncoder.setViewportWH(width, height);
					renderPassEncoder.setPipeline(mergeFn);
					renderPassEncoder.setResources({'baseTexture': ctx.colorView, 'lightTexture': ctx.lightView, 'emissiveTexture':ctx.emissiveView});
			...
			for(let commandEncoder of renderPassEncoder.createCommandEncoder()){
				for(let _ of commandEncoder.beginRenderPass('[0,0,0,0]+store;1+store;0+store', prevCtx.colorView, prevCtx.depthStencilView));
			}
		}
		...
		if(prevCtx && !this.isPicking){
			this.isPicking = true;
			const {view:pickView, texture, depthStencilView} = fetchPickTexture({width:1,height:1}, renderPassEncoder.device);
			const {width, height} = Vulkan.canvas;
			const buffer = fetchBuffer(4, GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST, renderPassEncoder.device);

...
void function(){
	...
	//const getBuffer = delayNew(() => Vulkan.device.createBuffer({usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST, size: bytePerInstance * maxCameraCount}))
	//const getBindGroup = delayNew(() => TextureMaterial.createBindGroupWith({buffer:getBuffer()}));

	const [addDrawTask, runDrawTask] = deferBunchDo(function(drawUnits){
		renderDrawUnits(drawUnits);
	});

	Camera3D.prototype.collectDrawUnits = function(root){
		this.updateViewFrustum();
		for(const entity of root.collectEntities(this)){
			for(const material of entity.getMaterials()){
				addDrawTask({entity, material});
			}
		}
	}

	Camera3D.prototype.draw = function(fetchBuffer){
		const bufferInfo = fetchBuffer(bytePerInstance);
		const {queue} = Vulkan.device;
		
		queue.writeBuffer(bufferInfo.buffer, 0, this.lens.rawData);
		queue.writeBuffer(bufferInfo.buffer, 64 , this.worldTransformInvert.rawData);
		queue.writeBuffer(bufferInfo.buffer, 112, this.viewportRect);

		Vulkan.renderPassEncoder.setBindGroup(0, TextureMaterial.createBindGroupWith(bufferInfo));
		runDrawTask();
	}
}();

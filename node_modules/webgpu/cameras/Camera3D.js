...
const {fetchTextures, fetchPickTexture, fetchBuffer} = require('../Vulkan/resource');
const Light3D = require('../lights/Light3D');
const Plane = require('../math/Plane');
const Vector3D = require('../math/Vector3D');
const {createDrawFn} = require('../helper/RectDrawer');
...
Camera3D.prototype.draw = function(){
	...
	return function(renderPassEncoder, prevCtx){
		...
		if(drawUnitCollector.mirrorList.length > 0){
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let {entity, material} of drawUnitCollector.mirrorList){
					...
					for(let renderPassEncoder of beginRenderPass('load+store;load+discard;load+discard', commandEncoder, ctx.colorView, ctx.depthStencilView)){
		...
		TextureMaterial.setResources(renderPassEncoder, {'vpMatrix': {buffer:tempBuffer}});
		...
		if(opaqueList.length > 0){
			...
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let renderPassEncoderDeferred of beginRenderPass('[0,0,0,0]+store:[0,0,0,0]+store:[0,0,0,0]+store;1+store;0+store', commandEncoder, ctx.colorView, ctx.normalView, ctx.emissiveView, ctx.depthStencilView)){
					//renderPassEncoderDeferred.setViewport(0, 0, width, height, 0, 1);
					TextureMaterial.setResources(renderPassEncoderDeferred, {'vpMatrix': {buffer:tempBuffer}});
					for(let [materialType, entityList] of opaqueList){
						materialType.draw(entityList, renderPassEncoderDeferred);
					}
				}
				for(let _ of beginRenderPass('[0,0,0,0]+store', commandEncoder, ctx.lightView));
			}
			drawBloom(Vulkan.device, ctx.emissiveTexture);
			for(let light of drawUnitCollector.lightList){
				light.drawDepthMap(drawUnitCollector.shadowCasterList, this);
				light.drawDeferred(ctx);
				//light.drawShadow(ctx);
			}
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let renderPassEncoder of beginRenderPass('[0,0,0,0]+store', commandEncoder, prevCtx.colorView)){
					renderPassEncoder.setViewport(0, 0, width, height, 0, 1);
					mergeFn(renderPassEncoder, {'baseTexture': ctx.colorView, 'lightTexture': ctx.lightView, 'emissiveTexture':ctx.emissiveView});
				}
			...
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let _ of beginRenderPass('[0,0,0,0]+store;1+store;0+store', commandEncoder, prevCtx.colorView, prevCtx.depthStencilView));
			}
		}
		...
		if(prevCtx && !this.isPicking){
			this.isPicking = true;
			const {view:pickView, texture, depthStencilView} = fetchPickTexture({width:1,height:1});
			const {width, height} = Vulkan.canvas;
			const buffer = fetchBuffer(4, GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST);
			const x = Vulkan.mouseMgr.x >> 0;
			const y = Vulkan.mouseMgr.y >> 0;
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let renderPassEncoder of beginRenderPass('[0,0,0,0]+store;1+discard;0+discard', commandEncoder, pickView, depthStencilView)){
					//renderPassEncoder.setViewport(0, 0, width, height, 0, 1);
					//renderPassEncoder.setScissorRect(x, y, 1, 1);
					tempBuffer.set(this.lens.offset(width, height, x, y, 1, 1));
					Material.drawPick(renderPassEncoder, drawUnitCollector.entityList, tempBuffer);
				}
				commandEncoder.copyTextureToBuffer({origin:[0,0,0], texture}, {buffer}, [1,1,1]);
			}
			buffer.mapAsync(GPUMapMode.READ).then(() => {
				const value = new Uint32Array(buffer.getMappedRange())[0];
				buffer.unmap();
				this.isPicking = false;
				console.log(Material.itemDict.get(value))
			});
		}
...
const mergeFn = createDrawFn([[
	...
	{name:'emissiveTexture', binding:2, visibility:GPUShaderStage.FRAGMENT, texture:{}},
	{binding:3, visibility:GPUShaderStage.FRAGMENT, sampler:{}, default:{minFilter:'linear',magFilter:'linear',mipmapFilter:'linear'}},
]], `
	struct Output {
	...
	/*
	@vertex
	fn mainVertex(@builtin(vertex_index) vertex_index: u32) -> @builtin(position) vec4<f32> {
		let vertex = vec2<f32>(vec2(vertex_index >> 1, vertex_index & 1));
...
*/
	...
	@group(0) @binding(2) var texture_emissive: texture_2d<f32>;
	...
	fn mainFragment(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
		let emissive = textureSampleLevel(texture_emissive, g_sampler, uv, 0);
		let baseColor = textureSample(texture_color, g_sampler, uv);
		let lightColor = textureSample(texture_light, g_sampler, uv);
		//return vec4<f32>(pow(emissive.rgb, vec3(0.45)), baseColor.a);
		return vec4<f32>(lightColor.rgb + ACESToneMapping(emissive.rgb, vec3<f32>(1)), baseColor.a);
		//return vec4<f32>(/*baseColor.rgb */ lightColor.rgb + emissive.rgb, baseColor.a);
	}
/*
	@fragment

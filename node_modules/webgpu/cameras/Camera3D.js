...
const {pipeline:mergePipeline} = require('../materials/MergeLightPass');
...
Camera3D.prototype.draw = function(){
	...
	return function(renderPassEncoder, prevCtx){
		...
		if(drawUnitCollector.mirrorList.length > 0){
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let {entity, material} of drawUnitCollector.mirrorList){
					material.camera.moveTo(this.x, 1000 - this.y, this.z);
					material.camera.drawUnitCollector = drawUnitCollector.clone();
					material.camera.drawUnitCollector.mirrorList.length = 0;
					material.camera.drawUnitCollector.entityList = material.camera.drawUnitCollector.entityList.filter(v => v.material.constructor.name != 'MirrorMaterial')
					tempBuffer.set(material.camera.lens.rawData);
					tempBuffer.set(material.camera.worldTransformInvert.rawData, 16);
					const {width, height} = Vulkan.canvas;
					const ctx = fetchTextures(Vulkan.canvas);
					material.texture = ctx.colorView;
					for(let renderPassEncoder of clearPlanePass(commandEncoder, ctx.colorView, ctx.depthStencilView)){
		...
		if(opaqueList.length > 0){
			...
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let renderPassEncoderDeferred of renderPass3D(commandEncoder, ctx.colorView, ctx.normalView, ctx.depthStencilView)){
				...
				for(let _ of clearLightPass(commandEncoder, ctx.lightView));
			}
			...
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let renderPassEncoder of clearLightPass(commandEncoder, prevCtx.colorView)){
					renderPassEncoder.setViewport(0, 0, width, height, 0, 1);
					mergePipeline.setPipeline(renderPassEncoder);
					mergePipeline.setBindGroup(renderPassEncoder, 0, [
						{binding:0, resource:ctx.colorView},
						{binding:1, resource:ctx.lightView},
					]);
					mergePipeline.setVertexBuffer(renderPassEncoder, 0, uv_scale);
			...
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let _ of clearPass(commandEncoder, prevCtx.colorView, prevCtx.depthStencilView));
			}
		}
...
const clearPlanePass = RenderPass({
...
const clearLightPass = RenderPass({
	colorAttachments:[{loadOp:'clear', clearValue:[0,0,0,0], storeOp:'store'}]
});

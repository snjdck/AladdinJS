...
const PickMaterial = require('../materials/PickMaterial');
...
Camera3D.prototype.draw = function(){
	...
	return function(renderPassEncoder, prevCtx){
		...
		if(opaqueList.length > 0){
			...
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let renderPassEncoderDeferred of beginRenderPass('[0,0,0,0]+store:[0,0,0,0]+store;1+store;0+store', commandEncoder, ctx.colorView, ctx.normalView, ctx.depthStencilView)){
			...
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let renderPassEncoder of beginRenderPass('[0,0,0,0]+store', commandEncoder, prevCtx.colorView)){
					renderPassEncoder.setViewport(0, 0, width, height, 0, 1);
					mergeFn(renderPassEncoder, {'baseTexture': ctx.colorView, 'lightTexture': ctx.lightView});
				}
		...
		if(prevCtx && !this.isPicking){
			this.isPicking = true;
			const {view:pickView, texture} = fetchPickTexture(Vulkan.canvas);
			const {width, height} = Vulkan.canvas;
			const buffer = fetchBuffer(4, GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST);
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let renderPassEncoder of beginRenderPass('[0,0,0,0]+store;load+store;load+store', commandEncoder, pickView, prevCtx.depthStencilView)){
					renderPassEncoder.setViewport(0, 0, width, height, 0, 1);
					PickMaterial.setResources(renderPassEncoder, {'vpMatrix': {buffer:tempBuffer}});
					for(let [materialType, entityList] of opaqueList){
						if(materialType.name == 'TerrainMaterial')continue;
						PickMaterial.draw(entityList, renderPassEncoder);
					}
				}
				const {x, y} = Vulkan.mouseMgr;
				commandEncoder.copyTextureToBuffer({origin:[x>>0,y>>0,0], texture}, {buffer}, [1,1,1]);
			}
			buffer.mapAsync(GPUMapMode.READ).then(() => {
				const value = new Uint32Array(buffer.getMappedRange())[0];
				buffer.unmap();
				this.isPicking = false;
				console.log(PickMaterial.itemDict.get(value))
			});
		}

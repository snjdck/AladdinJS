...
Camera3D.prototype.draw = function(){
	...
	return function(renderPassEncoder, prevCtx){
		...
		this.pickList = this.drawUnitCollector.entityList.slice();
...
Camera3D.prototype.drawPick = function(){
	...
	const groupFn = drawUnit => drawUnit.material.pick?.constructor;
	return function(renderPassEncoder, mouseX, mouseY, picking){
		const {width, height} = renderPassEncoder;
		vpMatrix.set(this.lens.offset(width, height, mouseX, mouseY, 1, 1));
		vpMatrix.set(this.worldTransformInvert.rawData, 16);
		renderPassEncoder.setResources({'vpMatrix': {buffer:vpMatrix}});
		this.pickList.forEach(v => picking.register(v));
		for(let [materialType, drawUnitList] of Map.groupBy(this.pickList, groupFn)){

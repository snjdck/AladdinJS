...
Camera3D.prototype.draw = function(){
	...
	return function(renderPassEncoder, prevCtx){
		...
		if(opaqueList.length > 0){
			const textureSize = renderPassEncoder;
			const colorTexture = renderPassEncoder.fetchTempTexture(textureSize, '2d,bgra8unorm,0x17,1');
			const lightTexture = renderPassEncoder.fetchTempTexture(textureSize, '2d,bgra8unorm,0x17,1');
			const normalTexture = renderPassEncoder.fetchTempTexture(textureSize, '2d,rgba16float,0x17,1');
			const emissiveTexture = renderPassEncoder.fetchTempTexture(textureSize, '2d,rgba16float,0x17,1');
			const depthTexture = renderPassEncoder.fetchTempTexture(textureSize, '2d,depth24plus-stencil8,0x17,1');

			const colorView = renderPassEncoder.fetchTextureView(colorTexture, '2d,all,0,1');
			const normalView = renderPassEncoder.fetchTextureView(normalTexture, '2d,all,0,1');
			const emissiveView = renderPassEncoder.fetchTextureView(emissiveTexture, '2d,all,0,1');
			const depthStencilView = renderPassEncoder.fetchTextureView(depthTexture, '2d,all,0,1');
			const depthView = renderPassEncoder.fetchTextureView(depthTexture, '2d,depth-only,0,1');
			const lightView = renderPassEncoder.fetchTextureView(lightTexture, '2d,all,0,1');

			for(let commandEncoder of renderPassEncoder.createCommandEncoder()){
				for(let renderPassEncoderDeferred of commandEncoder.beginRenderPass('[0,0,0,0]+store:[0,0,0,0]+store:[0,0,0,0]+store;1+store;0+store', colorView, normalView, emissiveView, depthStencilView)){
					//renderPassEncoderDeferred.setViewport(0, 0, width, height, 0, 1);
					renderPassEncoderDeferred.setResources({'vpMatrix': {buffer:tempBuffer}});
			...
			for(let commandEncoder of renderPassEncoder.createCommandEncoder()){
				...
				commandEncoder.copyTextureToTexture({texture:depthTexture}, {texture:prevCtx.depthTexture}, textureSize);//The entire subresource must be copied when the format (TextureFormat::Depth24PlusStencil8) is a depth/stencil format or the sample count (1) is > 1.
			}
		}else if(prevCtx){
			for(let commandEncoder of renderPassEncoder.createCommandEncoder()){
		...
		if(prevCtx && !this.isPicking){
			...
			const colorTexture = renderPassEncoder.fetchTempTexture(textureSize, '2d,r32uint,0x11,1');
			const depthTexture = renderPassEncoder.fetchTempTexture(textureSize, '2d,depth24plus-stencil8,0x10,1');
			const pickView = renderPassEncoder.fetchTextureView(colorTexture, '2d,all,0,1');
			const depthStencilView = renderPassEncoder.fetchTextureView(depthTexture, '2d,all,0,1');
			const {width, height} = renderPassEncoder;
			const buffer = renderPassEncoder.fetchTempBuffer(4, GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST);
			const x = Vulkan.mouseMgr.x >> 0;
			const y = Vulkan.mouseMgr.y >> 0;
			for(let commandEncoder of renderPassEncoder.createCommandEncoder()){
				for(let renderPassEncoder of commandEncoder.beginRenderPass('[0,0,0,0]+store;1+discard;0+discard', pickView, depthStencilView)){
				...
				commandEncoder.copyTextureToBuffer({origin:[0,0,0], texture:colorTexture}, {buffer}, textureSize);
			}
			renderPassEncoder.readBuffer(buffer, arrayBuffer => {

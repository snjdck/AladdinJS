	...
	return function(renderPassEncoder, prevCtx){
		...
		if(this.isMain){
			...
			for(let commandEncoder of CommandEncoder(Vulkan.device)){
				for(let renderPassEncoderDeferred of renderPass3D(commandEncoder, ctx.colorView, ctx.normalView, ctx.depthView)){
					renderPassEncoderDeferred.setViewport(0, 0, width, height, 0, 1);
					for(let [materialType, entityList] of groupList){
						if(materialType.opaque){
							materialType.setBindGroup(renderPassEncoderDeferred, 0, [{binding:0, resource:{buffer:tempBuffer}}]);
				...
				commandEncoder.copyTextureToTexture({texture:ctx.colorTexture}, {texture:prevCtx.colorTexture}, Vulkan.canvas);

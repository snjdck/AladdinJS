...
void function(){
	...
	Camera3D.collectDrawUnits = function(camera, root){
		...
		for(const entity of root.collectEntities(camera)){
			for(const material of entity.getMaterials()){
				const drawUnit = {entity, material, cameraIndex};
				drawUnitBucket.add(drawUnit);
			}
		}
	}
	...
	Camera3D.draw = function(bindGroupIndex=0){
		...
		Vulkan.renderPassEncoder.setBindGroup(bindGroupIndex, bufferBindGroup.getBindGroup());

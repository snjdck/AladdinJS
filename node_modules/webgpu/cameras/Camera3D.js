...
void function(){
	...
	const getBindGroup = delayNew(() => Vulkan.createBindGroupNew(bindGroupLayout, [{buffer:getBuffer()}, Vulkan.getSampler(Vulkan.sampler.linear_repeat)]));

	const [addDrawTask, runDrawTask] = deferBunchDo(function(drawUnits){
		Vulkan.renderPassEncoder.setBindGroup(0, getBindGroup());
		renderDrawUnits(drawUnits);
	});

	Camera3D.prototype.collectDrawUnits = function(root, cameraIndex){
		const {queue} = Vulkan.device;
		const uniformBuffer = getBuffer();
		const offset = bytePerInstance * cameraIndex;
		
		queue.writeBuffer(uniformBuffer, offset, this.lens.rawData);
		queue.writeBuffer(uniformBuffer, offset + 64 , this.worldTransformInvert.rawData);
		queue.writeBuffer(uniformBuffer, offset + 112, this.viewportRect);

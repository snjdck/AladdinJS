...
Camera3D.prototype.draw = function(){
	...
	return function(renderPassEncoder, prevCtx){
		...
		if(drawUnitCollector.mirrorList.length > 0){
			for(let commandEncoder of renderPassEncoder.createCommandEncoder()){
				for(let {entity, material} of drawUnitCollector.mirrorList){
					...
					const ctx = fetchTextures(Vulkan.canvas, Vulkan.device);
		...
		if(opaqueList.length > 0){
			const {width, height} = Vulkan.canvas;
			const ctx = fetchTextures(Vulkan.canvas, Vulkan.device);//sub camera area should smaller
			for(let commandEncoder of renderPassEncoder.createCommandEncoder()){
		...
		if(prevCtx && !this.isPicking){
			this.isPicking = true;
			const {view:pickView, texture, depthStencilView} = fetchPickTexture({width:1,height:1}, Vulkan.device);
			const {width, height} = Vulkan.canvas;
			const buffer = fetchBuffer(4, GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST, Vulkan.device);

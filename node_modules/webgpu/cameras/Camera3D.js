	...
	collectDrawUnits(root){
		...
		for(const entity of root.collectEntities(this)){
			for(const material of entity.getMaterials()){
				this.drawUnitList.push({entity, material});
			}
		}
	}
}
...
Camera3D.prototype.draw = function(){
	...
	return function(){
		TextureMaterial.setBindGroup(0, [{binding:0, resource:{buffer:[this.lens.rawData, this.worldTransformInvert.rawData, this.viewportRect]}}]);
		for(let [materialType, entityList] of Array.from(Map.groupBy(this.drawUnitList, groupFn)).sort(sortFn)){

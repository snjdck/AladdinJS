...
void function(){
	...
	Camera3D.prototype.collectDrawUnits = function(root){
		const cameraIndex = indexDistributor.getIndex(this);
		const uniformBuffer = bufferBindGroup.getBuffer();
		const offset = bytePerInstance * cameraIndex;

		Vulkan.copyDataToBuffer(uniformBuffer, this.lens.rawData, offset);
		Vulkan.copyDataToBuffer(uniformBuffer, this.worldTransformInvert.rawData, offset + 64);
		Vulkan.copyDataToBuffer(uniformBuffer, this.viewportRect, offset + 112);

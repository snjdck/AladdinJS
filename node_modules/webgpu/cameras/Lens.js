...
class Perspective
{
	...
	offset(bw, bh, x, y, w, h){
		...
		temp[ 7] = b * -originZ;
		return temp;
		//x1 = xScale * x / (z - originZ)
		//y1 = yScale * y / (z - originZ)
		//x2 = ((1 + x1) * 0.5 * bw - x) / w * 2 - 1 = ((1 + x1) * bw - 2 * x) / w - 1
		//y2 = 1 - ((1 - y1) * 0.5 * bh - y) / h * 2 = 1 - ((1 - y1) * bh - 2 * y) / h
		//x2 = x1 * bw / w + (bw - 2 * x) / w - 1 = xScale * x / (z - originZ) * bw / w + (bw - 2 * x) / w - 1
		//y2 = y1 * bh / h - (bh - 2 * y) / h + 1 = yScale * y / (z - originZ) * bh / h - (bh - 2 * y) / h + 1
		//x2' = xScale * x * bw / w + (z - originZ) * ((bw - 2 * x) / w - 1)
		//y2' = yScale * y * bh / h - (z - originZ) * ((bh - 2 * y) / h - 1)
	}
	/*
	screenToCameraRect(l, t, r, b){

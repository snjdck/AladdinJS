...
class FilterBucket
{
	...
	draw(scene){
		...
		for(let group of this.list.reverse()){
			...
			let element = bitmap3dMgr.list[0];
			element.rectPack.clear();
			let frameBuffer = element.frameBuffer;
			let renderPassEncoder = Vulkan.renderPassEncoder = commandEncoder.beginRenderPass({
				colorAttachments: [{
					attachment: frameBuffer.createView(),
					loadValue: [0,0,0,0]
				}]
			});
			...
			for(let filterItem of group){
				...
				renderPassEncoder.setViewport(rect.x, rect.y, rect.width, rect.height, 0, 1);
			...
			element = bitmap3dMgr.list[1];
			frameBuffer = element.frameBuffer;
			renderPassEncoder = Vulkan.renderPassEncoder = commandEncoder.beginRenderPass({
				colorAttachments: [{
					attachment: frameBuffer.createView(),
					loadValue: [0,0,0,0]
				}]
			...
			for(let filterItem of group){
				...
				const texture = bitmap3dMgr.list[0].frameBuffer.createView();

				filter.onDraw(texture);

				filter.width = canvas.width;
				filter.height = canvas.height;
				filter.texture = new Texture2D(bitmap3dMgr.list[1].frameBuffer.createView());
...
class BlurFilter extends Filter2D
{
	...
	onDraw(texture){
		...
		this.bindGroupDesc.bindings[2].resource = texture;
		renderPassEncoder.setPipeline(this.pipeline);

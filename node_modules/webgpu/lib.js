...
function preparePipelines(){
	...
	Vulkan.pipelineDict.pipeline = device.createRenderPipeline({
...
const onMaterialDraw = function(){
	...
	//const test = []
	return function(entityList){
		const {device, renderPassEncoder, currentMesh:mesh} = Vulkan;
		const textureBindGroupList = mesh.subMeshList.map(subMesh => {
			let texture = Vulkan.fetchTexture(subMesh.texture);
			if(!texture)return;
			const bindGroup = device.createBindGroup({
		...
		for(const {vertexCount, vertexOffset, bindGroup} of textureBindGroupList){
			...
			for(let i=0; i<drawCount; ++i){
				...
				renderPassEncoder.draw(vertexCount, 1, 0, 1);
				//console.log('--', count, this.totalEntityCount, drawCount, uniformBufferList)
			}
		}

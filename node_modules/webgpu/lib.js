...
const {createTextureWithImage} = require('webonly/utils_gpu');
...
Vulkan.loadTexture = createFetch(async name => {
	...
	if(name.toLowerCase().endsWith('.tga')){
		...
		texture = createTextureWithImage(device, imageBitmap);
	}else{
		//const imageBitmap = await createImageBitmap(await Vulkan.loadImage(path), {imageOrientation:'none'});
		texture = createTextureWithImage(device, await loadImage(name));
	}
...
Vulkan.addMeshBufferGetter = mesh => {
	for(let subMesh of mesh.subMeshList){
		...
		for(let [k, v] of Object.entries(subMesh.attributes)){
			...
			defineConstGetter(buffers, k, () => Vulkan.createBufferMapped(usage, v));
		}
		defineConstGetter(subMesh.attributes, 'indexWireframe', calcIndexBufferWireframe);
		defineConstGetter(buffers, 'indexWireframe', () => Vulkan.createBufferMapped(GPUBufferUsage.INDEX, subMesh.attributes.indexWireframe));

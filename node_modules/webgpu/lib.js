...
const backgroundLoader = function(){
	...
	return loader => function(...args){
		return scheduler.postTask(() => loader.apply(this, args), priority);
	}
}();
...
Vulkan.fetchTexture = createFetch(backgroundLoader(function(name){
	const {device} = Vulkan;
	if(name.toLowerCase().endsWith('.tga')){
		...
		return Vulkan.loadTexture(device, name);
	}
}));
Vulkan.loadModel = function(){
	const loadModel = createFetch(async path => Vulkan.addMeshBufferGetter(parse(new Uint8Array(await fetchArrayBuffer(path)))));

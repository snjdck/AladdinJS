...
class Scene3D
{
	...
	update(){
		traverseTree.call(this, this.root, this.onUpdate);
	}
...
class DrawUnitBucket extends ObjectBucket
{
	...
	onUpdateBuffer(instance, i, batchIndex){
		...
		const buffer = new Float32Array(515*4);
		buffer.set(instance.matrix.rawData);
		for(const bone of instance.boneObjectList){
			bone.worldTransform.copyToArray(buffer, (bone.id * 2 + 3) << 2);
		}
		this.transformUniformCache(this.totalEntityCount + batchIndex, 0, buffer);
		/*
		this.transformUniformCache(this.totalEntityCount + batchIndex, 0, new Float32Array([
			...instance.matrix.rawData,
			0,0,0,1,
			0,0,0,1,
		]));*/
	}

...
const {fetchArrayBuffer, backgroundDo} = require('webonly/utils');
...
Vulkan.fetchTexture = createFetch(async name => {
	const {device} = Vulkan;
	if(!name.toLowerCase().endsWith('.tga'))return Vulkan.loadTexture(device, name);
	const data = await fetchArrayBuffer(name);
	let image = await backgroundDo(() => imageUtil.decode(name, data));
...
Vulkan.loadModel = createFetch(async path => Vulkan.addMeshBufferGetter(parse(new Uint8Array(await fetchArrayBuffer(path)))));

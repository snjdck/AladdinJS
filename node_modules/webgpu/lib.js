...
class Scene3D
{
	...
	update(){
		traverseTree.call(this, this.root, onUpdate);
	}
...
const onUpdate = item => item.onUpdate?.();
...
class MeshEntity extends DisplayObject3D
{
	...
	onUpdate(){
		const {animation, animationTime, rootBone} = this;
		if(animation){
		...
		if(this.attachmentDict){
			for(const [attachment, boneName] of this.attachmentDict){
				traverseTree(attachment, onUpdate);
			}
		}
		rootBone?.forEachBone(this, this.onUpdateBone);
	}
...
class DrawUnitBucket extends ObjectBucket
{
	...
	add(element, override){
		if(element.attachmentDict){
			for(const [attachment, boneName] of element.attachmentDict){
				this.add(attachment);
				const {worldTransform:{rotation, translation}} = element.boneDict[boneName];
				let matrix = new Mat3x4();
				matrix.appendRotation(rotation);
				matrix.appendTranslation(translation.x, translation.y, translation.z);
				attachment.matrix = matrix.append(element.matrix);
			}
		}
	...
	onUpdateBuffer(instance, i, batchIndex){
		...
		objectBuffer3D.set(instance.matrix.rawData);
		for(const bone of Object.values((instance.skeleton || instance).boneDict)){
			bone.worldTransform.copyToArray(objectBuffer3D, (bone.id * 2 + 3) << 2);
		}

...
function preparePipelines(){
	...
	Vulkan.pipelineDict.pipelineCull = device.createComputePipeline({
		layout: device.createPipelineLayout({
			bindGroupLayouts:[
				Vulkan.bindGroupLayoutDict.bindGroupLayoutCull,
				Vulkan.bindGroupLayoutDict.bindGroupLayout2,
				Vulkan.bindGroupLayoutDict.bindGroupLayout2,
			]
		}),
	...
	static onDraw(infoList){
		...
		for(const subMesh of currentMesh.subMeshList){
			...
			for(const [index, offset, count] of infoList){
				...
				directBufferMgr.draw(vertexCount, count, vertexOffset, offset);
			}
		}
	}
	constructor(){
...
class ColorMaterial extends Material{
	...
	static onDraw(infoList){
		...
		for(const [index, offset, count] of infoList){
			...
			renderPassEncoder.setBindGroup(1, transformUniformCache(index));
			renderPassEncoder.setBindGroup(2, boneDataCache(index));
			directBufferMgr.draw(currentMesh.vertexCount, count, 0, offset);
		}
	}

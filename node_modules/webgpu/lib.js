...
class ColorMaterial extends Material{
	...
	static draw(entityList, {uniformBindGroupList, totalEntityCount}){
		...
		for(let i=0; i<drawCount; ++i){
			...
			const {bufferList, bindGroupList} = this.uniformCache(i);
			
			renderPassEncoder.setBindGroup(2, bindGroupList[i]);
			const count = Math.min(maxCountPerBatch, entityList.length - i * maxCountPerBatch);
			for(let j=0; j<count; ++j){
				bufferList[i].setSubData(0, new Float32Array(entityList[j].material.color));
			}
			renderPassEncoder.setBindGroup(1, uniformBindGroupList[i]);
	...
	static setBindGroup(entityList){
		const {device, renderPassEncoder} = Vulkan;
		if(!this.uniformCache){
			this.uniformCache = createUniformBufferListCache(Vulkan.bindGroupLayoutDict.bindGroupLayout4, 4 * 4 * this.maxCountPerBatch);
		}
...
function createUniformBufferListCache(layout, bufferSize=0x10000){
	const bufferList = [];
	const bindGroupList = [];
	const result = {bufferList, bindGroupList};
	return function(batchIndex){
		const {device} = Vulkan;

		...
		this.pipeline = device.createRenderPipeline({
			...
			vertexStage: {
				module: device.createShaderModule({
					code: Vulkan.vertex3DCode,
				}),
				entryPoint: 'main'
			},
			fragmentStage: {
				module: device.createShaderModule({
					code: Vulkan.fragment3DCode,
				}),
				entryPoint: 'main'
			},
...
class Filter2D
{
	...
	onDrawBegin(item){
		...
		item.scene.submit();
		this.frameBuffer = Vulkan.bitmap3dMgr.acquire(item.filter, Vulkan.renderPassLevel);
		const rect = item.filter.drawRect;
		this.renderPassDesc.colorAttachments[0].loadValue = [0,0,0,0];
		Vulkan.pushRenderPass(this.renderPassDesc, [this.frameBuffer.createView()]);
		Vulkan.setViewport(rect.x, rect.y, rect.width, rect.height);
...
class BlurFilter extends Filter2D
{
	...
	onDraw(){
		...
		this.bindGroupDesc.bindings[2].resource = this.frameBuffer.createView();
		renderPassEncoder.setPipeline(this.pipeline);
		renderPassEncoder.setBindGroup(0, device.createBindGroup(this.bindGroupDesc));
...
class Bitmap3DManager
{
	...
	acquire(target, level){
		...
		const element = this.list[level % 2];
		target.drawRect = element.rectPack.insert(width, height);
		return element.frameBuffer;
	}
...
class MeshEntity extends DisplayObject3D
{
	constructor(mesh, createSkeleton=true){
		...
		this.subEntityList = mesh.subMeshList.map(subMesh => Object.create(this, {
			...
			vertexBuffer: {value: function(){
				const vertexBuffer = device.createBuffer({
					size: subMesh.vertexData.byteLength,
					usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST
				});
				vertexBuffer.setSubData(0, subMesh.vertexData);
				return vertexBuffer;
			}()},
			uniformBuffer: {value: function(){
				const uniformBuffer = device.createBuffer({
					size: 2 * 4 * 4,
					usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
				});
				uniformBuffer.setSubData(0, new Float32Array([0,0,0,1,0,0,0,1]));
	...
	onDraw(){
		const {textureBindGroupDesc} = this.scene;
		...
		this.subEntityList.forEach(subEntity => {
			textureBindGroupDesc.bindings[0].resource.buffer = subEntity.uniformBuffer;
			textureBindGroupDesc.bindings[1].resource = window.texture.createView();
			renderPassEncoder.setBindGroup(1, device.createBindGroup(textureBindGroupDesc));
			renderPassEncoder.setVertexBuffer(0, subEntity.vertexBuffer);
			renderPassEncoder.draw(subEntity.subMesh.vertexCount, 1, 0, 0);
		})
	}
}

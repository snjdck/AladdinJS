...
class Camera3D extends Transform
{
	...
	draw(root){
		...
		const {drawUnitBucket, uniformBuffer, lens, worldTransformInvert} = this;
...
class Scene3D
{
	...
	draw(){
		...
		if(this.camera.cullEnabled){
			Vulkan.directBufferMgr.prepareCull(this.camera);
		}
...
class FilterBucket
{
	...
	draw(root){
		...
		for(const group of this.list.reverse()){
...
const updateScene2D = function(){
	const matrixStack = [new Matrix2D()];
	function onUpdate(item){
		item.transform.append(this[this.length-1], item.worldTransform);
		this.push(item.worldTransform);
		return this.pop;
	}
	return root => traverseTree.call(matrixStack, root, onUpdate);
}();
...
const drawScene2D = function(){
	...
	return root => traverseTree(root, onDraw);
}();
...
class Scene2D
{
	...
	update(){
		updateScene2D(this.root);
		updateTree(this.root);
	}

	draw(){
		const {renderPassDesc, root} = this;
		const {mainColorTexture, bitmapQueue} = Vulkan;

		this.bitmap3dMgr.draw(root);

		this.filterBucket.draw(root).clear();
...
class DirectBufferManager
{
	constructor(){
		...
		this.bindGroup = device.createBindGroup({
			layout: Vulkan.bindGroupLayoutDict.bindGroupLayoutCull,
			entries:[{
				binding: 0,
		...
		this.boundBindGroup = device.createBindGroup({
			layout: Vulkan.bindGroupLayoutDict.bindGroupLayout2,
			entries:[{
				binding: 0,
				resource: {buffer: this.boundBuffer}
			}]
		});
	...
	draw(subMesh, vertexCount, instanceCount, baseVertex, baseInstance){
		...
		for(let i=0; i<instanceCount; ++i){
			copyDataToBuffer(boundBuffer, new Float32Array([
				500,500,500,0,
				-500,-500,-500,0
			]), (this.totalIndex + i) * 32);
			copyDataToBuffer(buffer, new Uint32Array([vertexCount, 0, baseVertex, baseInstance+i]), offset);
...
*/
	prepareCull(camera){
		...
		computePassEncoder.setBindGroup(0, this.bindGroup);
		computePassEncoder.setBindGroup(2, camera.cullBindGroup);
	...
	setDispatchValue(){
		copyDataToBuffer(this.dispatchBuffer, new Uint32Array([this.totalIndex, 1, 1]));
	}
}
...
class BlurFilter extends Filter2D
{
	constructor(color=[1,0,1,1]){
		...
		this.uniformBuffer2 = createBufferMapped(GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST, new Float32Array(color));
	...
	onDraw(){
		...
		copyDataToBuffer(this.uniformBuffer, new Float32Array([
...
class TextureMaterial extends Material{
	...
	static onDraw(infoList){
		...
		const {renderPassEncoder, device, currentMesh, directBufferMgr} = Vulkan;
		...
		for(const subMesh of currentMesh.subMeshList){
			...
			for(const [index, offset, count] of infoList){
				...
				directBufferMgr.draw(subMesh, vertexCount, count, vertexOffset, offset);
			}
		}
	}
	constructor(){
...
class ColorMaterial extends Material{
	...
	static onDraw(infoList){
		const {transformUniformCache, boneDataCache} = Vulkan.transformUniformMgr;
		const {renderPassEncoder, currentMesh, directBufferMgr} = Vulkan;
		const {maxCountPerBatch} = this;
		for(const [index, offset, count] of infoList){
...
class BoundMaterial extends Material{
	...
	static onDraw(infoList){
		...
		for(const [index, offset, count] of infoList){
			renderPassEncoder.setBindGroup(1, transformUniformCache(index));
			renderPassEncoder.draw(24, 1, 0, offset);
		}
	}
	constructor(bound){
		super();
...
{
	static get pipeline(){
		return Vulkan.pipelineDict.pipelineWireframe;
	}
	static onDraw(infoList){
		const {transformUniformCache, boneDataCache} = Vulkan.transformUniformMgr;
		const {renderPassEncoder, currentMesh:mesh} = Vulkan;
		for(const [index, offset, count] of infoList){
			renderPassEncoder.setBindGroup(1, transformUniformCache(index));
			renderPassEncoder.setBindGroup(2, boneDataCache(index));
			renderPassEncoder.drawIndexed(mesh.wireframeIndexCount, count, 0, 0, offset);
		}
	}
	static setIndexBuffer(){
		const {renderPassEncoder, currentMesh:mesh} = Vulkan;
		if(!mesh._wireframeIndexBuffer){
			...
			mesh._wireframeIndexBuffer = createBufferMapped(GPUBufferUsage.INDEX, indexData);
		}
		renderPassEncoder.setIndexBuffer(mesh._wireframeIndexBuffer);
	}
}
...
function createVertexBufferListCache(size=0x10000){
	...
	return function(batchIndex, offset, value){
		...
		copyDataToBuffer(buffer, value, offset);
	}
}

function createUniformBufferListCache(layout, size=0x4000){
	const bufferList = [];
	const bindGroupList = [];
	return function(batchIndex, offset, value){
		const {device} = Vulkan;
		...
		copyDataToBuffer(uniformBuffer, value, offset);
	}
}
...
class TransformUniformManager
{
	constructor(){
		...
		this.boneDataCache = createUniformBufferListCache(
			Vulkan.bindGroupLayoutDict.bindGroupLayout2
		);

	...
	static draw(entityList, {transformUniformCache, totalEntityCount}){
		...
		for(const subMesh of currentMesh.subMeshList){
			...
			for(let i=0; i<drawCount; ++i){
				...
				renderPassEncoder.setBindGroup(1, transformUniformCache(i));
	...
	static draw(entityList, {transformUniformCache, totalEntityCount}){
		...
		for(let i=0; i<drawCount; ++i){
			...
			for(let j=0; j<count; ++j){
				this.uniformCache(i, 1).setSubData((totalEntityCount + j) * 4 * 4, new Float32Array(entityList[j].material.color));
			}
			renderPassEncoder.setBindGroup(1, transformUniformCache(i));
...
function createUniformBufferListCache(layout, bufferSize=0x10000){
	const bufferList = [];
	const bindGroupList = [];
	const result = [bindGroupList, bufferList];
	return function(batchIndex, index=0){
		const {device} = Vulkan;
		...
		return result[index][batchIndex];
	}
}
...
class DrawUnitBucket extends ObjectBucket
{
	...
	onUpdateBuffer(instance, i, batchIndex){
		const uniformBuffer = this.transformUniformCache(batchIndex, 1);

...
const {createBufferWith, createTextureWithImage} = require('webonly/utils_gpu');
const defineConstGetter = require('utils/object/defineConstGetter');
const {LRUCache} = require('utils/object/LRUCache');

const bufferCache = LRUCache(1000, (data, usage) => createBufferWith(Vulkan.device, usage, data));
...
Vulkan.loadTexture = createFetch(async name => {
	...
	if(name.toLowerCase().endsWith('.tga')){
		...
		texture = createTextureWithImage(Vulkan.device, await loadImage(name));
	}
...
const SubMeshProto = {
	...
	draw(instanceCount=1, firstInstance=0, index='index'){
		const indexData = this.attributes[index];
		if(indexData){
...
Vulkan.addMeshBufferGetter = mesh => {
	for(let subMesh of mesh.subMeshList){
		...
		//	defineConstGetter(buffers, k, () => createBufferWith(Vulkan.device, usage, v));
		//}
		defineConstGetter(subMesh.attributes, 'indexWireframe', calcIndexBufferWireframe);
		//defineConstGetter(buffers, 'indexWireframe', () => createBufferWith(Vulkan.device, GPUBufferUsage.INDEX, subMesh.attributes.indexWireframe));

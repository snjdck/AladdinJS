...
Vulkan.loadTexture = createFetch(async name => {
	...
	if(name.toLowerCase().endsWith('.tga')){
		...
		const image = imageUtil.decode(name, data, true);
		/*
		...
		*/
		texture = createTexture(Vulkan.device, image.width, image.height, 1, GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_DST);
		Vulkan.device.queue.writeTexture(
			{texture, origin:[0,0,0], mipLevel:0, aspect:'all'},
			image.data,
			{offset:0, bytesPerRow: image.width * image.bitPerPixel >> 3, rowsPerImage:undefined},
...
const SubMeshProto = {
	setBuffers(renderPassEncoder, vertex, index='index'){

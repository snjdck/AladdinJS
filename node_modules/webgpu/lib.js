...
class Scene2D
{
	...
	draw(){
		const {vertexBuffer, vertexData} = this;
...
class View3D
{
	constructor(){
		...
		Vulkan.depthStencilAttachment = this.depthStencilAttachment;
	}
	...
	drawScene(){
		const {device, canvas, swapChain} = Vulkan;
		const {depthStencilAttachment} = this;

		const commandEncoder = device.createCommandEncoder();

		Vulkan.commandEncoder = commandEncoder;
		Vulkan.renderPassEncoder = commandEncoder.beginRenderPass({
			colorAttachments: [{
...
class Filter2D
{
	...
	onDrawBegin(item){
		let {renderPassEncoder, depthStencilAttachment, commandEncoder, swapChain} = Vulkan;
		item.scene.submit();

		renderPassEncoder = commandEncoder.beginRenderPass({
			colorAttachments: [{
				attachment: this.frameBuffer.createView(),
				loadValue: allZero
			}],
			depthStencilAttachment
		});

		Vulkan.renderPassEncoder = renderPassEncoder;
	...
	onDrawEnd(item){
		const {filterBindGroup, filterUniformBuffer} = item.filter;
		let {renderPassEncoder, depthStencilAttachment, commandEncoder, swapChain, filterPipeline} = Vulkan;
		item.scene.submit();
		renderPassEncoder = commandEncoder.beginRenderPass({
			colorAttachments: [{
				attachment: swapChain.getCurrentTexture().createView(),

...
class TextureMaterial extends Material{
	...
	static draw(entityList, infoList){
...
class TransformUniformManager
{
	...
	register(instanceList, offset, instanceCount, batchIndex){
...
class DrawUnitBucket extends ObjectBucket
{
	...
	onMaterialDraw(entityList){
		const {currentMaterial} = Vulkan;
		const result = this.batchDraw(currentMaterial.maxCountPerBatch, entityList);
		currentMaterial.draw(entityList, result);
	}

	batchDraw(instanceCountPerDraw, instanceList){
		...
		while(instanceIndex < totalInstanceCount){
			...
			const value = this.onUpdateBufferBatch(instanceList, instanceIndex, instanceCount, drawCount++);
	...
	onUpdateBufferBatch(instanceList, offset, instanceCount, batchIndex){
		return Vulkan.transformUniformMgr.register(instanceList, offset, instanceCount, batchIndex);
	}
}

...
class Camera3D
{
	constructor(){
		...
		//this.entityTypeDict = new ArrayValueMap();
...
*/
	onDraw(item){
		//const klass = item.constructor.Draw;
		if(item.material){
			this.drawUnitBucket.add(item);
		}
		//const klass = this.findClass(item);
		//if(klass)this.entityTypeDict.add(klass, item);
	}
}
...
class Scene3D
{
	...
	draw(){
		...
		renderPassEncoder.setPipeline(Vulkan.pipelineDict.pipeline);
...
class TextureMaterial extends Material{
	...
	static draw(entityList, textureBindGroupList, uniformBindGroupList, totalEntityCount){
		const {renderPassEncoder, device} = Vulkan;
		const {maxCountPerBatch} = this;
		const drawCount = this.calcDrawCount(entityList.length);
		for(const {vertexCount, vertexOffset, textureView} of textureBindGroupList){
			...
			for(let i=0; i<drawCount; ++i){
				...
				renderPassEncoder.setBindGroup(1, uniformBindGroupList[i]);
				renderPassEncoder.draw(vertexCount, count, vertexOffset, totalEntityCount);
			}
		}
	}
	constructor(){
...
class ColorMaterial extends Material{
	...
	static draw(entityList, textureBindGroupList, uniformBindGroupList, totalEntityCount){
		...
		const {maxCountPerBatch} = this;
		const drawCount = this.calcDrawCount(entityList.length);
		this.setBindGroup(entityList);
		for(let i=0; i<drawCount; ++i){
			const count = Math.min(maxCountPerBatch, entityList.length - i * maxCountPerBatch);
			renderPassEncoder.setBindGroup(1, uniformBindGroupList[i]);
			renderPassEncoder.draw(currentMesh.vertexCount, count, 0, totalEntityCount);
		}
	}
	static setBindGroup(entityList){
		const {device, renderPassEncoder} = Vulkan;
		...
		if(entityList.length > 0){
			this.uniformBuffer.setSubData(0, new Float32Array(entityList[0].material.color));
		}
		renderPassEncoder.setBindGroup(2, device.createBindGroup({
			layout: Vulkan.bindGroupLayoutDict.bindGroupLayout4,
			bindings: [{
				binding: 0,
				resource: {buffer: this.uniformBuffer}
			}]
...
class DrawUnitBucket extends ObjectBucket
{
	...
	draw(){
		...
		this.forEach(onMaterialDraw, this);
	}

	onUpdateBuffer(instance, i, batchIndex){
		const {device} = Vulkan;
		const {uniformBufferList, uniformBindGroupList} = this;
		let uniformBuffer = uniformBufferList[batchIndex];
		if(uniformBuffer == null){
			uniformBuffer = uniformBufferList[batchIndex] = device.createBuffer({
				size: 0x10000,
				usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
			});
			uniformBindGroupList[batchIndex] = device.createBindGroup({
		...
		uniformBuffer.setSubData(3*4*4 * (i + this.totalEntityCount), instance.matrix.rawData);
	}

	onUploadBufferAndDraw(instanceCount){}
}

const onMaterialDraw = function(){
	//const uniformBufferList = [];
	//const uniformBindGroupList = [];
	return function(entityList){
		const {device, renderPassEncoder, currentMesh:mesh, currentMaterial} = Vulkan;
		const textureBindGroupList = mesh.subMeshList.map(subMesh => {
			if(!subMesh.texture)return;
			let texture = Vulkan.fetchTexture(subMesh.texture);
			if(!texture)return;
			const textureView = texture.createView();
			const {vertexCount, vertexOffset} = subMesh;
			return {vertexCount, vertexOffset, textureView};
		}).filter(Boolean);
		batchDraw.call(this, currentMaterial.maxCountPerBatch, entityList);
		
		/*
		const drawCount = currentMaterial.calcDrawCount(entityList.length);
		let entityIndex = 0;
		for(let i=0; i<drawCount; ++i){
			const index = i;
			let uniformBuffer = uniformBufferList[index];
			if(uniformBuffer == null){
				uniformBuffer = uniformBufferList[index] = device.createBuffer({
					size: 0x10000,
					usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
				});
				uniformBindGroupList[index] = device.createBindGroup({
					layout: Vulkan.bindGroupLayoutDict.bindGroupLayout2,
					bindings: [{
						binding: 0,
						resource: {buffer: uniformBuffer}
			...
			const count = Math.min(currentMaterial.maxCountPerBatch, entityList.length - entityIndex);
			for(let j=0; j<count; ++j){
				uniformBuffer.setSubData(3*4*4 * (j + this.totalEntityCount), entityList[entityIndex].matrix.rawData);
				++entityIndex;
			}
		}
		*/
		currentMaterial.draw(entityList, textureBindGroupList, this.uniformBindGroupList, this.totalEntityCount);
...
function updateMesh(mesh){
	...
	if(!mesh._vertexBuffer){
		...
		const vertexBuffer = mesh._vertexBuffer = device.createBuffer({

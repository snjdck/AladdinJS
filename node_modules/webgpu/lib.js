...
class MeshEntity extends DisplayObject3D
{
	...
	static Draw = function(){
		const meshDict = new ArrayValueMap();
		const countPerBatch = 10;
		const bytePerInstance = 5 * 4 * 4;
		const uniformBufferList = [];
		const uniformBindGroupList = [];
		return function(entityList){
			const {device, renderPassEncoder} = Vulkan;
			for(const entity of entityList){
				meshDict.add(entity.mesh, entity);
			}
			for(const [mesh, entityList] of meshDict){
				...
				const textureBindGroupList = mesh.subMeshList.map(subMesh => {
					...
					return {vertexCount, vertexOffset, bindGroup};
				}).filter(Boolean);
				const drawCount = Math.ceil(entityList.length / countPerBatch);
				for(let i=0; i<drawCount; ++i){
					let uniformBuffer = uniformBufferList[i];
					if(uniformBuffer == null){
						uniformBuffer = uniformBufferList[i] = device.createBuffer({
							size: bytePerInstance * countPerBatch,
							usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
						});
						uniformBindGroupList[i] = device.createBindGroup({
					...
					const count = Math.min(countPerBatch, entityList.length - i * countPerBatch);
					for(let j=0; j<count; ++j){
						const index = i * countPerBatch + j;
						uniformBuffer.setSubData(3*4*4 * j, new Float32Array([
							...entityList[index].matrix.rawData,
							0,0,0,1,
							0,0,0,1
						]));
					}
				}
				for(const {vertexCount, vertexOffset, bindGroup} of textureBindGroupList){
					renderPassEncoder.setBindGroup(2, bindGroup);
					for(let i=0; i<drawCount; ++i){
						const count = Math.min(countPerBatch, entityList.length - i * countPerBatch);
...
const onMaterialDraw = function(){
	...
	return function(entityList){
		...
		for(let i=0; i<drawCount; ++i){
			...
			for(let j=0; j<count; ++j){
				const index = i * countPerBatch + j;
				uniformBuffer.setSubData(3*4*4 * (j + this.totalEntityCount), entityList[index].matrix.rawData);
			}
		}
		currentMaterial.setBindGroup(entityList);
		for(const {vertexCount, vertexOffset, bindGroup} of textureBindGroupList){
			renderPassEncoder.setBindGroup(2, bindGroup);
			for(let i=0; i<drawCount; ++i){
				const count = Math.min(countPerBatch, entityList.length - i * countPerBatch);
				renderPassEncoder.setBindGroup(1, uniformBindGroupList[i]);
				renderPassEncoder.draw(vertexCount, 1, vertexOffset, this.totalEntityCount);
			}
		}

...
const SubMeshProto = {
	setVertexBuffer(keyList){
		for(let i=0, n=keyList.length; i<n; ++i){
			Vulkan.renderPassEncoder.setVertexBuffer(i, this.buffers[keyList[i]]);
		}
	},
	setIndexBuffer(key='index'){
		const list = this.attributes[key];
		const format = list instanceof Uint16Array ? 'uint16' : 'uint32';
		Vulkan.renderPassEncoder.setIndexBuffer(this.buffers[key], format);
	},
	draw(instanceCount, firstInstance=0, key='index'){
		const list = this.attributes[key];
		if(list){
			Vulkan.renderPassEncoder.drawIndexed(list.length, instanceCount, 0, 0, firstInstance);
		}else{

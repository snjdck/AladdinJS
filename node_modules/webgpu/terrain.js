...
const {getImageJpegData} = require('webonly/utils_canvas');
const {loadImage} = require('webonly/utils');
const {createMeshIndices} = require('utils/geometry');

module.exports = async function(){
	const {data:lightData} = getImageJpegData(await loadImage('./assets/World1/TerrainLight.jpg'));
	const {data:heightData} = decodeBmp(await fetch('./assets/World1/TerrainHeight.bmp').then(v => v.arrayBuffer()));
	const mapData = new Uint8Array(await fetch('./assets/World1/Terrain.map').then(v => v.arrayBuffer()));

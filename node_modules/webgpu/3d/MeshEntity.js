...
class MeshEntity extends DisplayObject3D
{
	constructor(mesh, createSkeleton=true){
		...
		this.boneState = this.rootBone.buildBoneState(this);
...
void function(){
	MeshEntity.prototype.bindObjectToBone = function(boneName, target, clearOtherObjects=true){
		if(clearOtherObjects)this.unbindObjects(boneName);
		this.findChildByName(boneName)?.addChild(target);
	}
	MeshEntity.prototype.unbindObjects = function(boneName){
		unbindObjects(this, false, boneName);
	}
	const unbindObjects = walkTree(function(child, boneName){
		if(boneName){
			if(child.type == 'bone' && child.name == boneName){
				for(let v of child.children){
					if(v.type == 'bone')continue;
					child.removeChild(v);
				}
				return true;
			}
...
MeshEntity.prototype.copyBoneStateToTempBuffer = function(){
	...
	return function(){
		...
		for(let i=0, n=boneState.length; i<n; ++i){
			if(!boneState[i])continue;
			boneState[i].calculateRelativeTransform(this, tempMatrix);
			let {rotation, translation, scale} = Matrix3D.decompose(tempMatrix.rawData)
			let offset = i << 3;
			rotation.copyToArray(buffer, offset);
...
const PlayAnimationAction = PlayAnimationListAction((target, animation, time) => target.rootBone.updateBone(animation, time, target.boneState));
/*
...
MeshEntity.prototype.playAnimationByIndex = function(index=0, mode='repeat'){
	const value = this.mesh.animationList[index];
	if(!value)return;
...
MeshEntity.prototype.playAnimationByName = function(name, mode='repeat'){
	let index = this.mesh.animationList.findIndex(v => v.name == name);
	return this.playAnimationByIndex(index, mode);
}

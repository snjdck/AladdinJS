...
class View3D
{
	constructor(){
		...
		mouseEvents.mouseDownSignal.add(evt => {
			forEachDo(this.picking.getTarget(), target => target.mouseDownSignal.notify(evt));
		})
		mouseEvents.mouseUpSignal.add(evt => {
			forEachDo(this.picking.getTarget(), target => target.mouseUpSignal.notify(evt));
		})

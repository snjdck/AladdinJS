...
class View3D
{
	...
	render(encoder){
		...
		this.drawPick(encoder, {x:mouseEvents.mouseX, y:mouseEvents.mouseY, width:1, height:1}, this.onBufferRead);
	}
	...
	drawPick(encoder, rect, callback){
		const {scene3d, scene2d} = this;
		const pickColorTexture = encoder.fetchTempTexture(rect, 'rgba32float,0x11');
		const pickDepthTexture = encoder.fetchTempTexture(rect, 'depth24plus-stencil8,0x10');
		...
		const pickBuffer = encoder.fetchTempBuffer(16 * rect.width * rect.height, GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST);
		this.picking.reset();
		for(let commandEncoder of encoder.createCommandEncoder()){
			...
			commandEncoder.copyTextureToBuffer({texture:pickColorTexture}, {buffer:pickBuffer}, pickColorTexture);
		}
		encoder.readBuffer(pickBuffer, callback.bind(this, this.picking.itemMap));
	}
}

...
class View3D
{
	constructor(){
		...
		this.onBufferRead = arrayBuffer => {
			const value = new Uint32Array(arrayBuffer)[0];
			this.picking.setTarget(Material.itemDict.get(value));
		}
	}
	...
	render(encoder){
		...
		const pickColorTexture = encoder.fetchTempTexture(pickTextureSize, '2d,r32uint,0x11');
		const pickDepthTexture = encoder.fetchTempTexture(pickTextureSize, '2d,depth24plus-stencil8,0x10');
		...
		const colorTexture = encoder.fetchTempTexture(canvasSize, '2d,bgra8unorm,0x11');
		const depthTexture = encoder.fetchTempTexture(canvasSize, '2d,depth24plus-stencil8,0x14');
		...
		for(let commandEncoder of encoder.createCommandEncoder()){
			...
			for(let renderPassEncoder of commandEncoder.beginRenderPass('[0,0,0,0]+store;1+discard;0+discard', pickColorView, pickDepthStencilView)){
				scene3d.camera.drawPick(renderPassEncoder, mouseEvents.mouseX, mouseEvents.mouseY);
			}
			for(let renderPassEncoder of commandEncoder.beginRenderPass('load+store', pickColorView)){
				scene2d.camera.drawPick(renderPassEncoder, mouseEvents.mouseX, mouseEvents.mouseY);
			}
			commandEncoder.copyTextureToBuffer({texture:pickColorTexture}, {buffer:pickBuffer}, pickTextureSize);
		}
		encoder.readBuffer(pickBuffer, this.onBufferRead);
	}
}

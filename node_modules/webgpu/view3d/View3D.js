...
class View3D
{
	...
	render(encoder){
		...
		while(taskList.length > 0){
			const [rect, callback] = taskList.shift();
			this.drawPick(encoder, rect, (itemMap, arrayBuffer) => {
	...
	drawPick(encoder, rect, callback){
		...
		const colorView = encoder.fetchTextureView(colorTexture);
		const depthStencilView = encoder.fetchTextureView(depthTexture);
		const buffer = encoder.fetchTempBuffer(rect.height > 1 ? bytesPerRow * rect.height : rect.width << 4, GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST);
		this.picking.reset();
		for(let commandEncoder of encoder.createCommandEncoder()){
			...
			commandEncoder.copyTextureToBuffer({texture:colorTexture}, {buffer, bytesPerRow: rect.height > 1 ? bytesPerRow : undefined}, colorTexture);
		}
		encoder.readBuffer(buffer, callback.bind(this, this.picking.itemMap));
	}

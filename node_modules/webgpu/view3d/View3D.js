...
const {fetchWorldMatrixListBuffer, fetchBoneStateTexture} = require('../materials/helper');

const collectWorldMatrixNodes = walkTree(function(item, result){
	...
	for(const [attachment, boneID] of item.attachmentDict){
		...
		collectWorldMatrixNodes.call(this, attachment, true, result);
	}
});

const collectBoneStateNodes = walkTree(function(item, result){
	if(!item.visible)return true;
	if(item.skeleton?.boneState)result.add(item.skeleton);
	if(!item.hasAttachment?.())return;
	for(const [attachment] of item.attachmentDict){
		collectBoneStateNodes.call(this, attachment, true, result);
	}
});

const collectBitmap3DNodes = walkTree(function(item, result){
	if(!item.visible)return true;
	if(item.type == 'bitmap3d'){
		result.add(item.root3d);
	}
});
...
class View3D
{
	...
	render(encoder){
		...
		const worldMatrixNodes = [];
		const skeletonNodes = new Set();
		collectWorldMatrixNodes(scene3d, false, worldMatrixNodes);
		collectBoneStateNodes(scene3d, false, skeletonNodes);
		const bitmap3dList = new Set();
		collectBitmap3DNodes(scene2d, false, bitmap3dList);
		for(let item of bitmap3dList){
			collectWorldMatrixNodes(item, false, worldMatrixNodes);
			collectBoneStateNodes(item, false, skeletonNodes);
		}
		encoder.data.worldMatrixList = fetchWorldMatrixListBuffer(encoder, worldMatrixNodes);
		encoder.data.boneStateTexture = fetchBoneStateTexture(encoder, Array.from(skeletonNodes));

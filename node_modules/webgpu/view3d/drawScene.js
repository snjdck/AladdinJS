...
const drawBitmap3D = ElementCollectorFactory(
	new FrameBuffer(true), ({width, height}) => [width, height],
	function(item){
...
const drawFilter2D = (drawUnitBucket => ElementCollectorFactory(
	new FrameBufferList(false), () => [Vulkan.canvas.width, Vulkan.canvas.height],
	function(level){
		const onLeave = () => --level;
...
const drawScene2D = drawSceneFactory(new FrameBuffer(false, 'load'), function(){
	Vulkan.viewportMgr.addViewPort(Vulkan.canvas);
	Vulkan.viewportMgr.collectDrawUnits(this);
	Vulkan.viewportMgr.draw();
});

const drawScene3D = drawSceneFactory(new FrameBuffer(true, [0,0,0,1]), function(){
	Vulkan.cameraMgr.collectDrawUnits(this.camera, this);
	Vulkan.cameraMgr.draw();
});

module.exports = function({scene3d, scene2d}){
	drawScene3D(scene3d);

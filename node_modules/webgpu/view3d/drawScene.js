...
const drawFilter2D = ((drawUnitBucket, matrixInv, copyList) => ElementCollectorFactory(
	...
	function*(){
		...
		Vulkan.renderPassEncoder.setBindGroup(0, Vulkan.createBindGroup('filter2D', Vulkan.getSampler('linear_clamp'), this.getColorView(0)));
		yield drawUnitBucket.draw().clear();
		while(copyList.length > 0){
			const [sx, sy, dx, dy, w, h] = copyList.pop();
			Vulkan.commandEncoder.copyTextureToTexture(
				{texture: this.colorBuffer, origin: [sx, sy, 0]},
				{texture: this.colorBuffer, origin: [dx, dy, 1]},
				[w, h, 1]
			);
		}
	},
	(elementList, drawFn) => elementList.reverse().forEach(drawFn)
))(new DrawUnitBucket(), new Matrix2D(), []);
...
const drawScene = drawSceneFactory(new FrameBufferBase({}, [0,0,0,1]), function(){

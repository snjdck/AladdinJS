...
const fetchView3D = LRUCache(1000, ({width, height}) => {
	console.log('new ');
...
const drawBitmap3D = function(){
	...
	return root => {
		collectFn(root);
		for(let bitmap3D of elementList){
			let {colorView, depthView} = fetchView3D(bitmap3D);
			bitmap3D.texture.rawTexture = colorView;
			bitmap3D.texture.width = bitmap3D.width;
			bitmap3D.texture.height = bitmap3D.height;

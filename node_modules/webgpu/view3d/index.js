...
const calcViewport = function(){
	...
	return (frameRect, drawRect, rootMatrix) => {
...
const collectDrawUnits = function(){
	const fn = walkTree(function(item, root, forceDrawFlag, result){
		...
		if(item.filter && !(item == root && forceDrawFlag)){
			...
			result.push(Object.create(item));
		}
	});
	return (root, forceDrawFlag=false, result=[]) => fn(root, true, root, forceDrawFlag, result) ?? result;
}();

function drawViewport(frameRect, drawRect, rootMatrix, useDepthStencilFlag=false, drawUnitBucket){
	if(drawUnitBucket.length <= 0)return;

	const data = calcViewport(frameRect, drawRect, rootMatrix)
...
const drawFn = item => {
	...
	renderPass2D(() => drawViewport(frameRect, drawRect, calcViewPortMatrix(item, areaRect), false, collectDrawUnits(item, true)), view0);
...
module.exports = class extends View3D {
	constructor(){
		...
		this.addDrawTask(view3d => {
			drawViewport(Vulkan.canvas, null, null, true, collectDrawUnits(view3d.scene2d));
		});
	}
}

...
function drawViewport(frameRect, rootMatrix, drawUnitBucket){
	...
	const bufferInfo = fetchBuffer({usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST, size: data.byteLength});
	Vulkan.device.queue.writeBuffer(bufferInfo.buffer, 0, data);
	Material2D.setBindGroup(0, [{binding: 0, resource:bufferInfo}]);
...
const drawFn = item => {
	...
	if(item.filter.justCopyFlag){
		...
		renderPass2D(() => {
			...
			materialType.setBindGroup(0, [{binding:1, resource:view0}]);
...
function drawScene2D(root){
	...
	for(let item of visitByDepth(root).reverse()){
		if(item.type == 'bitmap3d'){
			...
			renderPass3D(() => item.camera.draw(), colorView, depthView);
		}
		if(item.filter){
			drawFn(item);
		}

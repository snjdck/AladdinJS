...
type my_struct = [[block]] struct {
	[[offset(0)]] screenWH : vec2<f32>;
};

[[binding(0), set(0)]]
var<uniform> uniforms: my_struct;
...
fn main() -> void {
	...
	var inputMargin : vec4<f32> = select(vec4<f32>(1), vec4<f32>(0), vec2<i32>(vertexID & 3, vertexID >> 2).xxyy == vec2<i32>(1, 2).xyxy);
	inputMargin.yw = -inputMargin.yw;
	...
	xyuv.xy = fma(xyuv.xy, uniforms.screenWH, vec2<f32>(-1, 1));
...
const fragmentShaderGLSL = `
	...
	[[binding(1), set(0)]]
	var<uniform_constant> mySampler: sampler;

	[[binding(0), set(1)]]
	var<uniform_constant> myTexture: texture_sampled_2d<f32>;

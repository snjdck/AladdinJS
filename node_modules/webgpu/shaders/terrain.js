...
const fragmentShaderGLSL = `
	...
	[[binding(0), group(1)]] var _texture: texture_2d_array<f32>;
	...
	fn main(input: Input) -> Output {
		var output: Output;
		output.color = mix(
			textureSample(_texture, _sampler, vec2<f32>(input.uv), i32(input.layer.x)),
			textureSample(_texture, _sampler, vec2<f32>(input.uv), i32(input.layer.y)),
			input.layer.zzzz
		) * input.light;
		return output;
	}

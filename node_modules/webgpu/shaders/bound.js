...
const vertex3DShaderGLSL = `
	...
	[[block]] struct MVP_BLOCK {
		...
		[[offset(64)]] cameraMatrix: mat4x3<f32>;
	};
	...
	[[builtin(instance_idx)]] var<in> gl_InstanceIndex: i32;
	[[builtin(position)]] var<out> gl_Position: vec4<f32>;

	[[stage(vertex)]]
	fn main() -> void {
		var vertexID : i32 = indexList[gl_VertexIndex];
		var xyz : vec3<i32> = vec3<i32>(vertexID & 1, (vertexID >> 1) & 1, vertexID >> 2);
		var result : vec3<f32>;

		result[0] = bound.boundList[gl_InstanceIndex << 4 | xyz[0] ][0];
		result[1] = bound.boundList[gl_InstanceIndex << 4 | xyz[1] ][1];
		result[2] = bound.boundList[gl_InstanceIndex << 4 | xyz[2] ][2];

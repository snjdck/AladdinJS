...
const filterVertexShaderGLSL = `
	...
	[[builtin(vertex_index)]] var<in> vertex_index: i32;
	[[builtin(instance_index)]] var<in> instance_index: u32;

	[[location(0)]] var<in> sourceRect: vec4<f32>;
	[[location(0)]] var<out> uv: vec2<f32>;
	[[location(1)]] var<out> instanceIndex: u32;//flat
	...
	fn main() -> void {
		...
		var vertexXY: vec2<f32> = vec2<f32>(vec2<i32>(vertex_index >> 1u, vertex_index & 1));
		gl_Position = vec4<f32>(fma(vertexXY, destRect.zw, destRect.xy), 0.0, 1.0);
		uv = fma(vertexXY, sourceRect.zw, sourceRect.xy);
		instanceIndex = instance_index << 4u;//min bindgroup is 256 bytes, 16 of vec4<f32>
	}
`;
const filterFragmentShaderGLSL = `
	...
	[[block]] struct Color {
		[[offset(0)]] mixColor : array<vec4<f32>, 100>;
	};

	[[binding(0), group(1)]] var<uniform> _color: Color;

	[[location(0)]] var<in> uv: vec2<f32>;
	[[location(1)]] var<in> instanceIndex: u32;
	...
	fn main() -> void {
		...
		outColor = sum * _color.mixColor[instanceIndex];
	}
`;
//*/

...
const filterFragmentShaderGLSL = `
	...
	[[binding(0)]] var<uniform_constant> _sampler: sampler;
	[[binding(1)]] var<uniform_constant> _texture: texture_sampled_2d<f32>;

	type Color = [[block]] struct {
	...
	fn main() -> void {
		...
		var tc0 : vec2<f32> = uv + vec2<f32>(-s,   -t);
		var tc1 : vec2<f32> = uv + vec2<f32>( 0.0, -t);
		var tc2 : vec2<f32> = uv + vec2<f32>( s,   -t);
		var tc3 : vec2<f32> = uv + vec2<f32>(-s,  0.0);
		var tc4 : vec2<f32> = uv;
		var tc5 : vec2<f32> = uv + vec2<f32>( s,    0.0);
		var tc6 : vec2<f32> = uv + vec2<f32>(-s,    t);

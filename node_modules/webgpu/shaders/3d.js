...
uniform BONE_BLOCK {
	EntityInfo entityInfoList[4];
};

#define entityInfo entityInfoList[gl_InstanceIndex]
...
void main()
{
	...
	for(int i=0; i<1; ++i){
		uint index = boneIndex << 1;//every bone use two registers
		worldPosition += transform2(entityInfo.boneList[index], entityInfo.boneList[index+1], inputPosition) * 1.0;
	}
	worldPosition = vec4(worldPosition, 1) * entityInfo.worldMatrix;

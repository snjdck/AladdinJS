...
const {LRUCache} = require('utils/gc');
...
EncoderEx.prototype.loadTexture = function(){
	async function loadTexture(device, path){
	...
	async function loadTextureTga(device, path){
		const ba = new Uint8Array(await fetchArrayBuffer(path));
		/*--head begin--*/
	...
	return createFetch(function(path){
		if(path.toLowerCase().endsWith('.tga')){
			return loadTextureTga(this.device, path);
		}
		return loadTexture(this.device, path);
	}, LRUCache(60 * 1000))
}();

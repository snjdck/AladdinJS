...
class DirectBufferManager
{
	...
	onFrameBegin(){
		this.totalIndex = 0;
		console.log('DirectBufferManager');
	}
	...
	draw(subMesh, vertexCount, instanceCount, baseVertex, baseInstance){
		...
		for(let i=0; i<instanceCount; ++i){
			Vulkan.device.queue.writeBuffer(boundGroup.getBuffer(), (this.totalIndex + i) * 32, new Float32Array([
				500,500,500,0,
				-500,-500,-500,0
			]));
			Vulkan.device.queue.writeBuffer(group.getBuffer(), offset, new Uint32Array([vertexCount, 0, baseVertex, baseInstance+i]));
...
*/
	...
	setDispatchValue(){
		Vulkan.device.queue.writeBuffer(this.dispatchBuffer, 0, new Uint32Array([this.totalIndex, 1, 1]));
	}
}

...
const pipelineQuad = TextureMaterial.pipeline.clone({
	groups: [
		...
		[{name:'baseTexture', texture:{}}, {name:'uvScale', type:'vec2<f32>', buffer:{}}]
	],
	vertex: {
		buffers:undefined,
		code: `
			...
			@vertex Output {
				var vertex: vec2<f32>;
				vertex.x = f32(vertexIndex >> 1);
				vertex.y = f32(vertexIndex & 1);
				...
				output.uv = vec2(1-vertex.x, vertex.y) * uvScale;
				return output;
			}
		`
	},
...
const pipeline = TextureMaterial.pipeline.clone({
	...
	vertex:{code:`
		...
		buffers: [{name: 'position', arrayStride: 4 << 2, attributes: [{shaderLocation:0, offset: 0, format:'float32x3'}]}],
	},
	fragment: {
		targets:[{
			writeMask: 0,

...
const {calcBindGroupLayoutSign, calcPipelineLayoutSign} = require('webonly/utils_gpu');
...
const createBindGroup = function(){
	const fn = (resource, binding) => ({binding, resource});
	return (device, layout, entries) => device.createBindGroup({layout, entries: entries.map(fn)});
}();
/*
...
class Material {
	...
	static Subclass(pipelineDict, bindGroupSlots={}){
		...
		Sub.createBindGroup = (index, resources) => createBindGroup(Vulkan.device, bindGroupLayoutCache(pipelineDesc.groups[index]), resources);

...
const drawDepth = function(){
	const groupFn = depthType => drawUnit => drawUnit.material[depthType]?.constructor;
	const fnDict = {depth:groupFn('depth'), pick:groupFn('pick')};
	return (depthType, renderPassEncoder, list) => {
		for(let [materialType, drawUnitList] of Map.groupBy(list, fnDict[depthType])){
			materialType?.draw(drawUnitList, renderPassEncoder);
		}
	}
}();

Material.drawDepth = drawDepth.bind(null, 'depth');
Material.drawPick = drawDepth.bind(null, 'pick');

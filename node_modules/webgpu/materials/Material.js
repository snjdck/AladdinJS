...
function createComputePipeline(desc){
	const {compute} = desc;
	if(!desc.layout){
		desc.layout = pipelineLayoutCache(desc.groups)
	}
	if(!compute.module){
		compute.module = shaderCache(compute.code);
	}
	return Vulkan.device.createComputePipeline(desc);
}
...
class Material {
	...
	static Subclass(pipelineDesc, groupFnList){
		const createPipeline = pipelineDesc.compute ? createComputePipeline : createRenderPipeline;
		const Sub = class extends this {};
		if(groupFnList)Sub.groupFn = newGroupFn(groupFnList);
		Sub.pipelineDesc = pipelineDesc;
		defineConstGetter(Sub, 'pipeline', () => createPipeline(pipelineDesc));
	...
	static setBindGroup(renderPassEncoder, index, entries){
		renderPassEncoder.setBindGroup(index, this.createBindGroup(index, entries));
	}

	static setVertexBuffer(renderPassEncoder, index, value){

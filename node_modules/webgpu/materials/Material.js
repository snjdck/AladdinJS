	...
	return groups => (index, entries) => {
		...
		for(let entry of entries){
			...
			if(item.buffer){
				...
				const {buffer:data, size, dataOffset, dataSize} = entry.resource;
				const {buffer} = fetchBuffer({usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST, size: size ?? data.byteLength});
				Vulkan.device.queue.writeBuffer(buffer, 0, data, dataOffset, dataSize);
...
const groupMeshFn = drawUnit => drawUnit.entity.mesh;
function* cut(list, limit){
	const {length} = list;
...
class Material {
	...
	static Subclass(pipelineDict){
		const Sub = class extends this {};
		Sub.pipelineDict = pipelineDict;
	...
	static draw(entityList){
		...
		if(maxCountPerBatch <= 1){
			for(let drawUnit of entityList){
				drawUnit.material.onDraw(drawUnit);
			}
		}else if(this.useMeshFlag === true){
			for(let [mesh, subEntityList] of Map.grouBy(entityList, groupMeshFn)){
				for(let v of cut(subEntityList, maxCountPerBatch)){
					this.onDrawBatched(v, mesh);
				}
			}
		}else{
			for(let v of cut(entityList, maxCountPerBatch)){
				this.onDrawBatched(v);
			}
		}
	}
	...
	static setVertexBuffer(index, value){
		if(value instanceof GPUBuffer){
			...
			const {buffer} = fetchBuffer({usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST, size: value.byteLength});
			Vulkan.device.queue.writeBuffer(buffer, 0, value);
			Vulkan.renderPassEncoder.setVertexBuffer(index, buffer);
		}	
	}
}

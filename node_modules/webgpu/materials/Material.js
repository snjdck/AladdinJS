...
class Material{
	...
	*/
	...
	static onDraw(entityList, bindGroupInfoList, ...args){
		...
		const bindGroupSlots = this.bindGroupSlots();
		const bindInfoList = this.bindInfoList;
		...
		for(let i=0, n=entityList.length; i<n;){
			for(let j=bindGroupInfoList.length-1; j>=0; --j){
				if(i < offsetList[j])continue;
				const [batchIndex, bufferOffset, instanceCount] = bindGroupInfoList[j][indexList[j]++];
				offsetList[j] += instanceCount;
				const bindSlot = bindGroupSlots[j];
				const [bindIndex, multiply] = bindInfoList[j];
				renderPassEncoder.setBindGroup(bindIndex, bindSlot.getBindGroup(batchIndex), [bufferOffset << multiply]);
			}
			const offsetEnd = Math.min(...offsetList);
			this.doDraw(offsetEnd - i, ...args);
			i = offsetEnd;
		}
	}
}

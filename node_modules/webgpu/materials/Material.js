	...
	return groups => (index, entries) => {
		...
		for(let entry of entries){
			...
			if(item.buffer){
				...
				const buffer = fetchBuffer.fetch(bufferSize, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST);
...
class Material {
	...
	static Subclass(pipelineDesc, groupFnList){
		...
		defineConstGetter(Sub, 'pipeline', () => createRenderPipeline(pipelineDesc));
	...
	static setVertexBuffer(index, value){
		if(Array.isArray(value)){
			...
			const buffer = fetchBuffer.fetch(size, GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST);
			...
			const buffer = fetchBuffer.fetch(value.byteLength, GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST);//buffer may larger than request size

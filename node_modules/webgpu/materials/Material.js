...
class Material {
	...
	static Subclass(pipelineDesc, bindGroupSlots={}){
		...
		Sub.pipelineLayout = pipelineDesc.layout;
	...
	static setBindGroup(index, ...resourceList){
		const name = this.pipelineLayout[index];
		const bindGroup = Vulkan.createBindGroup(name, ...resourceList);
		Vulkan.renderPassEncoder.setBindGroup(index, bindGroup);
	}
}

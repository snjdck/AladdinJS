...
class Material {
	...
	static Subclass(pipelineDict){
		...
		Vulkan.bindMaterialSlots(Sub, findbindGroupSlots(pipelineDesc));
		return Sub;
	}
	...
	static draw(entityList, mesh){
		...
		if(this.name == 'TerrainMaterial' || this.name == 'BlurFilter'){
			if(entityList.length > 1){
				console.log(this.name, entityList.length);
			}
			...
			if(maxCountPerBatch <= 1){
				...
				for(let i=0, n=entityList.length; i<n; i+=maxCountPerBatch){
					this.onDrawBatched(entityList.slice(i, i+maxCountPerBatch));
				}
			}
			return;
		}

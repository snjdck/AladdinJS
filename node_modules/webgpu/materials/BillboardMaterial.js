...
const mapFn = drawUnit => Array.from(drawUnit.entity.worldTransform.rawData);
const groupFn = drawUnit => drawUnit.material.texture;
...
class BillboardMaterial extends Material.Subclass({pipeline}){
	...
	static onDrawBatched(drawUnitList){
		for(const [texture, subEntityList] of Map.groupBy(drawUnitList, groupFn)){
			this.setBindGroup(1, [{binding:0, resource:texture.createView()}]);
			this.setVertexBuffer(0, Float32Array.from(subEntityList.flatMap(mapFn)));
			Vulkan.renderPassEncoder.draw(4, subEntityList.length);
		}
	}
}

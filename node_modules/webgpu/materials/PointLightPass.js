...
const pipeline = new Pipeline({
	...
	fragment: {
		...
		code: `
			...
			@group(0) @binding(3) var texture_depth: texture_depth_cube;
			...
			fn main(
				...
				let lightPt = vec3<f32>(1000,1000,500);
				...
				let lightDir = normalize(lightPt - worldPosition);
				let lightPower = length(lightPt - worldPosition);
				
				let screenPosition: vec4<f32> = vec4<f32>(worldPosition - lightPt, 1) * light[0].screenMatrix;

				let ab = textureSample(texture_depth, sampler_non_filtering, -lightDir);

				//depth_ref <= the sampled depth
				//let depth_value = textureSampleCompare(texture_depth, sampler_depth, -lightDir, screenPosition.z / screenPosition.w - 0.001);
				let depth_value = textureSampleCompare(texture_depth, sampler_depth, -lightDir, 0);

				let normal = calcNormal(textureSample(texture_normal, sampler_non_filtering, uv).xy);
				let factor = dot(normal, lightDir);
				let baseColor = textureSample(texture_color, sampler_filtering, uv);

				//return vec4<f32>(baseColor.rgb * factor * depth_value, 1);
				return vec4<f32>(vec3<f32>(1 - lightPower / lightRange) * factor * ab, 1);
			}
		`
	},
	primitive: {

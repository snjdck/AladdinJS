...
const pipeline02 = {
	...
	compute: {
		...
		code: pipeline01.compute.code
	}
};

const Test01 = Material.Subclass(pipeline01);
const Test02 = Material.Subclass(pipeline02);
			...
			for(let subMesh of mesh.subMeshList){
				...
				for(let computePassEncoder of ComputePass(commandEncoder)){
					if(subMesh.attributes.index){
						...
						computePassEncoder.setPipeline(Test02.pipeline);
						Test02.setBindGroup(computePassEncoder, 0, [
							{binding:0, resource:{buffer:indexData}},
							{binding:1, resource:{buffer}},
						]);
					}else{
						computePassEncoder.setPipeline(Test01.pipeline);
						Test01.setBindGroup(computePassEncoder, 0, [
							{binding:1, resource:{buffer}},
						]);
					}

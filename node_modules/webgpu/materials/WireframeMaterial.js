...
class WireframeMaterial extends TextureMaterial.Subclass(pipeline){
	static onDraw(drawUnitList, mesh, renderPassEncoder){
		...
		const indexBufferList = genWireframeBuffer(mesh);
		for(let i=0; i<mesh.subMeshList.length; ++i){
			...
			renderPassEncoder.setIndexBuffer(indexBufferList[i], 'uint32');
			renderPassEncoder.drawIndexed(subMesh.vertexCount * 2, drawUnitList.length);
		}
	}
}

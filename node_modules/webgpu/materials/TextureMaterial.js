...
const pipeline = new Pipeline({
	groups: [[
		{name:'vpMatrix', type:'MVP', visibility:GPUShaderStage.VERTEX, buffer:{}},
		{name:'g_sampler', visibility:GPUShaderStage.FRAGMENT, sampler:{}, default:{magFilter:'linear', minFilter:'linear', addressModeU:'repeat', addressModeV:'repeat', addressModeW:'repeat'}}
	], [
		{name:'worldMatrixList', type:`array<mat3x4<f32>, ${maxCountPerDrawStatic}>`, visibility:GPUShaderStage.VERTEX, buffer:{}, temp:{array: new Float32Array(12 * maxCountPerDrawStatic), stride: 12, mapFn: v => v.entity.worldTransform.rawData}},
		{name:'animationTexture', visibility:GPUShaderStage.VERTEX, texture:{viewDimension: '2d-array', sampleType:'unfilterable-float'}},
		{name:'animationTextureInfo', type:`array<vec4<f32>, ${maxCountPerDrawStatic}>`,visibility:GPUShaderStage.VERTEX, buffer:{}, temp:{array: new Float32Array(4 * maxCountPerDrawStatic), stride: 4, mapFn: v => [v.animationTime ?? 0, v.animationIndex ?? 0]}},
	], [
	...
	vertex: {
		...
		code: `
			...
			fn main(
				...
				output.position = positionWorldToScreen(worldPosition, vpMatrix);
				output.uv = uv;
				return output;
			}
...
const wireframePipeline = pipeline.clone({
	...
	fragment:{code:`@fragment fn main() -> @location(0) vec4<f32> {return vec4<f32>(1, 0, 0, 1);}`},
...
const pickPipeline = pipeline.clone({
	groups: v => [v[0].slice(0, 1), v[1]],
	buffers: v => [v[0],v[3],{
			name: 'meshId',
			arrayStride: 1 << 2,
			stepMode: 'instance',

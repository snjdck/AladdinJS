...
const pipeline = new Pipeline({
	groups: [[
		...
		{name:'boneStateTexture', storageTexture:{viewDimension:'2d-array', format:'rgba32float', access:'read-only'}}
	],[
	...
	vertex: {
		buffers: [
			{name: 'position', attributes: [{format:'float32x3'}]},
			{name: 'normal', attributes: [{format:'float32x3'}]},
			{name: 'uv', attributes: [{format:'float32x2'}]},
			{name: 'boneJoints', attributes: [{format:'uint16x4'}]},
			{name: 'boneWeights', attributes: [{format:'float32x4'}]},
			{name: 'info', stepMode: 'instance', attributes: [{format:'uint16x2'}], temp:{type:Uint16Array, mapFn: v => [v.entity.$id_world, v.entity.skeleton.$id_boneState]}}
		],
		code: `
			#include<skeleton>
			#include<transform>
			...
			@vertex Output {
				let boneMatrix = getBoneMatrix(boneStateTexture, boneJoints, boneWeights, info.y);
				...
				for(let [{subMeshList}, itemList] of Map.groupBy(list3, v => v.entity.mesh)){
					for(let drawUnitList of Material.Utils.cut(maxCountPerDrawStatic, itemList)){
						...
						for(let i=0, n=subMeshList.length; i<n; ++i){
							...
							if(isWireframe){
								const {position, boneJoints, boneWeights, indexWireframe} = subMesh.attributes;
								renderPassEncoder.setResources({position, boneJoints, boneWeights, index:indexWireframe});
							}else{
								const texture = this.onFetchTexture(subMesh.texture, drawUnitList[0].material, renderPassEncoder);
								if(!texture)continue;
								const {position, normal, uv, boneJoints, boneWeights, index} = subMesh.attributes;
								renderPassEncoder.setResources({position, normal, uv, boneJoints, boneWeights, index, 'baseTexture': texture.createView()});
							}
							...
							for(let [offset, count] of groupDraw(v => drawUnitList[v].material.subMeshFilter & subMeshFilter, drawUnitList.length)){
								if(isWireframe || subMesh.attributes.index){
	...
	static onDraw(list, renderPassEncoder){
			...
			for(let [{subMeshList}, itemList] of Map.groupBy(list, v => v.entity.mesh)){
				for(let drawUnitList of Material.Utils.cut(maxCountPerDrawStatic, itemList)){
					...
					for(let i=0, n=subMeshList.length; i<n; ++i){
						const subMesh = subMeshList[i];
						const {position, boneJoints, boneWeights, index} = subMesh.attributes;
						renderPassEncoder.setResources({position, boneJoints, boneWeights, index});
						const subMeshFilter = 1 << i;
						for(let [offset, count] of groupDraw(v => drawUnitList[v].material.subMeshFilter & subMeshFilter, drawUnitList.length)){

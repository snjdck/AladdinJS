...
const pipeline = {
	groups: [[
		...
		{binding:0, visibility:GPUShaderStage.VERTEX, buffer:{hasDynamicOffset:true}, name:'bone'}
	], [
...
const bindGroupCache = createWeakMapGetter((texture, material) => material.createBindGroupWith(texture.createView()));
...
class TextureMaterial extends Material.Subclass({pipeline}){
	...
	static onDrawSubMesh(instanceCount, subMesh, i){
		...
		if(usage == 0)return;
		Vulkan.renderPassEncoder.setBindGroup(3, bindGroupCache(texture, this));

	...
	vertex: {
		...
		code: `
			...
			fn mainDynamic(
				...
				let index: u32 = calcBoneIndex(boneIndex, instanceIndex);
				let boneMatrix: mat3x3<f32> = cast2mat(boneStateList[index]);
			...
			fn mainCubeDepth(
				...
				let index: u32 = calcBoneIndex(boneIndex, instanceIndex);
				let boneMatrix: mat3x3<f32> = cast2mat(boneStateList[index]);
				let modelPosition: vec3<f32> = position * boneMatrix + boneStateList[index+1].xyz;
			...
			fn mainDeferred(
				...
				worldPosition = vec4<f32>(worldPosition, 1) * worldMatrix;
				worldNormal = worldNormal * mat3x3(worldMatrix[0].xyz, worldMatrix[1].xyz, worldMatrix[2].xyz);
	...
	fragment: {
		...
		code: `
			...
			fn mainDeferred(
				...
				var output: OutputDeferred;
				output.color = textureSample(g_texture, g_sampler, uv);
				output.normal = normalize(normal).xy;
				//output.position = vec4(position, 0);
				return output;
			}
		`
	},
...
class TextureMaterial extends Material.Subclass(pipeline, [Material.Utils.groupMesh]){
	...
	static onDraw(drawUnitList, {subMeshList}, renderPassEncoder){
		...
		for(let i=0, n=subMeshList.length; i<n; ++i){
			...
			if(!texture)continue;
			this.setResources(renderPassEncoder, {'baseTexture': texture.createView()});

...
const wireframePipeline = pipeline.clone({
	...
	vertex:{code:`#include<vertexPosition>`,buffers:[
		{name: 'position', arrayStride: 4 << 2, attributes: [{format:'float32x3'}]},
		{name: 'boneIndex', arrayStride: 1 << 2, attributes: [{format:'uint32'}]}]},
...
const pickPipeline = pipeline.clone({
	...
	`, buffers: [
		{name: 'position', arrayStride: 4 << 2, attributes: [{format:'float32x3'}]},
		{name: 'boneIndex', arrayStride: 1 << 2, attributes: [{format:'uint32'}]},
		{name: 'meshId', arrayStride: 1 << 2, stepMode: 'instance', attributes: [{format:'uint32'}], temp:{array: new Uint32Array(maxCountPerDrawStatic), stride: 1, mapFn: v => [v.id]}}
	]},
	fragment: {

...
const pipeline = new Pipeline({
	groups: [
		...
		[worldMatrixList(), {name:'boneStateTexture', texture:{viewDimension: '2d-array', sampleType:'unfilterable-float'}}],
...
const wireframePipeline = new Pipeline({
	groups: [[{name:'vpMatrix', type:'MVP', buffer:{}}], [worldMatrixList(), {name:'boneStateTexture', texture:{viewDimension: '2d-array', sampleType:'unfilterable-float'}}]],
...
function fetchBoneStateTexture(renderPassEncoder, drawUnitList){
	const textureSize = {
		width:Math.max(...drawUnitList.map(v => v.entity.skeleton.boneCount)),
		height:drawUnitList.length,
	...
	for(let j=0; j<textureSize.height; ++j){
		const {boneState} = drawUnitList[j].entity.skeleton;
	...
	const boneStateTexture = renderPassEncoder.fetchTempTexture(textureSize, 'rgba32float,0x6', '>=');
	renderPassEncoder.writeTexture({texture:boneStateTexture}, data, {bytesPerRow:textureSize.width*16, rowsPerImage:textureSize.height}, textureSize);
				...
				for(let [{subMeshList}, itemList] of Map.groupBy(list3, v => v.entity.mesh)){
					for(let drawUnitList of Material.Utils.cut(maxCountPerDrawStatic, itemList)){
						renderPassEncoder.setResources({
							...
							'boneStateTexture': fetchBoneStateTexture(renderPassEncoder, drawUnitList)
						});
...
const pickPipeline = Pipeline.Pick(
	[[worldMatrixList(), {name:'boneStateTexture', texture:{viewDimension: '2d-array', sampleType:'unfilterable-float'}}]],
	[{name: 'position', arrayStride: 4 << 2, attributes: [{format:'float32x3'}]}, {name: 'boneIndex', attributes: [{format:'uint32'}]}],
	`
	#include<transform>
	#include<skeleton>
	...
	static onDraw(drawUnitList, renderPassEncoder){
		for(let [{subMeshList}, itemList] of Material.Utils.groupMesh(drawUnitList)){
			renderPassEncoder.setResources({
				...
				'boneStateTexture': fetchBoneStateTexture(renderPassEncoder, itemList),
				'meshId':itemList
			});
			for(let i=0, n=subMeshList.length; i<n; ++i){
				const subMesh = subMeshList[i];

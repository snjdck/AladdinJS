...
const wireframePipeline = pipeline.clone({
	groups: [[{name:'vpMatrix', type:'MVP', buffer:{}},{name:'g_sampler', visibility:GPUShaderStage.FRAGMENT, sampler:{}, default:{}}], [worldMatrixList(), animationTexture(), animationTextureInfo()]],
	vertex:{code:`#include<vertexPosition>`,buffers:[{name: 'position', arrayStride: 4 << 2, attributes: [{shaderLocation:0, offset: 0, format:'float32x3'}]}, {name: 'boneIndex', arrayStride: 1 << 2, attributes: [{shaderLocation:3, offset: 0, format:'uint32'}]}]},
	fragment:{code:`@fragment{return vec4(1, 0, 0, 1);}`},
...
const pickPipeline = pipeline.clone({
	...
	`, buffers: [
		{name: 'position', arrayStride: 4 << 2, attributes: [{shaderLocation:0, offset: 0, format:'float32x3'}]},
		{name: 'boneIndex', arrayStride: 1 << 2, attributes: [{shaderLocation:3, offset: 0, format:'uint32'}]},
		{name: 'meshId', arrayStride: 1 << 2, stepMode: 'instance', attributes: [{shaderLocation:1, offset: 0, format:'uint32'}], temp:{array: new Uint32Array(maxCountPerDrawStatic), stride: 1, mapFn: v => [v.id]}}
	]},
	fragment: {

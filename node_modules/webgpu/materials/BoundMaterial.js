	...
	vertex:{
		buffers: [{
			...
			attributes: [
				...
				{shaderLocation:1, offset: 3 << 2, format:'float32x3'}
			],
			valueArray: new Float32Array(6 * maxCountPerDraw),
			valueStride: 6,
		}],
		entryPoint: 'main',
		code: `
			...
			struct Input {
				...
				@location(0) row0: vec3<f32>,
				@location(1) row1: vec3<f32>,
			};
			...
			fn main(input: Input) -> @builtin(position) vec4<f32> {
				...
				var vertex: vec3<f32> = select(input.row0, input.row1, vec3<bool>(xyz));
				/*
...
const mapFn = ({material:{bound}}) => [bound.minX, bound.minY, bound.minZ, bound.maxX, bound.maxY, bound.maxZ];

class BoundMaterial extends Material.Subclass({pipeline}, [Material.Utils.cut(maxCountPerDraw)]){
	static onDrawBatched(drawUnitList){
		this.setVertexBuffer(0, drawUnitList.map(mapFn));
		Vulkan.renderPassEncoder.draw(24, drawUnitList.length);
	}

...
const pipeline = {
	groups: [[
		{binding:0, visibility:GPUShaderStage.FRAGMENT, sampler:{}, default:{magFilter:'linear', minFilter:'linear', addressModeU:'clamp-to-edge', addressModeV:'clamp-to-edge', addressModeW:'clamp-to-edge'}},
	...
	vertex:{
		buffers: [{
			...
			valueArray: new Float32Array(4 * maxCountPerDraw),
			valueStride: 4,
		}],
		entryPoint: 'main',
		code: `
	...
	fragment:{
		...
		code: `
			...
			@binding(0) @group(0) var g_sampler: sampler;
			@binding(1) @group(0) var g_texture: texture_2d<f32>;
			@binding(0) @group(1) var<uniform> g_color: Color;
...
const filterFn = ({frameRect:{width, height}, drawRect}) => [drawRect.x / width, drawRect.y / height, drawRect.width / width, drawRect.height / height]

class BlurFilter extends Material.Subclass({pipeline}, [Material.Utils.cut(maxCountPerDraw)]){
	constructor(color){
		super();
		this.color = color;
	...
	static onDrawBatched(drawUnitList){
		this.setBindGroup(1, [{binding:0, resource:{buffer:Float32Array.from(drawUnitList.flatMap(v => v.material.color)), size:1600}}]);
		this.setVertexBuffer(0, drawUnitList.map(filterFn));

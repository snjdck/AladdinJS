...
const pipeline = new Pipeline({
	groups: [[
		...
		{name:'g_sampler', binding:1, visibility:GPUShaderStage.FRAGMENT, sampler:{}, default:{magFilter:'linear', minFilter:'linear', addressModeU:'clamp-to-edge', addressModeV:'clamp-to-edge', addressModeW:'clamp-to-edge'}}
	], [
		{name:'colorList', type:'Color', binding:0, visibility:GPUShaderStage.FRAGMENT, buffer:{}, temp:{array:new Float32Array(4 * maxCountPerDraw), stride:4, mapFn: v => v.material.color}}
	]],
	vertex:{
		buffers: [{
			name: 'sourceRectList',
	...
	fragment:{
		...
		code: `
			...
			fn main(input: Input) -> Output {
				...
				var output: Output;
				output.color = sum * colorList.mixColor[input.instanceIndex];
				return output;
			}
		`
	},

...
const pipeline = new Pipeline({
	groups: [[
		...
		{name:'colorList', type:`array<vec4<f32>, ${maxCountPerDraw}>`, buffer:{}, temp:{stride:4, mapFn: v => v.material.color}}
	]],
	vertex:{
		buffers: [{
			name: 'sourceRectList',
			stepMode: 'instance',
			attributes: [
				{format: 'float32x4', name:'sourceRect'}
			],
			temp:{stride:4, mapFn: ({frameRect:{width, height}, drawRect}) => [drawRect.x / width, drawRect.y / height, drawRect.width / width, drawRect.height / height]}
		}],
		code: `
			struct Output {
	...
	fragment:{
		...
		code: `
			...
			@fragment{
				...
				return sum * colorList[instanceIndex];
			}
		`
	},
	primitive: {

...
const checkVertexBuffers = function(){
	...
	const calcType = function(){
		...
		return format => {
			...
			return n ? `vec${n}<${cast(t)}>` : cast(t);
		}
	}();
	return vertex => {
		if(!vertex)return;
		const info = new RegExp(`@vertex\\s+fn\\s+${vertex.entryPoint}\\s*\\(\\s*\\)`).exec(vertex.code);
		if(!info)return;
		const body = findBody(vertex.code, info.index);
		const result = [];
		if(/\binstanceIndex\b/.test(body))result.push('@builtin(instance_index)instanceIndex:u32');
		if(/\bvertexIndex\b/.test(body))result.push('@builtin(vertex_index)vertexIndex:u32');
		for(let buffer of vertex.buffers ?? []){
			for(let {name, format, shaderLocation} of buffer.attributes){
				result.push(`@location(${shaderLocation})${name ?? buffer.name}:${calcType(format)}`);
			}
		}
		vertex.code = vertex.code.replace(info[0], v => v.slice(0, -1) + result.join(',') + ')');
	}
}();
...
class Pipeline {
	constructor(desc){
		...
		checkGroups(desc);
		checkVertexBuffers(desc.vertex);

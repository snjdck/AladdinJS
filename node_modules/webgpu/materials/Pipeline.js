...
function createPipeline(desc){
	...
	if(vertex && !vertex.module)vertex.module = shaderCache(processInclude(vertex.code));
	if(fragment && !fragment.module)fragment.module = shaderCache(processInclude(fragment.code));
	if(compute && !compute.module)compute.module = shaderCache(processInclude(compute.code));
		...
		for(let entry of entries){
			...
			if(item.buffer){
				...
				if(Array.isArray(bufferData)){
					...
					const {type=Float32Array, stride, mapFn} = item.temp;
					bufferData = TypedArrayCache(type, stride * Number(pattern.exec(item.type)[1]));
					bufferDataSize = 0;
					//if(mapFn){
					...
					/*}else{
						for(let i=0, n=value.length; i<n; ++i){
							bufferData.set(value[i], bufferDataSize);
...
class Pipeline {
	...
	setVertexBuffer(renderPassEncoder, index, value){
		if(value instanceof GPUBuffer){
			...
			const {arrayStride, temp:{type=Float32Array, stride, mapFn}} = this.desc.vertex.buffers[index];
...
Pipeline.Blit = function(){
	...
	`;
	return function(groups, target, fragmentCode, depthStencil){

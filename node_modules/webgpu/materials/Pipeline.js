...
const pipelineCache = createGetter(desc => (desc.compute ? createComputePipeline : createRenderPipeline)(desc), calcPipelineSign);
function createRenderPipeline(desc){
	...
	if(!vertex.module){
		vertex.module = shaderCache(calcShaderSign(vertex.code));
	}
	if(!fragment.module){
		fragment.module = shaderCache(calcShaderSign(fragment.code));
	}
...
function createComputePipeline(desc){
	...
	if(!compute.module){
		compute.module = shaderCache(calcShaderSign(compute.code));
	}

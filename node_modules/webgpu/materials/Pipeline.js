...
const checkStageFnInputs = function(){
	...
	const calcTargetsType = function(){
		const pattern = /^([rgba]{1,4})\d+(\w+)/;
		function calcTargetType(format){
			const info = pattern.exec(format);
			return calcVecType(info[1].length, info[2]);
		}
		return targets => {
			return 'struct Output{' + targets.map((v, i) => `@location(${i})v${i}:` + calcTargetType(v.format)).join(',') + '}';
		}
	}();
	...
	return ({vertex, fragment, compute}) => {
		...
		if(fragment){
			...
			if(info){
				...
				fragment.code = calcTargetsType(fragment.targets) + replaceFnArgs(fragment.code, info[0], result);
			}
		}
	}
}();

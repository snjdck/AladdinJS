...
const checkStageFnInputs = function(){
	...
	return ({vertex, fragment, compute}) => {
		if(vertex){
			const info = new RegExp(calcFnPattern('vertex', vertex.entryPoint) + `\\)`).exec(vertex.code);
			if(info){
				...
				vertex.code = replaceFnArgs(vertex.code, info[0], result);
			}
		}
		if(fragment){
			//if(/@fragment\s*\{/.test(fragment.code)){
				...
				if(info){
					...
					if(fragment.targets.length > 1){
						fragment.code = calcTargetsOutput(fragment.targets) + fragment.code.replace(info[0], `$& fn ${fragment.entryPoint}(${result.join(',')})->Output`);
					}else{
						fragment.code = fragment.code.replace(info[0], `$& fn ${fragment.entryPoint}(${result.join(',')})->@location(0)${calcTargetType(fragment.targets[0])}`);
					}
				}
				return
			/*}
			const info = new RegExp(calcFnPattern('fragment', fragment.entryPoint) + `\\)`).exec(fragment.code);
			if(info){
				const result = calcBuiltin(findBody(fragment.code, info.index), argsFragment);
				const vertexOutput = new RegExp(calcFnPattern('vertex', vertex.entryPoint) + `.*?\\)\\s*->\\s*(\\w+|@builtin\\(position\\)\\s*vec4<f32>)\\s*\\{`).exec(vertex.code)[1];
				if(!vertexOutput.startsWith('@')){
				...
				fragment.code = calcTargetsOutput(fragment.targets) + replaceFnArgs(fragment.code, info[0], result);
			}
			*/
		}
	}

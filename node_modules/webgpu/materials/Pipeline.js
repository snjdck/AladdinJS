...
Pipeline.prototype.clone = function(){
	...
	const combine = (a, b) => a && b ? Object.assign({}, a, b) : a ?? b;
	return function(modify){
		const {groups=identity, buffers=identity, targets=identity} = modify;
		const target = this.desc;
		return new this.constructor({
			groups: groups(target.groups),
			compute: exclude(combine(target.compute, modify.compute), ['module']),
			vertex: exclude(combine(combine(target.vertex, modify.vertex), {buffers: buffers(target.vertex?.buffers)}), ['module']),
			fragment: exclude(combine(combine(target.fragment, modify.fragment), {targets: targets(target.fragment?.targets)}), ['module']),

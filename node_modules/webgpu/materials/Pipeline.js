...
function createPipeline(desc){
	...
	if(vertex && !vertex.module)vertex.module = shaderCache(processInclude(calcResourceDefs(desc.groups, GPUShaderStage.VERTEX) + vertex.code));
	if(fragment && !fragment.module)fragment.module = shaderCache(processInclude(calcResourceDefs(desc.groups, GPUShaderStage.FRAGMENT) + fragment.code));
	if(compute && !compute.module)compute.module = shaderCache(processInclude(calcResourceDefs(desc.groups, GPUShaderStage.COMPUTE) + compute.code));
...
function checkEntryPoint(v){
	if(v && !v.entryPoint)v.entryPoint = 'main';
}

function checkBinding(groups){
	for(let entries of groups){
		let binding = 0;
		for(let entry of entries){
...
class Pipeline {
	constructor(desc){
		...
		checkVisibility(desc);
		checkBinding(desc.groups);
		checkEntryPoint(desc.vertex);
		checkEntryPoint(desc.fragment);
		checkEntryPoint(desc.compute);

...
const checkStageFnInputs = function(){
	...
	const calcTargetsOutput = targets => 'struct Output{' + targets.map((v, i) => `@location(${i})v${i}:` + calcTargetType(v)).join(',') + '}';
	...
	return ({groups, vertex, fragment, compute}) => {
		if(vertex){
			const info = /@vertex\b\s*(\w*)\s*(?=\{)/.exec(vertex.code);
			if(info){
				...
				vertex.code = calcResourceDefs(groups, GPUShaderStage.VERTEX) + vertex.code.replace(info[0], `@vertex fn ${vertex.entryPoint}(${result.join(',')})->${returnType}`);
			}
		}
		if(fragment){
			...
			if(info){
				...
				if(vertexOutput){
					...
					result.push(...structBody.match(/@location\(\d+\)[^,]+/g));
				}
				let prefix = '';
				...
				fragment.code = calcResourceDefs(groups, GPUShaderStage.FRAGMENT) + prefix + fragment.code.replace(info[0], `@fragment fn ${fragment.entryPoint}(${result.join(',')})->${returnType}`);
			}
		}
	}
}();

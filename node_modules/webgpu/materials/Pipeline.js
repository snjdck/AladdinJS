...
const createBindGroup = function(){
	...
	return (renderPassEncoder, desc, entries) => {
		for(let entry of entries){
			...
			if(item.buffer){
				...
				if(Array.isArray(bufferData)){
					...
					bufferData = renderPassEncoder.fetchTempTypedArray(length, type);
					bufferDataSize = 0;
				...
				const buffer = renderPassEncoder.fetchTempBuffer(bufferSize, bufferUsage | GPUBufferUsage.COPY_DST);
				renderPassEncoder.writeBuffer(buffer, 0, bufferData, 0, bufferDataSize);
...
const checkStageFnInputs = function(){
	...
	const calcBufferType = function(){
		...
		return format => {
			if(format == 'unorm10-10-10-2')return 'vec4<f32>';
	...
	return buffers => {
		...
		for(let buffer of buffers){
			...
			for(let attribute of buffer.attributes){
				...
				if(attribute.format == 'unorm10-10-10-2'){
					offset += 4;
					continue;
				}
...
class Pipeline {
	...
	setVertexBuffer(renderPassEncoder, index, value){
		if(value instanceof GPUBuffer){
			...
			const array = renderPassEncoder.fetchTempTypedArray(stride * value.length, type);
			for(let i=0, n=value.length; i<n; ++i)array.set(mapFn(value[i]), i * stride);
			const size = arrayStride * value.length;
			const buffer = renderPassEncoder.fetchTempBuffer(size, GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST);
			...
			if(stepMode === undefined){
				renderPassEncoder.setVertexBuffer(index, renderPassEncoder.fetchBufferBy(value, GPUBufferUsage.VERTEX));
				return;
			}
			const buffer = renderPassEncoder.fetchTempBuffer(value.byteLength, GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST);//buffer may larger than request size
	...
	setResources(renderPassEncoder, resources){
		for(let [index, entries] of Map.groupBy(Object.entries(resources), this.infoToGroupIndex)){
			if(index === undefined){
				for(let [name, value] of entries){
					if(name === 'index' && value){
						...
						renderPassEncoder.setIndexBuffer(renderPassEncoder.fetchBufferBy(value, GPUBufferUsage.INDEX), format);
						continue;
					}
			...
			const bindGroup = createBindGroup(renderPassEncoder, this.desc.groups[index], entries.map(this.infoToEntry));
...
Pipeline.Blit = function(){
	const vertexCode = `
		const vertexList = array<vec4<f32>, 3>(
			vec4<f32>(-1, -1, 0,  1),
			vec4<f32>( 3, -1, 2,  1),
			vec4<f32>(-1,  3, 0, -1),
		);
		...
		@vertex Output {
			let vertex = vertexList[vertexIndex];
			return Output(vec4(vertex.xy, 0, 1), vertex.zwxy);
		}
	`;
...
Pipeline.Pick = function(){
	const typeDefine = `struct Output{
		@builtin(position) position: vec4<f32>,
		@location(0) worldPosition: vec3<f32>,
		@location(1) @interpolate(flat) id: u32
	}`;
	return (groups, buffers, vertexCode, depthCompare, topology) => new Pipeline({
		groups: [[{name:'vpMatrix', type:'MVP', buffer:{}}], ...groups],
		vertex: {code:typeDefine+vertexCode, buffers: [...buffers, {name: 'meshId', stepMode: 'instance', attributes: [{format:'uint32'}], temp:{type:Uint32Array, mapFn: v => [v.pickID ?? v.entity.pickID]}}]},
		fragment: {targets:[{format: 'rgba32float'}], code: `@fragment{return vec4(worldPosition,f32(id));}`},

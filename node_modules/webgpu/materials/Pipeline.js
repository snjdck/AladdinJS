	...
	return ({vertex, fragment, compute}) => {
		if(vertex){
			...
			if(info){
				...
				const suffix = info[2] == '->' ? '' : '->@builtin(position)vec4<f32>';
				vertex.code = vertex.code.replace(info[1], `$& fn ${vertex.entryPoint}(${result.join(',')})${suffix}`);
			}
		}
		if(fragment){
			...
			if(info){
				...
				const vertexOutput = /@vertex.*?->\s*(\w+|@builtin\(position\)\s*vec4<f32>)/.exec(vertex.code)[1];

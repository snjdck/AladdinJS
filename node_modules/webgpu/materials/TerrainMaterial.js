...
const pipeline = new Pipeline({
	groups: [
		...
		[{name:'baseTexture', texture:{viewDimension: '2d-array'}}, {name:'g_sampler', sampler:{}, default:{magFilter:'linear', minFilter:'linear', addressModeU:'repeat', addressModeV:'repeat', addressModeW:'repeat'}}]
	],
	vertex: {
		buffers: [{
			name: 'layer0',
			attributes: [
				{offset:0, format:'uint8x2', name:'map_layer'},
				{offset:4, format:'unorm8x2', name:'map_layermix_height'},
				{offset:8, format:'unorm8x4', name:'map_light'}
			]
		}],
		code: `
...
class TerrainMaterial extends Material.Subclass(pipeline, []){
	static onDraw(drawUnitList, renderPassEncoder){
		...
		for(let subMesh of entity.mesh.subMeshList){
			...
			renderPassEncoder.setResources({layer0, index});

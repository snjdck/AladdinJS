...
const {transform} = require('./@draw_unit_fn');
...
const pipeline = {
	...
	vertex: {
		...
		code: `
			...
			@binding(0) @group(1) var<uniform> worldMatrix: TRANSFORM_BLOCK;
	...
	fragment: {
		...
		code: `
			...
			@binding(1) @group(1) var g_texture: texture_2d_array<f32>;
...
class TerrainMaterial extends Material.Subclass({pipeline}){
	...
	onDraw(drawUnit){
		...
		materialType.setBindGroup(1, [
			{binding:0, resource:{buffer:transform(drawUnit)}},
			{binding:1, resource:this.texture.createView({dimension: '2d-array'})},
		]);
		const {subMeshList} = drawUnit.entity.mesh;
		for(let subMesh of subMeshList){
			subMesh.setBuffers(this.constructor.vertexBuffers);
			subMesh.draw(1);
		}
	}
}

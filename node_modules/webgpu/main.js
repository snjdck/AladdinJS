...
const MoveAction = ActionBuilder(
	(target, value) => {
		return [[target.x, value.x ?? target.x + (value.dx ?? 0)], [target.y, value.y ?? target.y + (value.dy ?? 0)]]
	},
...
const FaceToAction = ActionBuilder(
	(target, value) => {
		...
		if(Math.abs(destR - currR) > Math.PI * 0.3){
			target.animation = target.animationList[value.standAni]
		}
		return [[currR, destR]];
	},
...
const FaceAndMoveAction = value => Sequence([
	...
	CallFunc(target => target.animation = target.animationList[value.walkAni]),
	MoveAction({...value,speed:0.2}),
	CallFunc(target => target.animation = target.animationList[value.standAni]),
])
...
const createTextureArray = function(){
	...
	return async function(pathList, width, height){
		...
		imageBitmapList.forEach((imageBitmap, i) => device.queue.copyExternalImageToTexture(
			{source: imageBitmap},
...
function moveTargetTo(playerEntity, pt, walkAni, standAni, speed=200){
	/*
	let dx = pt.x - playerEntity.x;
	let dy = pt.y - playerEntity.y;
	let angle = Math.atan2(dy, dx) - Math.PI * 0.5;
	//console.log(dx, dy, angle / Math.PI * 180);
	playerEntity.rotation = playerEntity.rotation.fromEulerAngles(0, 0, angle);
	let duration = Math.hypot(dx, dy) / speed;
	playerEntity.animation = playerEntity.animationList[walkAni];
	return new Tween(playerEntity, duration * 1000, {x:pt.x, y:pt.y}).start().then(interruptFlag => {
		if(interruptFlag)return;
		playerEntity.animation = playerEntity.animationList[standAni];
	});
	*/
	playerEntity.moveAction?.cancel();
	//playerEntity.animation = playerEntity.animationList[standAni]
	playerEntity.moveAction = runAction(playerEntity, FaceAndMoveAction({...pt, walkAni, standAni}));
	//playerEntity.moveAction.catch(() => playerEntity.animation = playerEntity.animationList[standAni])
	//playerEntity.moveAction.catch(() => {})
	return playerEntity.moveAction;
}
...
async function init(){
	...
	for(let key of ['filter', '2d', '3d', '3d_color', 'terrain', 'bound', 'billboard']){
	...
	*/
	...
	Vulkan.swapChain = canvas.getContext('webgpu');
	Vulkan.swapChainFormat = navigator.gpu.getPreferredCanvasFormat();
	Vulkan.swapChain.configure({
		device,
	...
	console.log(Array.from(adapter.features), adapter.limits);
	...
	mesh1.moveTo(300, 500, 100);
	mesh2.moveTo(300, 600, 100);
	mesh3.moveTo(300, 700, 100);
	mesh4.moveTo(300, 400, 100);
...
*/
	...
	for(let [i,info] of Object.entries(Object.values(monsterDict))){
		let playerEntity = new MeshEntity(info.mesh).addMaterial(new TextureMaterial())
		view3d.scene3d.addChild(playerEntity);
		playerEntity.moveTo((i% 10) * 300/* + 7000*/, 2000 + Math.floor(i / 10) * 500);
	}
	...
	void async function(){
		...
		Vulkan.mouseMgr.mouseDownSignal.add(pt => {
			moveTargetTo(playerEntity, pt, 23, 1);
		})
		hero = playerEntity;
...
//*/	
		...
		//box.filter = new BlurFilter([1,0.2,0.6,1]);
		...
		void async function(){
			const menu01 = await Vulkan.loadTexture(device, 'assets/Interface/menu01_new.jpg');
			const menu02 = await Vulkan.loadTexture(device, 'assets/Interface/Menu02.jpg');
			const menu03 = await Vulkan.loadTexture(device, 'assets/Interface/Menu03_new.jpg');
			const Menu_Red = await Vulkan.loadTexture(device, 'assets/Interface/Menu_Red.jpg');
			const Menu_Blue = await Vulkan.loadTexture(device, 'assets/Interface/Menu_Blue.jpg');
			const Menu_Character = await Vulkan.loadTexture(device, 'assets/Interface/Menu_Character.jpg');
			const Menu_Inventory = await Vulkan.loadTexture(device, 'assets/Interface/Menu_Inventory.jpg');
			const Item_Back01 = await Vulkan.loadTexture(device, 'assets/Interface/Item_Back01.jpg');
			const Item_Back02 = await Vulkan.loadTexture(device, 'assets/Interface/Item_Back02.jpg');
			const Level_box = await Vulkan.loadTexture(device, 'assets/Interface/Level_box.jpg');
			const level_01 = await Vulkan.loadTexture(device, 'assets/Interface/level_01.jpg');

			const Item_Fairy = await Vulkan.loadTexture(device, 'assets/Interface/Item_Fairy.jpg');
			const Item_Cap = await Vulkan.loadTexture(device, 'assets/Interface/Item_Cap.jpg');
			const Item_Wing = await Vulkan.loadTexture(device, 'assets/Interface/Item_Wing.jpg');
			const Item_Weapon01 = await Vulkan.loadTexture(device, 'assets/Interface/Item_Weapon01.jpg');
			const Item_NeckLace = await Vulkan.loadTexture(device, 'assets/Interface/Item_NeckLace.jpg');
			const Item_Upper = await Vulkan.loadTexture(device, 'assets/Interface/Item_Upper.jpg');
			const Item_Weapon02 = await Vulkan.loadTexture(device, 'assets/Interface/Item_Weapon02.jpg');
			const Item_Gloves = await Vulkan.loadTexture(device, 'assets/Interface/Item_Gloves.jpg');
			const Item_Ring = await Vulkan.loadTexture(device, 'assets/Interface/Item_Ring.jpg');
			const Item_Lower = await Vulkan.loadTexture(device, 'assets/Interface/Item_Lower.jpg');
			const Item_Boots = await Vulkan.loadTexture(device, 'assets/Interface/Item_Boots.jpg');
			const Item_box = await Vulkan.loadTexture(device, 'assets/Interface/Item_box.jpg');
			const Item_Money = await Vulkan.loadTexture(device, 'assets/Interface/Item_Money.jpg');
			const exit_01 = await Vulkan.loadTexture(device, 'assets/Interface/exit_01.jpg');
			const exit_02 = await Vulkan.loadTexture(device, 'assets/Interface/exit_02.jpg');
			const repair_01 = await Vulkan.loadTexture(device, 'assets/Interface/repair_01.jpg');
			...
			let itemDataRaw = await Vulkan.loadData('assets/icons/item.bmd', 'arraybuffer');

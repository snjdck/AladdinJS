...
const View3D = require('./view3d/View3D');
...
async function init(){
	...
	*/
	...
	Vulkan.swapChainFormat = Vulkan.swapChain.getPreferredFormat(adapter);
...
//*/
	showWeaponModel(view3d.scene3d, playerMesh, 'knife_gdf', 'Staff', new Array(12).fill(0).map((v, i) => i + 1), 1, i =>  [2,3,4,5,8,9].includes(i) ? 6 : 4, 1000, function(i){
		...
		if(i == 8){
			this.addMaterial(new WingTextureMaterial(0x2), 0x2);
			this.addMaterial(new TextureMaterial(~0x2), ~0x2);
			return
		}
		if(i == 12){
			this.addMaterial(new WingTextureMaterial(0x1), 0x1);
			this.addMaterial(new TextureMaterial(~0x1), ~0x1);
			return
		}
		...
		if(singleSword.includes(i)){
			if(i == 6 || i == 14 || i == 15){
				this.bindObjectToBone('hand_bofdgne01', new MeshEntity(weaponMesh)
					.addMaterial(new WingTextureMaterial(0x2), 0x2)
					.addMaterial(new TextureMaterial(~0x2), ~0x2)
				);
			}else{

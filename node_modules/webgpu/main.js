...
void main()
{
	...
	for(int i=0; i<1; ++i){
		int index = 0;//(0 + 1 * gl_InstanceIndex) << 1;//every bone use two registers
...
const {View3D, Vulkan, Bitmap, BlurFilter, ColorMatrixFilter} = require('./lib');
...
async function init(){
	...
	const weaponData = parse(new Uint8Array(await loadData('./Spear10.bmd', 'arraybuffer')));
	...
	const testUniformBuffer = device.createBuffer({
		size: 2 * 4 * 4,
		usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
	});
	const testVertexBuffer = device.createBuffer({
		size: weaponData.subMeshList[0].vertexData.byteLength,
		usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST
	});
	testUniformBuffer.setSubData(0, new Float32Array(0,0,0,1,0,0,0,1));
	testVertexBuffer.setSubData(0, weaponData.subMeshList[0].vertexData);

	window.texture = texture;
	window.testUniformBuffer = testUniformBuffer
	window.testVertexBuffer = testVertexBuffer;
	window.testVertexCount = weaponData.subMeshList[0].vertexCount;
	...
	const view3d = new View3D();
		...
		a.width = imageBitmap.width;
		a.height = imageBitmap.height;

		let b = texture2.createView();
		b.width = imageBitmap2.width;
		b.height = imageBitmap2.height;

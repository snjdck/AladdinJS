...
const FaceToAction = ActionBuilder(
	(target, value) => {
		...
		if(Math.abs(destR - currR) > Math.PI * 0.3){
			...
			target.playAnimationByIndex(getAniIndex({...value.player, action:'stand'}))
		}
		return [[currR, destR]];
	},
...
const FaceAndMoveAction = value => Sequence([
	...
	CallFunc(target => target.playAnimationByIndex(getAniIndex({...value.player, action:'walk'}))),
	Spawn([Delay(1000), MoveAction({...value,speed:0.2})], 1),
	CallFunc(target => target.playAnimationByIndex(getAniIndex({...value.player, action:'run'}))),
	MoveAction({...value,speed:0.3}),
	CallFunc(target => target.playAnimationByIndex(getAniIndex({...value.player, action:'stand'}))),
])
...
function moveTargetTo(playerEntity, pt, walkAni, standAni, speed=200){
	...
	playerEntity.moveAction = runAction(playerEntity, FaceAndMoveAction({...pt, walkAni, standAni, player:{role:'sorcerer'}}));
...
//*
		...
		playerEntity.playAnimationByIndex(animationID.call(playerEntity, i, totalCount, weaponMesh, materialType));
		//Vulkan.animationMgr.register(playerEntity);

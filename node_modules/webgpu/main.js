...
const {Quaternion, RADIAN} = require('opengl/math');
...
async function init(){
	const {getImageJpegData} = WebGL;
	const {data:lightData} = await getImageJpegData('./World1/TerrainLight.jpg');
	const {data:heightData} = decodeBmp(await loadData('./World1/TerrainHeight.bmp', 'arraybuffer'));
	console.log(lightData, heightData);
	let mapData = await loadData('./World1/Terrain.map', 'arraybuffer');
	...
	mesh1.transform.appendRotation(rot).moveTo(0, -100, 0);
	mesh2.transform.appendRotation(rot).moveTo(0, 0, 0);
	mesh3.transform.appendRotation(rot).moveTo(0, 100, 0);
	monsterMesh.transform.appendRotation(rot).moveTo(-100, -100, 0);
	...
	playerEntity.transform.appendRotation(rot);

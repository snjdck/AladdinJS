...
const {createTextureArray} = require('webonly/utils_gpu');
...
const {EncoderEx} = require('./core/CommandEncoder')

const BlurFilter = require('./materials/BlurFilter');
const {keyboardEvents} = require('./core/Events');
...
const {mainLoop} = require('utils/gc');
...
async function init(){
	...
	supportDrag(function({files}){
		Array.from(files).map(v => {
			...
			reader.onload = () => {
				...
				//runAction(entity, RotateEntityZ);
				if(lastDragItem){
	...
	*/
	...
	encoder = new EncoderEx(device, canvas);
	...
	async function loadWingFiles(dict, type){
		...
		let fileList = await Promise.all(nameList.map(v => encoder.loadData(`./assets/${type}/${v}.bmd`)));
	...
	let fileList = await Promise.all(monsterNameList.map(v => encoder.loadData(`./assets/Monster/${v}.bmd`)));
	...
	const playerMesh = await encoder.loadData('./player.bmd');
	playerMesh.boundAABB = new AABB().setMinMax(-50, -50, 0, 50, 50, 200);
	playerMesh.animationList[146].copyFirstFrameToEnd();
	playerMesh.animationList[11].copyFirstFrameToEnd();
	playerMesh.animationList[12].copyFirstFrameToEnd();
	console.log(playerMesh)
	const weaponData = await encoder.loadData('./assets/Item/Spear10.bmd');
	const monster03Data = await encoder.loadData('./Monster03.bmd');
	...
	mainLoop(timeElapsed => {
		...
		view3d.render(encoder);
		//console.log('----')
		//console.log(Object.keys(Vulkan))
	});
	...
	let terrainTexture = await createTextureArray(device, await Promise.all(mapTileList.map(v => loadImage(`assets/World1/${v}.jpg`))));
	const meshTerrain = view3d.scene3d.root.addChild(new MeshEntity(await terrain()).addMaterial(new TerrainMaterial(terrainTexture)));
	//meshTerrain.z -= 2000;
	//meshTerrain.addMaterial(new WireframeMaterial())
	let mirrorMesh = (createPlane());
	let mirror = new MeshEntity(mirrorMesh);
	mirror.x = 500;
	mirror.y = 300;
	mirror.addMaterial(new MirrorMaterial());
	//view3d.scene3d.addChild(mirror);

	view3d.scene3d.addChild(new DirectionLight3D());
	let pointLight = new PointLight3D();
	pointLight.x = pointLight.y = 1000;
	pointLight.z = 500;
	view3d.scene3d.addChild(pointLight);
	...
	mesh2.addMaterial(new ColorMaterial('color://[0,255,0]'));
	mesh3.addMaterial(new ColorMaterial('color://[0,0,255]'));
	mesh4.addMaterial(new ColorMaterial('color://[255,255,0]'));
	//monsterMesh.addMaterial(new TextureMaterial())
	monsterMesh.addMaterial(new TextureMaterialDeferred())
	.addMaterial(new BoundMaterial(new AABB().setMinMax(0,0,0,100,100,200)))
	.addMaterial(Object.assign(new TextureMaterial(), {wireframe:true}));
	monsterMesh.bound = new AABB().setMinMax(0,0,0,100,100,200);
	monsterMesh.playAnimationByIndex(1)
...
*/
	...
	void async function(){
		const meshData = await encoder.loadData('./assets/Skill/Magic02.bmd');
		let entity = new MeshEntity(meshData).addMaterial(new GlowTextureMaterial());
		
		runAction(entity, function*(){
	...
	showSkill(view3d, playerMesh, 'Protect01', 1500, entity => runAction(entity, RotateEntityZ))
	showSkill(view3d, playerMesh, 'Protect02', 2000, entity => runAction(entity, RotateEntityZ))
	showSkill(view3d, playerMesh, 'Inferno01', 3000, entity => runAction(entity, RotateEntityZ))
	showSkill(view3d, playerMesh, 'WaveForce', 3000, entity => runAction(entity, RotateEntityZ))
	...
	showSkill(view3d, playerMesh, 'Blast01', 3500, entity => {
		entity.moveTo(0,100,0);
		runAction(null, function*(){
			let t = 0;
			for(;;){
				entity.moveTo(0,100,900 - t % 900);
				t = yield
			}
		})
	});
	...
	for(let [i,info] of Object.entries(Object.values(wingDict))){
		...
		playerEntity.info.hasWing = true;
		playerEntity.playAnimationWithAction('stand')
		let wing = new MeshEntity(info.mesh);
		wing.castShadow = true;
		wing.addMaterial(new info.material());
		//wing.addMaterial(new TextureMaterialDeferred());
		wing.playAnimationByIndex(0);
		playerEntity.bindObjectToBone("Bone05", wing);
...
*/	
	...
	void async function(){
		...
		meshTerrain.mouseDownSignal.add(pt => {
...
//*/
	showWeaponModel(view3d.scene3d, playerMesh, 'knife_gdf', 'Staff', new Array(12).fill(0).map((v, i) => i + 1), 1, i =>  [2,3,4,5,8,9].includes(i) ? 'staff' : 'axe', 1000, function(i){
		if(i == 7 || i == 9){
			return GlowTextureMaterial
		}
		if(i == 8){
			this.addMaterial(new GlowTextureMaterial(0x2));
			this.addMaterial(new TextureMaterial(~0x2));
			return
		}
		if(i == 6){//chuan shuo
			this.addMaterial(new TextureMaterialDeferred(0x2));
			this.addMaterial(new TextureMaterial(~0x2));
			return
		}
		if(i == 12){
			this.addMaterial(new GlowTextureMaterial(0x1));
		...
		if(singleSword.includes(i)){
			if(i == 6 || i == 14 || i == 15){
				this.bindObjectToBone('hand_bofdgne01', new MeshEntity(weaponMesh)
					.addMaterial(new GlowTextureMaterial(0x2))
	...
	const bow07 = await encoder.loadData('./assets/Item/Bow07.bmd');
	console.log(bow07);
	
	const Arrows02 = await encoder.loadData('./assets/Item/Arrows02.bmd');
	showWeaponModel(view3d.scene3d, playerMesh, 'hand_bofdgne01', 'Bow', [1,2,3,4,5,6,7,18,19,20,24], 3, function(_, i){
		...
		if(i == 7){
			//Vulkan.loadTexture('Shiny02.jpg').then(texture => {
				let material = new BillboardMaterial('Shiny02.jpg', 32, 64);
	...
	const Arrows01 = await encoder.loadData('./assets/Item/Arrows01.bmd');
...
//*/	
	...
	const texture = await encoder.loadImage('1.png')
	const texture2 = await encoder.loadImage('2.jpg')

		var bitmap = new Bitmap();
		bitmap.texture = new Texture2D(texture);
		bitmap.texture.scale9grid = [20,20,20,20];
		//bitmap.width = bitmap.height = 500;

		var bmp2 = new Bitmap();
		bmp2.x = 250;
		bmp2.texture = new Texture2D(texture2);
		...
		//box.filter = new BlurFilter([0.1,0.2,0.66,1]);
		...
		runAction(null, function*(){
			for(;;){
				...
				if(keyboardEvents.isKeyDown('KeyA')){//left
					cameraWorldPos.x -= moveStep;
				}else if(keyboardEvents.isKeyDown('KeyD')){//right
					cameraWorldPos.x += moveStep;
				}
				if(keyboardEvents.isKeyDown('KeyW')){//up
					cameraWorldPos.y -= moveStep;
				}else if(keyboardEvents.isKeyDown('KeyS')){//down
					cameraWorldPos.y += moveStep;
				}
				let scaleRatio = 1;
		...
		void async function(){
			const menu01 = await encoder.loadImage('assets/Interface/menu01_new.jpg');
			const menu02 = await encoder.loadImage('assets/Interface/Menu02.jpg');
			const menu03 = await encoder.loadImage('assets/Interface/Menu03_new.jpg');
			const Menu_Red = await encoder.loadImage('assets/Interface/Menu_Red.jpg');
			const Menu_Blue = await encoder.loadImage('assets/Interface/Menu_Blue.jpg');
			const Menu_Character = await encoder.loadImage('assets/Interface/Menu_Character.jpg');
			const Menu_Inventory = await encoder.loadImage('assets/Interface/Menu_Inventory.jpg');
			const Item_Back01 = await encoder.loadImage('assets/Interface/Item_Back01.jpg');
			const Item_Back02 = await encoder.loadImage('assets/Interface/Item_Back02.jpg');
			const Level_box = await encoder.loadImage('assets/Interface/Level_box.jpg');
			const level_01 = await encoder.loadImage('assets/Interface/level_01.jpg');

			const Item_Fairy = await encoder.loadImage('assets/Interface/Item_Fairy.jpg');
			const Item_Cap = await encoder.loadImage('assets/Interface/Item_Cap.jpg');
			const Item_Wing = await encoder.loadImage('assets/Interface/Item_Wing.jpg');
			const Item_Weapon01 = await encoder.loadImage('assets/Interface/Item_Weapon01.jpg');
			const Item_NeckLace = await encoder.loadImage('assets/Interface/Item_NeckLace.jpg');
			const Item_Upper = await encoder.loadImage('assets/Interface/Item_Upper.jpg');
			const Item_Weapon02 = await encoder.loadImage('assets/Interface/Item_Weapon02.jpg');
			const Item_Gloves = await encoder.loadImage('assets/Interface/Item_Gloves.jpg');
			const Item_Ring = await encoder.loadImage('assets/Interface/Item_Ring.jpg');
			const Item_Lower = await encoder.loadImage('assets/Interface/Item_Lower.jpg');
			const Item_Boots = await encoder.loadImage('assets/Interface/Item_Boots.jpg');
			const Item_box = await encoder.loadImage('assets/Interface/Item_box.jpg');
			const Item_Money = await encoder.loadImage('assets/Interface/Item_Money.jpg');
			const exit_01 = await encoder.loadImage('assets/Interface/exit_01.jpg');
			const exit_02 = await encoder.loadImage('assets/Interface/exit_02.jpg');
			const repair_01 = await encoder.loadImage('assets/Interface/repair_01.jpg');

			let wrapper = new DisplayObject2D();
			let bottomBox = new DisplayObject2D();
			bottomBox.y = 480 - menu01.height;
			bottomBox.addChild(new Bitmap(new Texture2D(menu01)))
			bottomBox.addChild(new Bitmap(new Texture2D(menu02))).x = 256;
			bottomBox.addChild(new Bitmap(new Texture2D(menu03))).x = 384;
			//let btnBox = new DisplayObject2D();
			bottomBox.addChild(new Bitmap(new Texture2D(Menu_Red))).x = 98;
			bottomBox.addChild(new Bitmap(new Texture2D(Menu_Blue))).x = 489;
			...
			panel1.addChild(new Bitmap(new Texture2D(Item_Back01)))
			panel1.addChild(new Bitmap(new Texture2D(Item_Back02))).y = Item_Back01.height;

			Object.assign(
				panel1.addChild(new Bitmap(new Texture2D(Level_box))),
				{x:13,y:60}
			)
			Object.assign(
				panel1.addChild(new Bitmap(new Texture2D(level_01))),
				{x:120,y:60}
			)
			...
			panel2.addChild(new Bitmap(new Texture2D(Item_Back01)))
			panel2.addChild(new Bitmap(new Texture2D(Item_Back02))).y = Item_Back01.height;
			...
			Object.assign(
				itemSlots.addChild(new Bitmap(new Texture2D(Item_Fairy))),
				{x:0,y:0}
			)
			Object.assign(
				itemSlots.addChild(new Bitmap(new Texture2D(Item_Cap))),
				{x:60,y:0}
			)
			Object.assign(
				itemSlots.addChild(new Bitmap(new Texture2D(Item_Wing))),
				{x:100,y:0}
			)
			Object.assign(
				itemSlots.addChild(new Bitmap(new Texture2D(Item_Weapon01))),
				{x:0,y:40+gap}
			)
			Object.assign(
				itemSlots.addChild(new Bitmap(new Texture2D(Item_NeckLace))),
				{x:40,y:40+gap}
			)
			Object.assign(
				itemSlots.addChild(new Bitmap(new Texture2D(Item_Upper))),
				{x:60,y:40+gap}
			)
			Object.assign(
				itemSlots.addChild(new Bitmap(new Texture2D(Item_Weapon02))),
				{x:120,y:40+gap}
			)
			Object.assign(
				itemSlots.addChild(new Bitmap(new Texture2D(Item_Gloves))),
				{x:0,y:100+gap*2}
			)
			Object.assign(
				itemSlots.addChild(new Bitmap(new Texture2D(Item_Ring))),
				{x:40,y:100+gap*2}
			)
			Object.assign(
				itemSlots.addChild(new Bitmap(new Texture2D(Item_Lower))),
				{x:60,y:100+gap*2}
			)
			Object.assign(
				itemSlots.addChild(new Bitmap(new Texture2D(Item_Ring))),
			...
			Object.assign(
				itemSlots.addChild(new Bitmap(new Texture2D(Item_Boots))),
				{x:120,y:100+gap*2}
			)
			...
			let boxTexture = new Texture2D(Item_box)
			for(let i=0; i<8; ++i){
				for(let j=0; j<8; ++j){
					Object.assign(
			...
			Object.assign(
				panel2.addChild(new Bitmap(new Texture2D(Item_Money))),
			...
			bitmap3d.mouseOverSignal.add(() => {
				const action = runAction(weaponEntity, RotateEntityY)
...
function createMuBtn(parent, texture, callback, key){
	...
	btn.type = 'button';
	let icon = btn.addChild(new Bitmap(new Texture2D(texture)));
...
function createMuBtn2(parent, textureUp, textureDown){
	let btn = new DisplayObject2D();
	let up = btn.addChild(new Bitmap(new Texture2D(textureUp)));
	let down = btn.addChild(new Bitmap(new Texture2D(textureDown)));
	down.visible = false;
...
const createPlayer = function(){
	...
	return async function(scene3d, playerMesh, id=0, filter=0xFF, autoRot=true){
		const playerEntity = new MeshEntity(playerMesh);//no mesh, just bone
		playerEntity.bound = playerMesh.boundAABB
		//playerEntity.addMaterial(new BoundMaterial(playerMesh.boundAABB));
		scene3d.addChild(playerEntity);
		if(autoRot){
			runAction(playerEntity, RotateEntityZ);
		}
		if(id > 0){
			const fileList = prefixList.filter((v, i) => filter & 1 << (4 - i)).map(v => v + id.toString().padStart(2, '0'));
			const meshList = await Promise.all(fileList.map(v => encoder.loadData("./assets/Player/Class/" + v + ".bmd")));
			if(id == 3){
				meshList.forEach(mesh => mesh.subMeshList[0].texture = 'skin_archer_01.jpg')
			}
			for(let mesh of meshList){
				let meshEntity = new MeshEntity(mesh, false);
				meshEntity.castShadow = true;
				//meshEntity.addMaterial(new TextureMaterialDeferred());
				meshEntity.addMaterial(new TextureMaterial());
				//meshEntity.addMaterial(new WireframeMaterial());
				meshEntity.shareSkeletonWith(playerEntity);
				playerEntity.addChild(meshEntity);
			}
		}
...
//*
		...
		const weaponMesh = await encoder.loadData(`./assets/Item/${prefix + i.toString().padStart(2, '0')}.bmd`);
		const weaponEntity = new MeshEntity(weaponMesh);
		weaponEntity.castShadow = true;
		const materialType = materialFn.call(weaponEntity, i);
		//weaponEntity.playAnimationByIndexList(weaponMesh.animationList.map(v => v.name));
		weaponEntity.playAnimationByIndex(0);
		if(materialType){
			weaponEntity.addMaterial(Reflect.construct(materialType, []))
			//weaponEntity.addMaterial(new TextureMaterialDeferred())
		}
...
async function showSkill(view3d, playerMesh, skillName, posX, skillFn){
	const meshData = await encoder.loadData(`./assets/Skill/${skillName}.bmd`);
	const entity = new MeshEntity(meshData).addMaterial(new GlowTextureMaterial());
...
const wingDict = {
	"Wing01":{material:GlowTextureMaterial}, "Wing02":{material:AlphaTextureMaterial}, "Wing03":{material:TextureMaterial}, 
	"Wing04":{material:GlowTextureMaterial}, "Wing05":{material:TextureMaterial}, "Wing06":{material:AlphaTextureMaterial},
	"Wing07":{material:TextureMaterial}, "Wing08":{material:GlowTextureMaterial}, "Wing09":{material:TextureMaterial},
	"Wing10":{material:TextureMaterial}, "Wing11":{material:TextureMaterial}, "Wing42":{material:TextureMaterial},
	"Wing43":{material:TextureMaterial}, "Wing44":{material:TextureMaterial},
	'DarkLordRobe':{material:TextureMaterial},
	'darklordrobe02':{material:TextureMaterial},
}
const helperDict = {
	'Angel':{},
	'Helper01':{},

...
const loadModel = async path => parse(new Uint8Array(await loadData(path, 'arraybuffer')));
const loadTexture = async (device, path) => {
...
async function init(){
	...
	const weaponData = await loadModel('./Spear10.bmd');
	
	const texture = await loadTexture(device, './1.png')
	const texture2 = await loadTexture(device, './2.jpg')

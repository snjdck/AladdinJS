...
class AABB
{
	...
	hitTestAABB(other){
		...
		if(this.minZ >= other.maxZ)return false;
		if(this.maxX <= other.minX)return false;
		if(this.maxY <= other.minY)return false;
		if(this.maxZ <= other.minZ)return false;
		return true;
	}
	...
	closestPoint(pt, result=pt){
		const {minX, minY, minZ, maxX, maxY, maxZ} = this;
		result.x = truncate(pt.x, minX, maxX);
		result.y = truncate(pt.y, minY, maxY);
		result.z = truncate(pt.z, minZ, maxZ);
		return result;
	}
}

const truncate = (value, min=0, max=1) => (value <= min) ? min : (value >= max ? max : value);

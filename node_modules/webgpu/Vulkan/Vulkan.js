...
const {
	createBindGroupLayout, createBindGroup, createShaderModule,
	createDepthTexture, setViewport, createBuffer, createBufferWith, calcBindGroupLayoutSign, calcPipelineLayoutSign
} = require('webonly/utils_gpu');
...
void function(){
	const shaderCache = createListGetter(code => createShaderModule(Vulkan.device, code), Object.create(null));
	...
	function createRenderPipeline(desc){
		let {layout, vertex, fragment, ...rest} = desc;
		if(desc.layoutDefine){
			layout = pipelineLayoutCache(desc.layoutDefine)
		}else{
			//layout = createPipelineLayout(desc.layout)
		}
		if(!vertex.module){

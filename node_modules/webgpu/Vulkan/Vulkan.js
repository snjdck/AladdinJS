...
Vulkan.defineProperty = function(target, name, fn){
...
Vulkan.getSampler = name => Vulkan._samplers[name];
Vulkan.registerSampler = function(name, descriptor){
	this.defineProperty(this._samplers, name, () => this.device.createSampler(descriptor));
}

Vulkan.bindComputePipeline = function(target, descriptor, name='pipeline'){
	this.defineProperty(target, name, () => this.createComputePipeline(descriptor));
}
...
Vulkan.createComputePipeline = function(descriptor){
	...
	return this.device.createComputePipeline({
		...
		computeStage: {module: this._shaderModules[compute.module], entryPoint: compute.entryPoint || 'main'}
	});
}

Vulkan.bindRenderPipeline = function(target, descriptor, name='pipeline'){
	this.defineProperty(target, name, () => this.createRenderPipeline(descriptor));
}
...
Vulkan.createRenderPipeline = function(descriptor){
	...
	return this.device.createRenderPipeline({
		layout: this.createPipelineLayout(layout),
		/*
		vertex: {module: this._shaderModules[vertex.module], entryPoint: vertex.entryPoint || 'main', buffers: vertex.buffers},
		fragment: {module: this._shaderModules[fragment.module], entryPoint: fragment.entryPoint || 'main', targets: fragment.targets},
		...rest
		//*/
		vertexStage: {module: this._shaderModules[vertex.module], entryPoint: vertex.entryPoint || 'main'},
		fragmentStage: {module: this._shaderModules[fragment.module], entryPoint: fragment.entryPoint || 'main'},
...
Vulkan.registerShaderModule = function(name, code){
	this.defineProperty(this._shaderModules, name, () => this.device.createShaderModule({code}));
}

Vulkan.createPipelineLayout = function(bindGroupLayouts){
	bindGroupLayouts = bindGroupLayouts.map(name => this._bindGroupLayouts[name]);
...
Vulkan.registerBindGroupLayout = function(name, entryList){
	...
	this.defineProperty(this._bindGroupLayouts, name, () => this.device.createBindGroupLayout({entries}));
}

Vulkan.createBindGroup = function(name, resourceList){
	const entries = resourceList.map((resource, binding) => ({binding, resource}));
	return this.device.createBindGroup({layout: this._bindGroupLayouts[name], entries});
}

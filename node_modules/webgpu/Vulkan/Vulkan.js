...
Vulkan.registerSampler = function(name, descriptor){
	Object.defineProperty(this, `sampler_${name}`, {
		configurable: false, enumerable: true,
		get: delayNew(() => this.device.createSampler(descriptor))
	});
}

Vulkan.registerComputePipeline = function(name, descriptor){
	const {layout, compute} = descriptor;
	Object.defineProperty(this, `pipeline_compute_${name}`, {
		configurable: false, enumerable: true,
		get: delayNew(() => this.device.createComputePipeline({
...
Vulkan.registerRenderPipeline = function(name, descriptor){
	...
	Object.defineProperty(this, `pipeline_${name}`, {
		configurable: false, enumerable: true,
		get: delayNew(() => this.device.createRenderPipeline({
			layout: this.createPipelineLayout(layout),
			/*
...
Vulkan.registerShaderModule = function(name, code){
	Object.defineProperty(this, `shader_${name}`, {
		configurable: false, enumerable: true,
		get: delayNew(() => this.device.createShaderModule({code}))
	});
}

Vulkan.createPipelineLayout = function(bindGroupLayouts){
	bindGroupLayouts = bindGroupLayouts.map(name => this[`layout_${name}`]);
	return Vulkan.device.createPipelineLayout({bindGroupLayouts});
}
...
Vulkan.registerBindGroupLayout = function(name, entryList){
	...
	Object.defineProperty(this, `layout_${name}`, {
		configurable: false, enumerable: true,
		get: delayNew(() => this.device.createBindGroupLayout({entries}))
	});
}

Vulkan.createBindGroup = function(layout, resourceList){
	const entries = resourceList.map((resource, binding) => ({binding, resource}));
	return Vulkan.device.createBindGroup({layout, entries});
}

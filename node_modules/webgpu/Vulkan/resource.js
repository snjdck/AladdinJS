...
const fetchTextures = TexturePool((width, height) => {
	...
	const emissiveTexture = device.createTexture({format: 'bgra8unorm', size, usage/*, mipLevelCount:2*/});
...
const fetchBloomTexture = TexturePool((width, height) => {
	...
	const texture0 = device.createTexture({format: 'bgra8unorm', size, usage, mipLevelCount});
	const texture1 = device.createTexture({format: 'bgra8unorm', size, usage, mipLevelCount});
	const texture2 = device.createTexture({format: 'bgra8unorm', size:[width, height], usage});
...
const fetchPickTexture = TexturePool((width, height) => {
	const size = [width, height, 1];
	const texture = Vulkan.device.createTexture({
		size,
		format: 'r32uint',

...
const fetchBuffer = function(){
	...
	return (size, usage) => {
		if(!map.has(usage)){
			map.set(usage, BufferPool(size => getDevice().createBuffer({usage, size})));
		}
		return map.get(usage)(size);
	}
}();

const bufferCache = LRUCache((data, usage) => createBufferWith(getDevice(), usage, data));

const colorTextureCache = LRUCache(color => {
	const texture = createTexture(getDevice(), 1, 1, 1, GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_DST);//bgra
	getDevice().queue.writeTexture(
...
const fetchBloomTexture = TexturePool(({width, height}) => {
	const device = getDevice()
...
const fetchCubeDepth = TexturePool(({width, height}) => {
	...
	const depthTexture = getDevice().createTexture({
		size: [width, height, 6],
		format: 'depth32float',
		usage
	});
...
const fetchAnimationTexture = LRUCache(mesh => {
	const device = getDevice()
...
const fetchPickTexture = TexturePool(({width, height}) => {
	const size = [width, height, 1];
...
const fetchView3D = TexturePool(({width, height}) => {
	console.log('new view3d',width, height);
	let colorTexture = createTexture(getDevice(), width, height, 1, GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.RENDER_ATTACHMENT);
	let depthTexture = createDepthTexture(getDevice(), width, height);
	return {
		width, height,
		colorView:colorTexture.createView(),
...
const fetchView2D = TexturePool(({width, height}) => {
	...
	let texture = createTexture(getDevice(), width, height, 2, GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.RENDER_ATTACHMENT | GPUTextureUsage.COPY_SRC | GPUTextureUsage.COPY_DST);
	return {
		width, height,

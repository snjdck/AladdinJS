...
`;
	const pipeline1 = new Pipeline({
		...
		fragment: {entryPoint: 'mainFragment', code:code1, targets:[{format: 'rgba16float'}]},
	...
	const pipeline2 = new Pipeline({
		...
		fragment: {entryPoint: 'mainFragment', code:code2, targets:[{format: 'rgba16float'}]},
		primitive: {topology: 'triangle-strip'}
	});
	const pipeline3 = new Pipeline({
		...
		vertex: {entryPoint: 'mainVertex', code:code2},
		fragment:{entryPoint: 'mainFragment', code:code + `
		@fragment
		fn mainFragment(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
			return textureSample(g_texture, g_sampler, uv) * 5;
		}
	...
	return (device, texture) => {
		...
		for(let commandEncoder of CommandEncoder(device)){
			...
			for(let renderPassEncoder of beginRenderPass('[0,0,0,0]+store', commandEncoder, createView(texture))){
				...
				pipeline3.setResources(renderPassEncoder, {texture: createView(texture1)});
				renderPassEncoder.draw(4);
			}

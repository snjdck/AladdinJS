...
const drawBloom = function(){
	...
	return (commandEncoder, texture) => {
		const {texture0, texture1} = fetchBloomTexture({width:texture.width, height:texture.height}, commandEncoder.device);
		const n = texture0.mipLevelCount;
			...
			for(let i=0; i<n; ++i){
				for(let renderPassEncoder of commandEncoder.beginRenderPass('[0,0,0,0]+store', createView(texture0, i))){
					renderPassEncoder.setPipeline(blit1);
					renderPassEncoder.setResources({texture: i==0 ? createView(texture, 0) : createView(texture0, i-1)});
			...
			for(let i=n-2; i>=0; --i){
				for(let renderPassEncoder of commandEncoder.beginRenderPass('[0,0,0,0]+store', createView(texture1, i))){
					renderPassEncoder.setPipeline(blit2);
			...
			for(let renderPassEncoder of commandEncoder.beginRenderPass('[0,0,0,0]+store', createView(texture))){
				renderPassEncoder.setPipeline(blit3);
				renderPassEncoder.setResources({texture: createView(texture1)});
				renderPassEncoder.drawQuad();
			}

...
function createSlot(maxCountPerSlot, bytePerInstance, valueFn, activeFn, cacheFn){
	...
	return {
		...
		bind: (materialClass, bindIndex) => ({add, active: (batchIndex, bufferOffset) => activeFn.call(materialClass, bindIndex, cacheFn(batchIndex), bufferOffset * bytePerInstance)})
	};
}
...
const BufferSlot = (usage, sizeFn, activeFn) => (maxCountPerSlot, bytePerInstance, valueFn) => createSlot(maxCountPerSlot, bytePerInstance, valueFn, activeFn, createCache(usage, sizeFn(maxCountPerSlot, bytePerInstance)));

const VertexBufferSlot = BufferSlot(GPUBufferUsage.VERTEX, (a, b) => a * b, function(bindIndex, buffer, bufferOffset){
	Vulkan.renderPassEncoder.setVertexBuffer(bindIndex, buffer, bufferOffset);
});

const UniformBufferSlot = BufferSlot(GPUBufferUsage.UNIFORM, (a, b) => a * b << 1, function(bindIndex, buffer, bufferOffset){
	Vulkan.renderPassEncoder.setBindGroup(bindIndex, this.createBindGroup(bindIndex, [{binding:0,resource:{buffer,offset:0,size:0x4000}}]), [bufferOffset]);
});

...
const UniformBufferSlot = function(){
	function createCache(visibility){
		const layout = [{binding:0, visibility, buffer:{hasDynamicOffset: true}}];
		...
		const getBindGroup = createListGetter(i => Vulkan.createBindGroupNew(layout, [{buffer:getBuffer(i), size: 0x4000}]));
		return function(batchIndex, offset, value){
			if(value === undefined)return getBindGroup(batchIndex);
			Vulkan.device.queue.writeBuffer(getBuffer(batchIndex), offset, value);
		}
	}
	const activeFn = (bindIndex, value, bufferOffset) => Vulkan.renderPassEncoder.setBindGroup(bindIndex, value, [bufferOffset]);
	return (visibility, bytePerInstance, valueFn) => createSlot(Math.floor(0x4000 / bytePerInstance), bytePerInstance, valueFn, activeFn, createCache(visibility));
}();

...
function createSlot(maxCountPerSlot, bytePerInstance, valueFn, activeFn, cacheFn){
	...
	return {
		...
		add: createAddFn(maxCountPerSlot, bytePerInstance, cacheFn, valueFn, slotUsage),
		active: (bindIndex, batchIndex, bufferOffset) => activeFn(bindIndex, cacheFn(batchIndex), bufferOffset * bytePerInstance),
	};
}
...
const VertexBufferSlot = function(){
	...
	const activeFn = (bindIndex, value, bufferOffset) => Vulkan.renderPassEncoder.setVertexBuffer(bindIndex, value, bufferOffset);
...
const UniformBufferSlot = function(){
	function createCache(layout){
		const getGroup = createListGetter(() => new BufferBindGroup(
			[[GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST, 0x8000]],
			buffer => [[layout, {buffer, size: 0x4000}]],
		));
		return function(batchIndex, offset, value){
			if(value === undefined)return getGroup(batchIndex).getBindGroup();
			Vulkan.device.queue.writeBuffer(getGroup(batchIndex).getBuffer(), offset, value);
		}
	}
	const activeFn = (bindIndex, value, bufferOffset) => Vulkan.renderPassEncoder.setBindGroup(bindIndex, value, [bufferOffset]);
	return (bindGroupLayout, bytePerInstance, valueFn) => createSlot(Math.floor(0x4000 / bytePerInstance), bytePerInstance, valueFn, activeFn, createCache(bindGroupLayout));
}();

...
function createSlot(maxCountPerSlot, cacheFn, activeFn){
	const usage = [];
	return {
		reset: () => usage.length = 0,
		add: createAddFn(maxCountPerSlot, cacheFn, usage),
		active: (bindIndex, batchIndex, bufferOffset) => activeFn(bindIndex, cacheFn(batchIndex), bufferOffset),
	};
}
...
const VertexBufferSlot = function(){
	...
	return (maxCountPerSlot, bytePerInstance, valueFn) => {
		return createSlot(maxCountPerSlot, createVertexBufferListCache(maxCountPerSlot, bytePerInstance, valueFn), (bindIndex, value, bufferOffset) => {
			Vulkan.renderPassEncoder.setVertexBuffer(bindIndex, value, bufferOffset * bytePerInstance);
		});
	}
}();
...
const UniformBufferSlot = function(){
	...
	return (bindGroupLayout, bytePerInstance, valueFn) => {
		return createSlot(Math.floor(0x4000 / bytePerInstance), createUniformBufferListCache(bindGroupLayout, bytePerInstance, valueFn), (bindIndex, value, bufferOffset) => {
			Vulkan.renderPassEncoder.setBindGroup(bindIndex, value, [bufferOffset * bytePerInstance]);
		});
	}
}();

...
const blit = function(){
	const vertexCode = `
		...
		fn main(@builtin(vertex_index) vertex_index: u32) -> Output {
			...
			return Output(vec4<f32>(position, 0, 1), vec4(vertex.x, 1-vertex.y, position));
		}
	`;
	return function(groups, target, fragmentCode){
		const pipeline = new Pipeline({
			groups,
			vertex: {entryPoint: 'main', code:vertexCode},
			fragment: {entryPoint: 'main', code:fragmentCode, targets:[target]},
...
const createDepthDrawFn = (viewDimension, code) => blit([[
		...
		{name:'cameraMatrixList', type:'Info', binding:4, visibility:GPUShaderStage.FRAGMENT, buffer:{}},
		{name:'sampler_depth', binding:7, visibility:GPUShaderStage.FRAGMENT, sampler:{type:'comparison'}, default:{compare:'less-equal'}},//greater, less-equal
	]], {format: 'bgra8unorm', blend: {

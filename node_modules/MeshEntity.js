...
class MeshEntity extends DisplayObject3D
{
	...
	collectDrawUnits(collector, gl, mode){
		...
		if(mode == "pick"){
			...
			this.collectBoneMatrixList();
			let uid = gl.mouseMgr.pickRegister(this);
			collector.addDrawUnit({vao:this.vao, material:this.hitTestMaterial, uid, boneMatrixList:this.boneMatrixList});
			return;
		}
		this.collectBoneMatrixList();
		for(let material of this.materials){
			collector.addDrawUnit({vao:this.vao, material, boneMatrixList:this.boneMatrixList});
		}
	}

	collectBoneMatrixList(){
		let {boneMatrixList, boneDict, subMesh:{boneData}} = this;
		for(let i=boneData.length-1; i>=0; --i){
			let boneID = boneData[i];
			boneDict[boneID].worldTransform.copyToArray(boneMatrixList, i << 3);
		}
	}
}

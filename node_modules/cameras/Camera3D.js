...
class Camera3D extends DisplayObject3D
{
	...
	render(root, gl, mode){
		...
		gl.uniformMatrix4fv(gl.getUniformLocation(program, "screenMatrix1"), false, this.lens);
		this.worldTransform.copyToArray(matrix);
		gl.activeTexture(gl.TEXTURE1);
		for(let light of lightList){
			if(!light.hasShadows)continue;
			light.camera.worldTransformInvert.copyToArray(matrix, 8);
			gl.uniform4fv(gl.getUniformLocation(program, "cameraMatrix"), matrix);

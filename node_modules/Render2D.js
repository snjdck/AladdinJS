...
class Render2D
{
	constructor(gl){
		...
		this.uniformInstanceIDBase = new Uint32Array(this.uniformData.buffer, 8, 1);
	...
	drawEnd(){
		...
		const {gl, uniformData, vertexArray, uniformBuffer, targetList} = this;
		gl.bindTexture(gl.TEXTURE_2D, this.activeTexture);
		let instanceIndex = 0;
		let drawCount = Math.ceil(targetCount / MAX_INSTANCE_COUNT);
		while(drawCount--){
			if(isPickMode){
				this.uniformInstanceIDBase[0] = gl.mouseMgr.nextPickID;
			}
			...
			for(let i=0; i<instanceCount; ++i){
				...
				worldTransform.copyToArray(uniformData, offset);
				uniformData.set(texture.xyuvMul, offset+8);
				uniformData.set(texture.xyuvAdd, offset+12);
				uniformData.set(texture.scale9grid, offset+20);

				uniformData[offset+16] = width;
				uniformData[offset+17] = texture.width;
				uniformData[offset+18] = height;
				uniformData[offset+19] = texture.height;

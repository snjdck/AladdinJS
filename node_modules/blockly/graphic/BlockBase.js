...
class BlockBase
{
	...
	onMouseDown(e){
		...
		function onMouseMove(e){
			let x = Math.min(Math.max(e.x - dx, 0), svg.viewportElement.getAttribute("width")  - block.getTotalBlockWidth());
			let y = Math.min(Math.max(e.y - dy, 0), svg.viewportElement.getAttribute("height") - block.getTotalBlockHeight());
			block.setXY(x, y);
			block.relayout();

		...
		switch(insertPt.type){
			...
			case INSERT_PT_WRAP:
				...
				if(!dragTarget.hasSubBlock2()){
					...
					let childHeightList = [0, 1].map(i => dragTarget.getSubHeight(i));
					childHeightList[insertPt.index] = block.getTotalBlockHeight();
					path = drawer.drawStatement(dragTarget.getBlockWidth(), dragTarget.getPositionSub(0), true, true, ...childHeightList);
				}
				break;
		}
		if(path){

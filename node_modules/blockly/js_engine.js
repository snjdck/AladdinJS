...
const castBlock = function(){
	...
	return function(block){
		...
		if(opcode === 'controls_flow_statements'){
			return block.field['#text'].toLowerCase();
		}
		...
		if(opcode == 'control_wait'){
			return `for(let end=performance.now()+1000*(${
				castArg(block.value)
			});performance.now()<end;)yield yieldOneTickFlag;`;
		}
		if(opcode == 'control_forever'){
			const {statement} = block;
			if(!statement)return 'yield suspendFlag';
			const code = castBlockList(statement.block);
			return `for(;;)` + insertYieldFlag(code);
		}
		if(opcode == 'control_if'){
			const {value, statement} = block;
			const code = castBlockList(statement && statement.block);
			return `if(${castArg(value)}){\n${code}}`;
		}
		if(opcode == 'control_if_else'){
			const statement = castToDict(block.statement, v => castBlockList(v.block));
			return `if(${castArg(block.value)}){\n${statement.SUBSTACK || ''}}else{\n${statement.SUBSTACK2 || ''}}`;
		}
		if(opcode == 'control_wait_until'){
			return `while(!(${castArg(block.value)}))` + insertYieldFlag();
		}
		if(opcode == 'control_repeat_until'){
			const {value, statement} = block;
			const code = castBlockList(statement && statement.block);
			return `while(!(${castArg(value)}))` + insertYieldFlag(code);
		}
		...
		if(opcode === 'controls_repeat_ext'){
			const {value, statement} = block;
			const code = castBlockList(statement && statement.block);
			return `for(let i=${castArg(value)};i>0;--i)` + insertYieldFlag(code);
		}
		if(opcode === 'controls_for'){
			const value = castToDict(block.value, castArg);
			let code = variables_set(block.field['#text'], 'i');
			if(block.statement)code += castBlockList(block.statement.block);
			return `for(let i=${value.FROM}, n=${value.TO}, s=${value.BY};i<=n;i+=s)` + insertYieldFlag(code);
		}
		if(opcode === 'controls_forEach'){
			const list = block.value ? castArg(block.value) : '[]';
			let code = variables_set(block.field['#text'], 'i');
			if(block.statement)code += castBlockList(block.statement.block);
			return `for(let i of ${list})` + insertYieldFlag(code);
		}
		if(opcode === 'controls_whileUntil'){
			const {value, statement} = block;
			const condition = block.field['#text'] === 'UNTIL' ? `!(${castArg(value)})` : castArg(value);
			let code = castBlockList(statement && statement.block);
			return `while(${condition})` + insertYieldFlag(code);
		}
		if(opcode === 'procedures_ifreturn'){
			const args = castToDict(block.value, castArg);
			return `if(${args.CONDITION || false})return${args.VALUE ? ' ' + args.VALUE : ''};`;
		}
		...
		if(funcCallBlockNames.includes(opcode)){
			return `yield* fn${generateKey(block.field['#text'])}(${generateObject(castToArray(block.value).map(v => [v.name, castArg(v)]))})`;
		}
		...
		return prefix + `$${generateKey(opcode)}(${generateObject(castArgList(block))})`;
	}
}();

function castBlockList(block){
	let result = [];
	while(block){
		result.push(castBlock(block));
		block = block.next?.block;
	}
	return tab(result.map(v => v + '\n').join(''));
}
...
function genFunction(blocks, startBlockName){
	...
	code += '}\nreturn {';
	code += startBlocks.map(v => generateFuncDef(v.id, castBlockList(v.next?.block)));
	code += '}\n}';
	console.log(code);
...
function exec(blocks, startBlockName, context){
	...
	return Promise.all(startBlocks.map(v => factory[v.id]()).map(js_vm.exec));
}

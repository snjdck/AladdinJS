...
const castBlock = function(){
	...
	return function(block, offset=0){
		...
		if(funcDefBlockNames.includes(opcode)){
			...
			const result = mutation ? castToArray(mutation.arg).map(v => variables_set(v.name, `args[${quote(v.name)}]`)) : [];
		...
		if(opcode === 'procedures_callnoreturn' || opcode === 'procedures_callreturn'){
			...
			return name + '({' + value.map(v => quote(args[v.name.slice(3)].name) + ':' + castArg(v)) + '})';
		}
...
function genFunction(blocks, startBlockName){
	...
	let code = 'return $ => {\nlet fn = {';
	code += fnDefBlocks.map(v => `*[${quote(v.field['#text'])}](args){\n${castBlock(v)}}`);
	code += '}\nreturn {';
	code += startBlocks.map(v => `*[${quote(v.id)}](){\n${castBlockList(v.next && v.next.block)}}`);
	code += '}\n}';
	console.log(code);

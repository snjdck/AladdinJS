...
const castBlockList = function(){
	...
	const castToDict = v => castToArray(v).reduce((dict, v) => (dict[v.name] = v.block, dict), Object.create(null));
	...
	function castBlock(block, offset=0){
		...
		if(opcode === 'logic_ternary'){
			...
			return `((${IF ? castBlock(IF) : 0}) ? (${THEN ? castBlock(THEN) : 0}) : (${ELSE ? castBlock(ELSE) : 0}))`;
		}
		...
		if(opcode === 'controls_if'){//Blockly
			...
			let result = `${tab}if(${value.IF0 ? castBlock(value.IF0) : false}){\n${castBlockList(statement.DO0, offset+1)}${tab}}`;
			for(let i=1; i<=elseifCount; ++i){
				result += `else if(${value['IF'+i] ? castBlock(value['IF'+i]) : false}){\n${castBlockList(statement['DO'+i], offset+1)}${tab}}`;
			}
			if(elseCount > 0){
				result += `else{\n${castBlockList(statement.ELSE, offset+1)}${tab}}`;
			}
			return result;
		}

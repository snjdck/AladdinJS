	...
	function castBlock(block, offset=0){
		...
		if(opcode === 'procedures_ifreturn'){
			...
			if(Array.isArray(block.value)){
				...
				return tab + `if(${castArg(condition)[1]})return ${castArg(value)[1]};`;
			}
			if(block.value.name === 'CONDITION'){
				return tab + `if(${castArg(block.value)[1]})return;`;
			}
			if(block.value.name === 'VALUE'){
				return tab + `if(false)return ${castArg(block.value)[1]};`;
			}
		}
		if(funcDefBlockNames.includes(opcode)){
			...
			if(block.value){
				result += `\treturn ${castArg(block.value)[1]};\n`;
			}
			return result + `}`;
		}

...
{
	runPass(codeList, OpCode.JUMP_IF, optimizeConstCondition);
...
function optimizeConstCondition(codeList, index){
	const prevIndex = index - 1;
	const [op, value] = codeList[prevIndex];
	if(op !== OpCode.PUSH)return;
	const count = codeList[index][1];
	codeList[prevIndex] = ['jump', 1];
	codeList[index] = ['jump', value ? count : 1];
}

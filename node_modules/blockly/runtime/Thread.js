...
class Signal
{
	...
	notify(...args){
		for(const [handler, once] of this.handlerMap){
			handler(...args);
			if(once)this.delete(handler);
		}
	}
}
...
class Thread
{
	...
	execNextCode(){
		...
		if(ip < codeList.length){
			execute(codeList[ip], codeList[ip+1]);
		}else{
			this._finishFlag = true;
		}
	}
	...
	updateSuspendState(){
		if(this.suspendUpdater)this.suspendUpdater();
	}
...
const execute = require('./InstructionExector');

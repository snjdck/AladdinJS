...
function genExpressionCode(block)
{
	switch(block["type"]){
		...
		case "array":
			return [
				OpFactory.GetVar("newArray"),
				OpFactory.Invoke(0, 1),
				...block.value.map((v, i) => [
					OpFactory.Push(i),
					...genExpressionCode(v),
					OpFactory.GetVar("setKey"),
					OpFactory.Invoke(3, 1)
				]).reduce((a, b) => a.concat(b), [])
			];
		case "object":
			return [
				OpFactory.GetVar("newObject"),
				OpFactory.Invoke(0, 1),
				...block.value.map(([k, v]) => [
					...genExpressionCode(k),
					...genExpressionCode(v),
...
function genLoopTimesCode({count, code, key})
{
	...
	if(key){
		result = genForCodeImpl([
			...
			OpFactory.GetVar(key),
			OpFactory.GetVar("increase"),
			OpFactory.Invoke(1, 1),
			OpFactory.SetVar(key)
		], code);
	}else{
		result = genForCodeImpl([], [
			OpFactory.GetVar("decrease"),
			OpFactory.Invoke(1, 1),
			[OpCode.DUPLICATE],
			OpFactory.GetVar("isNotNegative"),
			OpFactory.Invoke(1, 1)
		], [], code);
	}
	return [...genExpressionCode(count), ...result, OpFactory.Pop()];
}

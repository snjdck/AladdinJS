...
{
	static Create(pa, pb){
		return new this(Math.abs(pa.x - pb.x), Math.abs(pa.y - pb.y), Math.min(pa.x, pb.x), Math.min(pa.y, pb.y));
	}
	...
	containsPt(pt){
		return this.x <= pt.x && pt.x < this.right
			&& this.y <= pt.y && pt.y < this.bottom;
	}

...
const defineSubMainProp = function(){
	const subPropFn = (setFn, value) => [() => value, function(newValue){
		if(value === newValue)return;
		value = newValue;
		setFn();
	}];
	return (mainProp, mainPropFn, subPropMap) => (target, value) => {
		const [getFn, setFn] = createGetSet(mainPropFn, false, value);

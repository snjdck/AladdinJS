...
const str2bin = isNodeJS ? v => Buffer.from(v).toString('hex')   : ((encoder, num2str) => v => Array.from(encoder.encode(v), num2str).join(''))(new TextEncoder(), v => v.toString(16).padStart(2, '0'));
const bin2str = isNodeJS ? v => Buffer.from(v, 'hex').toString() : ((decoder, str2num) => v => decoder.decode(Uint8Array.from(v.match(/\w{2}/g), str2num)))(new TextDecoder(), v => parseInt(v, 16));

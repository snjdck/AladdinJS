'use strict';

function waitWhen(target, event, timeout, checker){
	return new Promise((resolve, reject) => {
		const timer = setTimeout(() => {
			cancelEvent();
			cancelError();
			reject('timeout');
		}, timeout);
		const cancelEvent = addListener(target, event, (...args) => {
			const result = checker(...args);
			if(result === false)return;
			cancelEvent();
			cancelError();
			clearTimeout(timer);
			resolve(result);
		});
		const cancelError = addListener(target, 'error', error => {
			cancelEvent();
			cancelError();
			clearTimeout(timer);
			reject(error);
		});
	});
}

function waitOnce(target, event, timeout=1000){
	return waitWhen(target, event, timeout, (...args) => args);
}

const listen1 = (target, event) => callback => target.once(event, callback);
const listen2 = (target, event) => {
	let waitCount = 0;
	return callback => {
		let count = waitCount++;
		target.on(event, function fn(v){
			if(count > 0)return count--;
			waitCount--;
			target.off(event, fn);
			callback(v);
		});
	}
}
const listen = (target, event) => {
	const waitList = [];
	target.on(event, v => waitList.shift()(v));
	return () => new Promise(resolve => waitList.push(resolve));
	//return callback => waitList.push(callback);
}

function addListener(target, event, callback){
	target.on(event, callback);
	return () => target.off(event, callback);
}
/*
function onEvt(target, event){
	return {[Symbol.asyncIterator](){
		const waitList = [];
		return {
			next: () => new Promise(resolve => waitList.push(resolve)),
			return: addListener(target, event, (...args) => waitList.shift()({value:args})),
		};
	}};
}
//*/

exports.waitWhen = waitWhen;
exports.waitOnce = waitOnce;
exports.listenFIFO = listen;
exports.addListener = addListener;

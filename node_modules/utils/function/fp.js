...
const overArgs = (fn, transforms) => (...args) => fn(...args.map((v, i) => (transforms && transforms[i] || identity)(v)));
...
const get = curry((key, target) => target[key]);
//const set = curry((k, v, target) => (target[key] = v, target));
const pick = curry((keys, target) => keys.reduce((result, key) => (result[key] = target[key], result), {}));
const map = curry((fn, list) => list.map(fn));
const filter = curry((fn, list) => list.filter(fn));
const reject = curry((fn, list) => list.filter(v => !fn(v)));
const reduce = curry((fn, init, list) => list.reduce(fn, init));
const propEq = curry((key, value, target) => target[key] === value);
const sortBy = curry((fn, list) => list.sort((a, b) => (a=fn(a), b=fn(b), a<b?-1:a>b?1:0)));

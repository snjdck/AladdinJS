...
const replaceBackRef = function(){
	const ref = /(\\+)(\d+)/g;
	return ({regexp, index, group}) => regexp.replace(ref, (raw, prefix, v) => prefix.length & 1 ? prefix + (Number(v) + index) : raw);
}();

function initPattern(patternList){
	let index = 0;
	for(let i=0, n=patternList.length; i<n; ++i){
		const pattern = patternList[i];
		pattern.index = ++index;
		if(!pattern.group)continue;
		index += pattern.group;
		pattern.regexp = replaceBackRef(pattern);
	}
}

function findPattern(patternList, match){
	for(let i=0, n=patternList.length; i<n; ++i){
		const pattern = patternList[i];
		if(match[pattern.index])return pattern;
	}
}
...
function highlight(...patternList){
	...
	return function*(code){
		...
		for(const match of code.matchAll(regexp)){
			...
			const {style} = findPattern(patternList, match);
			if(offset < index){

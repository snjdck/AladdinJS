...
async function main(){
	...
	const ptList2 = [
		{op:'forward', value:140},
		{op:'turnRight'},
		{op:'forward', value:120},
		{op:'turnLeft'},
		{op:'forward', value:390},
		{op:'turnLeft'},
		{op:'forward', value:120},
		{op:'turnRight'},
		{op:'forward', value:130},
		{op:'turnRight'},
		{op:'forward', value:360},
	]
	...
	for(const {op, pt, value} of ptList2){
		if(op == 'move'){
			...
			await carImage.moveForward(value, 400);
		}else if(op == 'turnLeft'){
			await carImage.turnLeft(300);
		}else if(op == 'turnRight'){
			await carImage.turnRight(300);
		}
		label.text = '总分:' + calcScore();
	}
	//*/

	function mygame_drag(){
		let item = itemList.find(item => carImage.hitTestCircle(item, 20))
		if(item){
			carImage.addChild(item);
			item.x = item.y = 0;

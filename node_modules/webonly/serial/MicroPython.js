...
async function exec_raw(serial, timeout){
	const result = await exec_raw_repl(serial, timeout);
...
function gen_download_code(name){
	const result = [];
	result.push('import sys;from binascii import b2a_base64\r');
	result.push(`with open('${castUnicode(name)}','b') as f:\r`);
	result.push(`\twhile True:\r`);
	result.push(`\t\td=f.read(192)\r`);
	result.push(`\t\tif not d:break\r`);
	result.push(`\t\tsys.stdout.write(b2a_base64(d)[:-1])\r`);
	return result;
}

...
const download_file = function(){
	function* gen_code(name, offset, length){
		if(offset == 0){
			yield 'def d(p,i,n):\r\timport binascii as b\r\twith open(p,"b")as f:\r';
			yield `\t\tf.seek(i)\r\t\tsys.stdout.write(b.b2a_base64(f.read(n))[:-1])\r`;
		}
		yield `d("${name}",${offset},${length})`;
	}
	return async function(serial, name, offset, length){
...
const upload_file = function(){
	function* gen_code(name, offset, length){
		if(offset == 0){
			yield 'def u(p,m,n):\r\timport binascii as b\r\twith open(p,m+"b")as f:\r';
			yield `\t\tf.write(b.a2b_base64(sys.stdin.read(n)))\r`;
		}
		yield `u("${name}","${offset>0?'a':'w'}",${length})`;
	}
	return async function(serial, name, offset, data){

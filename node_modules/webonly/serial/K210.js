...
const {delay, ints_to_buffer, prependInts, align_data, encode_c0db, checker_c0db, text_checker} = require('./helper');
...
async function write_dataframe(serial, cmd, frameSize, data, offset, progressFn=null){
	for(let [payload, totalSize] of to_frame(data, frameSize, (v, i) => prependInts(v, offset+frameSize*i, v.length))){
...
async function write_flash(serial, data, offset=0, progressFn=null){
	...
	await write_dataframe(serial, 0xd4, 65536, align_data(data, 4096), offset, progressFn);
}

const select_chip = (serial, type) => send_cmd(serial, 0xd7, ints_to_buffer(type, 0));
const erase_flash = (serial, address, length) => send_cmd(serial, 0xd3, ints_to_buffer(address, length), 20000);

async function send_cmd(serial, op, data, timeout=1000){
	await serial.send_buffer(encode_c0db(prependInts(data, op, crc32(data))));
	if(timeout < 0)return;

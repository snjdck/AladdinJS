...
const mapArgs = require('utils/function/mapArgs');
...
const checksum = (f => v => v.reduce(f, 0))((s, v) => s ^ v);
const encode = (index => v => {
	const buffer = Buffer.from([0x1B, index(), v.length >> 8, v.length & 0xFF, 0x0E, ...v, 0]);
...
const readFactory = (pageSize, addressFn, onRead) => async function(payload, sendFn, progressFn){
	for(let address=0, total=payload.length; address < total;){
		...
		const data = await sendFn(packet, bytesSend).then(stripFn);
...
//*/
function uploadFactory(pageSize, encoder, onEnter, onLeave, addressFn, onWrite, onRead){
	const doWrite = writeFactory(pageSize, addressFn, onWrite);
	const doRead = readFactory(pageSize, addressFn, onRead);
	return mapArgs((sendFn, resetFn) => async function(payload, progressFn, verifyFlag=false){
		let success;
		await resetFn();
		await delay(400);
...
const uploadMega = uploadFactory(
	0x100, encode,

...
const calcBindGroupLayoutSign = function(){
	...
	const sortFn = (a, b) => a.binding - b.binding;
	const mapFn = v => Array.from(sign(v)).join(',');
	return entries => entries.sort(sortFn).map(mapFn).join('&');
}();
...
const calcPipelineSign = function(){
	function* sign(desc){
		...
		if(desc.compute){
			yield desc.compute.entryPoint;
			yield desc.compute.code;
			return;
		}

		yield desc.vertex?.entryPoint;
		yield desc.vertex?.code;
		if(desc.vertex?.buffers){
			for(let buffer of desc.vertex.buffers){
				yield buffer.arrayStride;
				yield buffer.stepMode;
				for(let attribute of buffer.attributes.sort(sortFn)){
		...
		yield desc.fragment?.entryPoint;
		yield desc.fragment?.code;
		if(desc.fragment?.targets){
			for(let target of desc.fragment.targets){
				...
				yield target.blend?.color.srcFactor ?? 'one';
				yield target.blend?.color.operation ?? 'add';
				yield target.blend?.color.dstFactor ?? 'zero';
				yield target.blend?.alpha.srcFactor ?? 'one';
				yield target.blend?.alpha.operation ?? 'add';
				yield target.blend?.alpha.dstFactor ?? 'zero';
			}
		}
		...
		yield desc.depthStencil?.stencilFront?.compare ?? 'always';
		yield desc.depthStencil?.stencilFront?.depthFailOp ?? 'keep';
		yield desc.depthStencil?.stencilFront?.failOp ?? 'keep';
		yield desc.depthStencil?.stencilFront?.passOp ?? 'keep';
		yield desc.depthStencil?.stencilBack?.compare ?? 'always';
		yield desc.depthStencil?.stencilBack?.depthFailOp ?? 'keep';
		yield desc.depthStencil?.stencilBack?.failOp ?? 'keep';
		yield desc.depthStencil?.stencilBack?.passOp ?? 'keep';

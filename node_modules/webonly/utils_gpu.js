...
function createComputePipeline(device, layout, module, entryPoint='main', constants){
	return device.createComputePipeline({layout, compute: {entryPoint, module, constants}});
}

function createRenderPipeline(device, layout, vertex, fragment, depthStencil, primitive, multisample){
	return device.createRenderPipeline({layout, vertex, fragment, depthStencil, primitive, multisample});
}
...
const calcBindGroupLayoutSign = function(){
	function* sign(desc){
		...
		if(desc.buffer){
			...
			yield desc.storageTexture.access ?? 'write-only';
...
exports.createComputePipeline = createComputePipeline;
exports.createRenderPipeline = createRenderPipeline;

...
async function createTextureArray(device, imageList){
	...
	const texture = device.createTexture({
		format: 'bgra8unorm',
		size: [width, height, arrayLayerCount],
		usage: GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_DST | GPUTextureUsage.RENDER_ATTACHMENT
	});
	for(let i=0; i<arrayLayerCount; ++i){
		const image = imageList[i];
		const source = (image.naturalWidth == width && image.naturalHeight == height) ? image : await createImageBitmap(image, resizeOption);
		device.queue.copyExternalImageToTexture({source}, {texture, origin:{z:i}}, [width, height]);
	}
	return texture;
}
/*
	...
	return (groups, visibility) => {
		...
		for(let [i, entries] of Object.entries(groups)){
			for(let entry of entries.toSorted(sortEntryFn)){
				if(entry.name && entry.visibility & visibility){

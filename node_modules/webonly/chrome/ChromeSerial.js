...
const connect = port => new Promise((resolve, reject) => serial.connect(port, {bitrate:115200}, info => info ? resolve(info.connectionId) : reject(runtime.lastError.message)));
...
class Serial extends EventEmitter
{
	...
	connect(port){
		return connect(port).then(connectionId => {
			this.port = port;
	...
	reconnect(handler){
		return this.disconnect(true).then(handler).finally(() => this.connect(this.port));
	}
}

...
function createSocket(gatewayKeyDict){
	const socket = dgram.createSocket('udp4').on('error', function(error){
		...
		if(msg.cmd == 'heartbeat' && msg.model == 'acpartner.v3'){
			this._gatewayDict[info.address].token = msg.token;
			return;
		}
		if(msg.cmd == 'heartbeat' || msg.cmd == 'report'){
...
const SocketPrototype = {
	...
	_doSendAndWait(data, handler, address){
		this.on('message', function callback(message, info){
	...
	doRead(sid, handler, address){
		this._doSendAndWait({cmd:'read', sid}, handler, address || this._getGatewayAddress());
	},
	doWrite(sid, params, handler, address){
		address = address || this._getGatewayAddress();

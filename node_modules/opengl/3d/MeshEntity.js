...
class SubMeshEntity extends DisplayObject3D
{
	...
	collectDrawUnits(collector){
		let {textureMgr} = WebGL;
		let {drawUnitCache, parent, subMesh} = this;
		if(this.materials.length <= 0){
			let texture = textureMgr.fetch(subMesh.texture);
			if(texture){
	...
	updateBoneMatrixList({worldTransform, boneList}){
		...
		if(this.vertexData){
			let {boneMatrixList, subMesh, vertexData} = this;
			subMesh.updateVertexDataForCPU(vertexData, boneList);
			zeroMatrix3D.copyToArray(boneMatrixList);
			return;
		}
...
const zeroMatrix3D = new Matrix3D();
...
class MeshEntity extends DisplayObject3D
{
	constructor(mesh, createSkeleton=true){
		super();
		this.mesh = mesh;
		this.subEntityList = mesh.subMeshList.map(subMesh => this.addChild(new SubMeshEntity(subMesh)));

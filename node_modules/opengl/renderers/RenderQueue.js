...
class ShaderGroupRule extends GroupRule
{
	...
	active(drawUnit){
		...
		let program = programMgr.fetch(drawUnit.material.shader);
		gl.useProgram(program);
		drawUnit.material.onActiveUniform(camera, program);
	}
}
...
class MaterialGroupRule extends GroupRule
{
	...
	active(drawUnit){
		const {renderState, profileMgr} = WebGL;
		profileMgr.profile3d.stateSwitchCount++;
		let material = drawUnit.material;
		renderState.merge(material.renderState);
...
function draw(drawUnitList){
	const {material} = this;
	material.draw(this, drawUnitList);
}

...
class RenderQueue
{
	...
	onDraw(drawUnitList, camera){
		let {profileMgr} = WebGL;
		//uniformBuffer.registerCount = this._vao.boneCount;
		let maxCountPerDraw = Math.floor(1024 / this._vao.boneCount);
		//boneDataBuffer.instanceFloatCount = this._vao.boneCount << 2;
		let drawCount = batchDraw.call(this, maxCountPerDraw, drawUnitList);
		profileMgr.profile3d.drawCount += drawCount;
	}

	onPrepareBuffer(instanceCount){
		//boneDataBuffer.alloc(instanceCount);
	}

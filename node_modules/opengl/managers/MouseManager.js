...
class MouseManager
{
	...
	update(view3d){
		...
		gl.scissor(mouseX, height - mouseY, 1, 1);
	...
	pickRegister(target){
		...
		list[list.length] = target;
	}

	pickRegisterList(targetList, index, count){
		const list = this.objectList;
		const offset = list.length;
		list.length = offset + count;
		for(let i=0; i<count; ++i){
			list[offset+i] = targetList[index+i];
		}
	}

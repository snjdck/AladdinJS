...
class ProgramManager
{
	...
	newProgram(name){
		...
		if(index >= 0){
			define = name.slice(index);
			name = name.slice(0, index);
		}
		let [vn, fn] = name.split('&');
		fn = fn || vn;
		return this.createProgram(`${vn}.vert${define}`, `${fn}.frag${define}`);
	}
	...
	calcProgramInfo(program, name){
		...
		for(let i=0; i<ACTIVE_ATTRIBUTES; ++i){
			...
			gl.bindAttribLocation(program, newIndex, name);
...
const genDefines = list => list.map(genDefine).join('');

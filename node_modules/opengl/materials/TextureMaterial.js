...
const {ProgramName, MaterialRenderOrder} = require('../const');
...
class TextureMaterial extends Material{
	constructor(texture){
		...
		this.renderState = RenderState.get({
			depthTestEnabled: true,
			blendEnabled: texture && texture.transparent,
		});
	}
}

TextureMaterial.order = MaterialRenderOrder.texture;
TextureMaterial.shader = ProgramName.shader3d_normal;

TextureMaterial.onDrawBatch = function(drawUnitList, offset, instanceCount, batchIndex){
	const address_boneList = this.address['boneList[0]'];
	...
	if(subMesh.cpuMode){
		...
		if(isFirstBatch){
			gl.uniform4fv(address_boneList, nullBone);
		}
	...
	gl.uniform4fv(address_boneList, buffer.subarray(0, instanceCount * float32PerBone));
}

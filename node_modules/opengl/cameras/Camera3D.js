...
class Camera3D extends DisplayObject3D
{
	...
	render(root, mode){
		const {gl, util, programMgr, renderState, renderQueue, renderStateStack} = WebGL;
		...
		const collectFn = (mode instanceof Function) ? mode : (mode == DrawMode.Pick ? collectPickUnits : collectDrawUnits);
		...
		if(lightList.length <= 0){//no lights
			...
			programMgr.useProgram('screen&normal');
			util.drawScreen();
			...
			for(let {camera} of lightList){
				camera.worldTransformInvert.copyToArray(uniformBuffer.floatView, 24);
				uniformBuffer.setFloatv(32, camera.lens);
...
const getUniformBuffer = delayNew(() => new UniformBuffer(0, 13, 128));

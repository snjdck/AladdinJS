...
def cast(node, retCount=1):
	...
	if isinstance(node, ast.Attribute):
		return call("getKey", [cast(node.value), newString(node.attr)], 1)
	...
	if isinstance(node, ast.Assign):
		...
		if isinstance(target, ast.Attribute):
			return call("setKey", [cast(target.value), newString(target.attr), cast(node.value)], 0)
	...
	if isinstance(node, ast.For):
		return {"type":"for",
			"init":[setVar(node.target.id, newNumber(0))],
			"condition":call("<", [getVar(node.target.id), cast(node.iter.args[0])], 1),
			"iter":[setVar(node.target.id, call("+", [getVar(node.target.id), newNumber(1)], 1))], 
			"code":castList(node.body)
		}
	if isinstance(node, ast.FunctionDef):
		funcRef = {"type":"newFunction", "argList":cast(node.args), "code":castList(node.body), "userData":[True]}
		return setVar(node.name, funcRef)

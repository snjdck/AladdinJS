...
function createMatrixRawData(from, offset){
...
async function parse(info, path){
	...
	if(info.skins){
		for(let skin of info.skins){
			...
			for(let bone of boneList){
				bone.matrix = new Mat3x4(createMatrixRawData(inverseBindMatrices, bone.id << 4));
	...
	function createNode(node, factory){
		...
		if(node.matrix){
			result.transform = new Mat3x4(createMatrixRawData(node.matrix, 0));
		}else{
			if(node.rotation){

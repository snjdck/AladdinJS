...
async function parse(info, path){
	...
	const materials = info.materials.map(v => v.pbrMetallicRoughness.baseColorTexture ? path + textures[v.pbrMetallicRoughness.baseColorTexture.index] : 'color://[0,255,0]');
	const accessors = [];
	...
	for(let v of info.meshes){
		...
		for(let primitives of v.primitives){
			...
			subMesh.texture = materials[primitives.material];

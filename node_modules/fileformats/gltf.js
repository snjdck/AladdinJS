...
function castBuffer(bufferView, {count, type, componentType}){
	...
	if(componentType === 5126){
		...
		console.assert(bufferView.length == count * floatCount << 1);
	}else{
	...
	if(isVec){
		for(let i=0; i<count; ++i){
			...
			for(let j=0; j<floatCount; ++j){
				if(componentType === 5126){
					...
					value[j] = bufferView.readUInt16LE((i * floatCount + j) << 1);
				}
			}
			result[i] = value;
		}
		...
		for(let i=0; i<count; ++i){
			if(componentType === 5126){
				...
				result[i] = bufferView.readUInt16LE(i << 1);
			}
		}
	}
	return result;

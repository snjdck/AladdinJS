...
class View3D
{
	constructor(){
		...
		gl.__program__ = {};
		gl.render2d = new Render2D(gl, this.mouseMgr);
		gl.render3d = new Render3D(gl, this.mouseMgr);
		...
		this.pickBuffer = gl.createRenderbuffer();
		gl.bindRenderbuffer(gl.RENDERBUFFER, this.pickBuffer);
		gl.renderbufferStorage(gl.RENDERBUFFER, gl.RGBA32F, canvas.width, canvas.height);
		gl.bindRenderbuffer(gl.RENDERBUFFER, null);

		this.frameBuffer = gl.createFramebuffer();
		gl.bindFramebuffer(gl.FRAMEBUFFER, this.frameBuffer);
		gl.framebufferRenderbuffer(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.RENDERBUFFER, this.pickBuffer);
		gl.bindFramebuffer(gl.FRAMEBUFFER, null);

		this.pickData = new Float32Array(4);
	...
	onTick(timestamp){
		...
		const {gl, mouseMgr, width, height} = this;
		...
		if(mouseMgr.requestPick){
			mouseMgr.requestPick = false;
			gl.bindFramebuffer(gl.FRAMEBUFFER, this.frameBuffer);
			...
			this.drawScene("pick");
			gl.readPixels(mouseMgr.mouseX, height - mouseMgr.mouseY, 1, 1, gl.RGBA, gl.FLOAT, this.pickData);
	...
	registerProgram(...nameList){
		...
		for(var name of nameList){
			...
			fn = fn || vn;
			var vertexSource   = fs.readFileSync(`shaders/${vn}.vs`, "utf8");
			var fragmentSource = fs.readFileSync(`shaders/${fn}.fs`, "utf8");

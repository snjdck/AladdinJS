...
function fftF3(invertFlag){
	return function(p, size){
		const level = Math.log2(size);
		let result = Array.from(bitTest(level)).map(v => [p[v] ?? new Complex()]);
		for(let i=0; i<level; ++i){
			const n = 1 << i;
			const list = Array.from(Complex.generateCircle(n<<1, n), v => invertFlag ? v.conj() : v);
			result = Range(size >> i + 1).map(j => zipAB(result[j<<1], result[j<<1|1].map((v, k) => v.mul(list[k])), n));
		}
		return invertFlag ? result[0].map(v => v.divR(size)) : result[0];
	}
}

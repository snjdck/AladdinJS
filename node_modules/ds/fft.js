...
const zipAB = (f => (a, b, n) => zip(a, b, n, f))((a, b, i, n) => [[i, a.add(b)], [i+n, a.sub(b)]]);

const fftF3 = (core => invertFlag => {
	if(invertFlag){
		return function(p, size){
			const list = Array.from(Complex.generateCircle(size, size >> 1), v => v.conj());
			const f = 1 / size;
			return core(p, size, list).map(v => v.mulR(f));
		}
	}else{
		return function(p, size){
			const list = Array.from(Complex.generateCircle(size, size >> 1));
			return core(p, size, list);
		}
	}
})(function(p, size, list){
	const level = Math.log2(size);
	let result = Array.from(bitTest(level), v => [p[v] ?? Complex.zero]);
	for(let i=0; i<level; ++i){
		const n = 1 << i;
		const shift = level - i - 1;
		result = Range(size >> i + 1, 2).map(j => zipAB(result[j], result[j|1].map((v, k) => v.mul(list[k<<shift])), n));
	}
	return result[0];
});
...
function fftF2(invertFlag){
	return function fn(p, size){
		...
		b = Array.from(Complex.generateCircle(size, halfLen), (v, i) => b[i].mul(invertFlag ? v.conj() : v));
		const result = zipAB(a, b, halfLen);

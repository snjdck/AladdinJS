...
const handler = new Proxy({
	...
	FunctionExpression(node, ctx){
		...
		return function(...args){
			const scope = ctx.pushScope(defineScope);
	...
	AwaitExpression(node, ctx){
		console.log('await', this(node.argument, ctx));
	},
	...
	CallExpression(node, ctx){
		return this(node.callee, ctx).apply(null, collectArgs.call(this, node, ctx));
	},
	...
	MemberExpression(node, ctx){
		...
		return this(node.object, ctx)[node.property.name];
	},
	...
	UnaryExpression(node, ctx){
		...
		switch(node.operator){
			...
			case 'delete': console.error('todo: delete');
		}
		console.error(node);
	},
	...
	UpdateExpression(node, ctx){
		...
		var oldValue = scope[argument.name];
		var newValue;
		if(node.operator == '++'){
			newValue = oldValue + 1;
		...
		switch(argument.type){
			...
			case 'MemberExpression':
				this(argument.object, ctx)[argument.property.name] = newValue;
				break;
		}
	...
	ObjectExpression(node, ctx){
		...
		node.properties.forEach(function(propNode){
			...
			result[key] = self(propNode.value, ctx);
		})
		return result;
	},
	...
	AssignmentExpression(node, ctx){
		...
		console.error('todo AssignmentExpression', node)
	},
...
const walkFactory = handler => function self(node, ctx){
	return handler[node.type].call(self, node, ctx, handler);
};
...
exports.exec = function(code){
	walkFactory(handler)(acorn.parse(code), ctx);
}

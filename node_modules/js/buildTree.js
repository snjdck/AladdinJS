...
const matchLeft = opList => nextNode => function(nodeList){
	let a = nextNode.call(this, nodeList);
...
const matchRight = opList => nextNode => function self(nodeList){
	const a = nextNode.call(this, nodeList);
	if(nodeList.matchAny(opList)){
...
const matchInvoke = opList => nextNode => function(nodeList){
	let a = nextNode.call(this, nodeList);
	while(nodeList.matchAny(opList)){
		if(nodeList.expect(NodeType.OP_DOT)){
			a = calcute(nodeList.next(), a, nextNode.call(this, nodeList));
			continue;
		}
		a = calcute(nodeList.next(), a, this.call(this, nodeList));
		nodeList.next();
	}
	return a;
}

const matchParallel = (...matchList) => nextNode => function(nodeList){
}

function createSuperFunc(...ruleList){
	const fn = compose(...ruleList.reverse())(nodeList => null);
	return fn.bind(fn);
}
...
module.exports = createSuperFunc(
	...
	matchInvoke([NodeType.PARENTHESES_LEFT, NodeType.BRACKETS_LEFT, NodeType.OP_DOT]),

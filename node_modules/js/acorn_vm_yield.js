...
function newNodeFunctionExpression(params, body){
	return {
		...
		body: newNodeBlockStatement(body)
	}
}
...
const handler = new Proxy({
	...
	AwaitExpression(node, ctx){
		...
		let {blockStatement:{body}, expressionStatement} = ctx;
		const name = ctx.getName();
		const index = body.indexOf(expressionStatement);
		let rest = body.splice(index+1);
		body.push(expressionStatement=newNodeExpressionStatement(newNodeCallExpression(newNodeMemberExpression(node.argument, 'then'), [
			newNodeFunctionExpression([name], rest)
		])));
			/*
		let name = ctx.insertExpression(node.argument);
	...
	BinaryExpression(node, ctx){
		...
		this(node.right, ctx);
		if(ctx.isInAsyncFn()){
			let left = ctx.insertExpression(node.left);
			let right = ctx.insertExpression(node.right);
			Object.assign(node.left, newNodeIdentifier(left));
			Object.assign(node.right, newNodeIdentifier(right));
		}
	},
...
class Context {
	...
	insertExpression(expression){
		const {blockStatement:{body}, expressionStatement} = this;
		const name = this.getName();
		body.splice(
			body.indexOf(expressionStatement),
			0,
			newNodeExpressionStatement(newNodeAssignmentExpression(name, expression))
		);
		return name;
	}
	getName(){
...
const vm_exec = code => walk(acorn.parse(code), new Context(code));

vm_exec("async function a(tt){await Promise.resolve((await 3) + 4)}")

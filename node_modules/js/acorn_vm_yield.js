...
const handler = new Proxy({
	...
	AwaitExpression(node, ctx){
		...
		blockStatementBody.push(newNodeExpressionStatement(newNodeCallExpression(newNodeMemberExpression(node.argument, 'then'), [
	...
	BinaryExpression(node, ctx){
		const hasYield = containsYield(node);
		this(node.left, ctx);
		this(node.right, ctx);
		if(hasYield){
			let left = ctx.insertExpression(node.left);
			let right = ctx.insertExpression(node.right);
			node.left = newNodeIdentifier(left);
			node.right = newNodeIdentifier(right);
		}
	},

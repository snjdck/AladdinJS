...
const handler = {
	...
	ExportDefaultDeclaration(node){
		return 'export derault ' + walk(node.declaration);
	},
	...
	YieldExpression(node){
		return `yield` + (node.delegate ? '*' : ' ') + walk(node.argument);
	},
	...
	UnaryExpression(node){
		return node.operator + ' ' + walk(node.argument);
	},
	BinaryExpression(node){
		return walk(node.left) + ' ' + node.operator + ' ' + walk(node.right);
	},
	AssignmentExpression(node){
		return handler.BinaryExpression(node);
	},

	...
	for(let _=0; _<subMeshCount; ++_){
		...
		for(let i=0; i<triangleCount; ++i){
			...
			for(let j=0; j<3; ++j){
				...
				vertexData.setUint32(dataOffset+32, index, true);
			}
		}
		...
		/*subMeshList[subMeshIndex] = Object.assign(
			...
			{vertexData, vertexCount, boneData, texture, boneCount:boneData.length}
		);*/
		subMeshList[subMeshIndex] = {vertexData, vertexCount, boneData, texture, boneCount:boneData.length};
	}
...
const subMeshProto = {
	...
	updateVertexDataForCPU:function(){
		...
		return function(dataView, boneList){
			...
			for(let i=0; i<vertexCount; ++i){
				...
				normal.x = vertexData.getFloat32(dataOffset+12, true);
				normal.y = vertexData.getFloat32(dataOffset+16, true);
				normal.z = vertexData.getFloat32(dataOffset+20, true);
				let boneIndex = vertexData.getUint32(dataOffset+32, true);
				let boneID = boneData[boneIndex];
				let {worldTransform} = boneList[boneID];
				worldTransform.transformPoint(vertex);
				worldTransform.transformNormal(normal);

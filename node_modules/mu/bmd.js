...
function parse(fileData){
	...
	for(let _=0; _<subMeshCount; ++_){
		...
		for(let i=0; i<vertexListSize; ++i){
			const vec3 = castPosition(readVector3(fileData, offset + 4));
		...
		for(let i=0; i<triangleCount; ++i){
			...
			for(let j=0; j<3; ++j){
				...
				positionData[index << 2] = vertex.x;
				positionData[index << 2 | 1] = vertex.y;
				positionData[index << 2 | 2] = vertex.z;

				normalData[index << 2] = normal.x;
				normalData[index << 2 | 1] = normal.y;
				normalData[index << 2 | 2] = normal.z;

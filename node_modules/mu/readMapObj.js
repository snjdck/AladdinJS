...
module.exports = async function(worldID, encoder){
	...
	for(let i=0; i<objCount; ++i){
		...
		castPosition(pos);
		castRotation(rot);
		const matrix = {rotation:new Quaternion(),translation:new Vector3D()};
	...
	for(let file of new Set(objList.map(v => v.name))){
		let mesh = await encoder.loadData(`./assets/${file}.bmd`);
		calcMeshBound(mesh);

...
function castNode(node){
	...
	if(type == 'Subscript')return castNode(node.value) + castWrap(node.slice, '[', ']');
	...
	if(type == 'Tuple')return mapWrap(node.elts, '(', node.elts.length != 1 ? ')' : ',)');
	if(type == 'List')return mapWrap(node.elts, '[', ']');
	if(type == 'Set')return mapWrap(node.elts, 'new Set([', '])');
	if(type == 'Dict')return wrap(node.keys.map((k, i) => `[${castNode(k)},${castNode(node.values[i])}]`), 'new Map([', '])');
	...
	if(type == 'GeneratorExp')return wrap(calcGenerator(node), '(', ')');
	if(type == 'ListComp')return wrap(calcGenerator(node), 'Array.from(', ')');
	if(type == 'SetComp')return wrap(calcGenerator(node), 'new Set(', ')');
	if(type == 'DictComp'){
	...
	if(type.startsWith('Yield')){
		...
		return castWrap(node.value, `(yield${suffix} `, ')');
	}
	if(type == 'Await')return castWrap(node.value, '(await ', ')');
	if(type == 'Lambda'){
		scopeStack.push();
		const args = castNode(node.args);
		generatorFlagStack.push();
	...
	if(type == 'BoolOp')return mapWrap(node.values, '(', ')', wrap(castOP(node.op._type), ' ', ' '));
	if(type == 'UnaryOp')return castOP(node.op._type) + castNode(node.operand);
	if(type == 'Compare'){
		const n = node.ops.length;
		if(n == 1)return `(${castNode(node.left)} ${castOP(node.ops[0]._type)} ${castNode(node.comparators[0])})`;
		const result = ['(function(_1, _2){', '_1 = ' + castNode(node.left)];
		for(let i=0; i<n; ++i){
	...
	if(type == 'JoinedStr')return mapWrap(node.values, '(', ')', ' + ');

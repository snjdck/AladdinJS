...
function castFunction(node, classFlag){
	...
	if(classFlag)return reduceDecorator(node, prefix + `function${sign}(${castNode(node.args)}){\n${body}}`);
	return prefix + `function${sign} ${node.name}(${castNode(node.args)}){\n${body}}\n` + addDecorator(node);
}
...
function castNode(node){
	...
	if(type == 'Assign'){
		let prefix = '';
		const name = castNode(node.targets[0]);
		...
		return prefix + name + ' = ' + castNode(node.value);
	}
	if(type == 'AnnAssign'){
		if(node.value == 'None')return '';
		return castNode(node.target) + ' = ' + castNode(node.value);
	}
	if(type == 'AugAssign'){
	...
	if(type.endsWith('FunctionDef')){
		return castFunction(node);
	}
	if(type == 'arguments'){
	...
	if(type == 'JoinedStr'){
		return node.values.map(castNode).join(' + ');
	}
	if(type == 'FormattedValue'){
		return castNode(node.value);
	}
	if(type == 'Bytes'){
		return `Buffer.from(${quote(node.s)})`;
	}
	if(type == 'Try'){
		let result = `try{\n${castNodeList(node.body)}}`;
	...
	if(type == 'Raise'){
		return 'throw ' + castNode(node.exc);
	}
	if(type == 'ClassDef'){
...
let data = parse(process.cwd(), 'test2.txt');
console.log(data);

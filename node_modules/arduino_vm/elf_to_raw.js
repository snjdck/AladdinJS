...
function findSectionData(buffer, shstrtabOffset, sectionList, name){
	for(const section of sectionList){
		const offset = shstrtabOffset + section.sh_name;
		if(buffer.toString('utf8', offset, offset + name.length) !== name)continue;
...
function parse(buffer){
	...
	*/
	...
	return Buffer.concat([
		findSectionData(buffer, shstrtabOffset, sectionList, '.text\x00'),
		findSectionData(buffer, shstrtabOffset, sectionList, '.data\x00')
	]);
}

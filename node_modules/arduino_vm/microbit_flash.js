...
function parseHex(data){
	...
	let addressHigh = 0;
	for(let v of list){
		const recordType = v[3];
		if(recordType == 3 || recordType == 5 || recordType > 5)continue;
		if(recordType == 1)break;
		if(recordType == 2){
			addressHigh = readN(v, 4, v[0]) << 4;
			continue;
		}
		if(recordType == 4){
			addressHigh = readN(v, 4, v[0]) << 16;
			if(addressHigh == 0x10000000)break;
			continue;
		}
		const address = addressHigh | readN(v, 1, 2);
		if(address != result.length){
			console.log('pad', address - result.length);
			while(result.length < address){
				result.push(0);
			}
		}

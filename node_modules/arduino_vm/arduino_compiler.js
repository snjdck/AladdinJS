...
function compile(mcu, code, avr_root, includes=[], libs=[]){
	const avr_gcc = 	avr_root + `avr-gcc`;
	const avr_gpp = 	avr_root + `avr-g++`;
	const avr_objcopy = avr_root + `avr-objcopy`;
	...
	try{
		...
		execFileSync(avr_gpp, ['-c', '-w', '-Os', '-std=gnu++11', '-fpermissive', '-fno-exceptions', '-ffunction-sections', '-fdata-sections', '-fno-threadsafe-statics', '-flto',
			`-mmcu=${mcu}`,
			'-D', 'ARDUINO_ARCH_AVR',
			...includes.map(v => `-I${v}`),
			'-o', sketch_o,
		...
		execFileSync(avr_gcc, ['-Os', '-flto', '-fuse-linker-plugin', '-Wl,--gc-sections', '-lm',
			`-mmcu=${mcu}`,

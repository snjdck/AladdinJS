...
class Injector
{
	...
	getRule(key, inherit=true){
		if(!inherit)return this.#ruleDict.get(key);
	...
	new(type, ...args){
		let injection = type[NewTag];
		if(Array.isArray(injection))
			args.push(...injection.map(v => this.getInstance(v)));
		return newClass(type, ...args);
	}

	injectNew(type, ...args){
		let target = this.new(type, ...args);
...
const NewTag	= Symbol('InjectorNewTag');

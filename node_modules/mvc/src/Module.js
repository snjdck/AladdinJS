...
class Module
{
	...
	regMsgs(dispatched, interested){
		for(let v of new Set(this.meta.message.dispatched)){
			if(v in dispatched){
				console.error('msg %s from %s has been dispatched by %s!', v.valueOf(), this.name, dispatched[v].name);
				continue;
			}
			dispatched[v] = this;
		}
		for(let v of new Set(this.meta.message.interested)){
			if(dispatched[v] === this){
				console.warn("no need to interest msg: %s", v.valueOf());
				continue;
			}
			valueSet.add(interested, v, this);
			addIfNotExist(dispatched, v);
		}
	}
	...
	notify(msg){
		...
		if(!msg.isFrom(this, this._application)){

...
import {Injector, NewTag} from 'ioc';
import Msg from './Msg';
import safeCall from 'utils/function/safeCall';

class Module
{
	static [NewTag] = [Injector];

	#injector;
	#controllerDict = new Map();
	#viewSet = new Set();
	#roleSet = new Set();
	...
	constructor(meta, injector){
		...
		this.#injector.mapValue(Module, this, null);
		this.#injector.mapValue(Injector, this.#injector, null);
		assertMeta(meta);
		this.meta = meta;
	}
	...
	regService(serviceInterface, serviceClass, asLocal=false){
		...
		injector.mapSingleton(serviceInterface, serviceClass, this.#injector);
	...
	regController(controllerType){
		...
		let controller = this.#injector.new(controllerType);

...
class Application
{
	...
	_globalMsgDispatched = Object.create(null);
	_globalMsgInterested = Object.create(null);
	_roleMap = new Map();
	...
	notifyAll(msgName, msgData, dispatcher){
		...
		if(!(msgName in this._globalMsgDispatched)){
			return;
		}
		let allowed = this._globalMsgDispatched[msgName];
		if(allowed && allowed !== dispatcher){
			console.warn("%s can't notify msg %s!", dispatcher.name, msgName.valueOf());
			return;
		}
		let moduleList = this._globalMsgInterested[msgName];
		if(!moduleList)return;
		if(allowed){
			for(let module of moduleList){
				module.notify(new Msg(msgName, msgData, this));
			}
		}else if(moduleList.includes(dispatcher)){
			console.warn("%s can't notify self interested msg %o", dispatcher.name, msgName.valueOf());
		}
	}
	...
	onStartup(){
		...
		for(let module of moduleList)
			module.regMsgs(this._globalMsgDispatched, this._globalMsgInterested);

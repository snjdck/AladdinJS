...
class Application
{
	...
	notifyAll(msgName, msgData, dispatcher){
		...
		}else if(dispatcher.canDispatch(msgName)){
			for(let module of Object.values(this._moduleDict)){
				if(!module.hasInterest(msgName))continue;
				if(dispatcher === module)continue;
	...
	onStartup(){
		let globalMsgDispatched = Object.create(null);
		let moduleList = Array.from(Object.values(this._moduleDict));
		for(let module of moduleList) module.checkConflictedMsgs(globalMsgDispatched);

...
Blockly.Extensions.registerMutator('procedures_definition', {
	...
	this.getField("NAME").setValidator(function(newName){
		...
		if(oldName != newName && oldName != legalName){
			const blockList = this.getSourceBlock().workspace.getAllBlocks(false);
			for(let i=0; i < blockList.length; ++i){
				const block = blockList[i];
				if(block.type != 'procedures_call')continue;
				if(!Blockly.Names.equals(block.getFieldValue('NAME'), oldName))continue;
				block.setFieldValue(legalName, 'NAME');
			}
		}
		return legalName;
	});
	this.getField("sub_btn").setOnClickHandler(function(){
		...
		argList.pop();
		block._fireEvent('procedures_sub_arg', argList);
	});
	this.getField("add_btn").setOnClickHandler(function(){
		...
		block._fireEvent('procedures_add_arg', [...argList, name]);
	});
});

...
function procedures_mutator(fn){
	...
	return {
		...
		_updateShape(funcName, argList){
			this._onUpdateShape(funcName, argList, fn(this.inputList));
		}
	};
}
Blockly.Extensions.registerMutator('procedures_definition_arg', {
	...
	_onUpdateShape(funcName, argList, argInputList){
		console.log(this, funcName, argList, argInputList)
		...
		for(let name of argList){
			...
			console.log(this.getInputTargetBlock(name), '====')
	...
	workspace.addChangeListener(function(evt){
		...
		block._updateShape(block.getFieldValue('NAME'), evt.newValue);
	});
...
Blockly.Extensions.registerMutator('procedures_call_arg', {
	...procedures_mutator(inputList => inputList.slice(1)),
	_onUpdateShape(funcName, argList, argInputList){
		this.setFieldValue(funcName, 'NAME');
		for(let name of argList){
			let input = this.getInput(name);
...
overrideMethod(Blockly.InsertionMarkerManager.prototype, 'maybeShowPreview_', oldFn => function(candidate){
	...
	if(!closest)return;
	if(closest.getSourceBlock().type == "procedures_definition" && closest.type == Blockly.INPUT_VALUE)return;
	this.closestConnection_ = closest;
	this.localConnection_ = candidate.local;
	this.showPreview_();
});
...
`<block type="${type}">
	<mutation name="${block.getFieldValue('NAME')}">
		${block.arguments_.map(argFn).join('')}
	</mutation>
</block>`;

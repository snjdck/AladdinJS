...
Blockly.Extensions.registerMutator('procedures_definition', {
	...
	onchange(evt){
		...
		const blockName = this.getFieldValue('NAME');
		const hasOutput = this._hasOutput();
		this._updateShape(blockName, evt.newValue, hasOutput);
		this._forEachCallDo(block => block._updateShape(blockName, evt.newValue, hasOutput));
	}
}, function(){
...
Blockly.Gesture.prototype.handleBlockStart = function(e, block) {
	...
	if(block.type == 'procedures_param' && block.getParent() && block.getParent().type == 'procedures_definition'){
		const valueBlock = Blockly.Xml.domToBlock(

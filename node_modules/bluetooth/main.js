...
function initWorkspace(blocklyDiv){
	...
	const onResize = () => console.log('block activ') || Blockly.svgResize(workspace);
...
const WeatherPanel = createElement(function(tag, end){
	tag('div', {style:{display:'flex', flexDirection:'column'}})
		tag('label')
			tag`迭代次数:`
			tag('input', {name:'iterations', type:'number', step:1, defaultValue:1000}, true)
		end('label')
		tag('label')
			tag`误差阈值:`
			tag('input', {name:'errorThresh', type:'number', step:0.001, defaultValue:0.009}, true)
		end('label')
		tag('label')
			tag`学习速度:`
			tag('input', {name:'learningRate', type:'number', step:0.01, defaultValue:0.2}, true)
		end('label')
		tag('div')
			tag('label')
				tag`数据from:`
				tag('input', {name:'sliceFrom', type:'number', step:1, defaultValue:0}, true)
			end('label')
			tag('label')
				tag`数据to:`
				tag('input', {name:'sliceTo', type:'number', step:1, defaultValue:10}, true)
			end('label')
		end('div')
		tag('label')
			tag`属性:`
			tag(Select, {name:'attrIndex', children:csvData[0].slice(1).map((label, i) => ({label, value:i+1}))}, true)
		end('label')
		tag('div')
			tag('label')
				tag`预测数据from:`
				tag('input', {name:'forecastSliceFrom', type:'number', step:1, defaultValue:0}, true)
			end('label')
			tag('label')
				tag`预测数据to:`
				tag('input', {name:'forecastSliceTo', type:'number', step:1, defaultValue:5}, true)
			end('label')
			tag('label')
				tag`往后预测天数:`
				tag('input', {name:'forecastAhead', type:'number', step:1, defaultValue:5}, true)
			end('label')
		end('div')
	end('div')
});
...
const MainFrame = createElement(function(tag, end){
	const [tabPanelKey, setTabPanelKey] = React.useState('code_editor');
	...
	tag('div', {className:'frame', style:{width:'100%',height:'100%',backgroundColor:'aliceblue'}})
		...
		tag(TabPanel, {activeKey:tabPanelKey, onChange:setTabPanelKey})
			tag('div', {key:'blockly',title:'tab 1', style:{position:'relative'}})
				tag('div', {style:{width:'100%',height:'100%'},ref:initWorkspace}, true)
				tag('div', {id:'overlay'})
					tag(MyDrawer, true)
				end('div')
			end('div')
			tag('div', {key:'code_editor', title:'tab 2', ref:initCodeMirror}, true)
			tag('div', {key:'dialogPanel',title:'tab 4'})
				tag('div')
					tag('button', {onClick(){
						dialog.current.showModal((key, info) => {
							if(key != 'ok')return;
							//setTimeout(forecastWeather, 0, info)
						})
					}}), tag`click`, end('button')
					/*tag(Select, {name:'attrIndex', onChange(evt){
						console.log(evt.target.value)
					}, children:csvData[0].slice(1).map((label, i) => ({label, value:i+1}))}, true)*/
				end('div')
				tag(Dialog, {maskClosable:false, resetOnClose:false, title:'标题', ref:dialog, style:{width:550,height:400}, buttonList:[
					...
					tag(Fieldset, {title:'', disabled:true})
						tag`sdfsdf`
						tag`===`
					end(Fieldset)
					tag(WeatherPanel, true)
				end(Dialog)
				tag('div', {title:'天气图表', id:'weatherDiv'}, true)
			end('div')
		end(TabPanel)
		tag(Collapse, {style:{width:'600px',padding:'8px'}})
			tag(CodeView, {id:'testEditor', title:'代码预览',open:true})
			tag`asdfasdf\nadsfasdf`
			end(CodeView)
			tag('div', {title:'输出日志', id:'logDiv', open:true, style:{overflow:'auto'}}, true)
		end(Collapse)
	end('div')
})

...
const confirmWrap = (self, method) => overrideMethod(self, method, oldFn => () => 
	antd.Modal.confirm({
		title: '确定上传?',
		okText: '确定',
		cancelText: '取消',
...
class MyDrawer extends React.Component
{
	constructor(){
		...
		this.onDrawerClick = this.onDrawerClick.bind(this);
		this.onRunCode = this.onRunCode.bind(this);
		confirmWrap(this, 'onUploadFirmware');
		confirmWrap(this, 'onUploadCode');
		this.onUploadModalClose = this.onUploadModalClose.bind(this);
	}
	...
	async onUploadCode(){
		...
		//return
		this.setState({uploadModalVisible: true, step: 0});
		let data = await loadData('https://www.weeemake.com.cn:444/compile', 'arraybuffer', 'POST', code);
		this.setState({step: 1});
		await this.doUpload(data);
		this.setState({step: 3});
		setTimeout(this.onUploadModalClose, 2000);
		...
		return React.createElement('div', null,
			//React.createElement('div', null,
				React.createElement('div', {
					...
					React.createElement('img', {onClick: this.onRunCode,width:'50',height:'50',src:'./icons/play.png'}),
				),
			//),
			React.createElement(antd.Modal, {
				maskClosable: step == 3,

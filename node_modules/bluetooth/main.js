...
const variables_sync_type_helper = handler => function(){
	const fieldVar = this.getField("VAR");
	fieldVar.setValidator(handler);
	setTimeout(() => handler.call(fieldVar, fieldVar.getValue()), 0);
};
Blockly.Extensions.register('variables_set_sync_value_type', variables_sync_type_helper(function(id){
	this.getSourceBlock().getInput('VALUE').setCheck(
		Blockly.getMainWorkspace().getVariableById(id).type
	);
}));
Blockly.Extensions.register('variables_get_sync_output_type', variables_sync_type_helper(function(id){
	this.getSourceBlock().setOutput(true, Blockly.getMainWorkspace().getVariableById(id).type);
}));
...
Blockly.Variables.flyoutCategory = function(){
	...
	const variableModelList = workspace.getVariablesOfType('Number');
...
`<block type="math_change">
	...
	<value name="DELTA">
		...
		...variableModelList.map(variable => 

...
function procedures_mutator(fn, hasOutputFn){
	...
	return {
		...
		_updateShape(funcName, argList, hasOutput){
			this._onUpdateShape(funcName, argList, fn(this.inputList), hasOutput);
		}
	};
}
Blockly.Extensions.registerMutator('procedures_definition_arg', {
	...procedures_mutator(inputList => {
		const index = inputList.findIndex(v => v.name == 'PARAMS');
		return inputList.slice(1, index);
	}, block => block.getInput('OUTPUT') != null),
	_onUpdateShape(funcName, argList, argInputList, hasOutput){
		//console.log(this, funcName, argList, argInputList, hasOutput)
		this.setFieldValue(funcName, 'NAME');
		for(let name of argList){
			let input = this.getInput(name);
			if(input)continue;
			const typeName = funcName + name;
			input = this.appendValueInput(name).appendField(name);
			//input.setCheck(typeName);
			this.moveInputBefore(name, 'PARAMS');
			const valueBlock = Blockly.Xml.domToBlock(
				Blockly.Xml.textToDom(`<block type="procedures_param"><field name="NAME">${name}</field></block>`),
				this.workspace
			);
			//valueBlock.setOutput(true, typeName);
			valueBlock.isInFlyout = true;
			valueBlock.outputConnection.connect(input.connection);
		}
		for(let input of argInputList){
			if(argList.includes(input.name))continue;
			this.getInputTargetBlock(input.name).dispose();
			this.removeInput(input.name);
		}
		if(hasOutput != !this.getInput('OUTPUT'))return;
		if(hasOutput){
			this.appendValueInput('OUTPUT').appendField('返回').setAlign(Blockly.ALIGN_RIGHT);
		}else{
	...
	workspace.addChangeListener(function(evt){
		const workspace = Blockly.Workspace.getById(evt.workspaceId);
		switch(evt.element){
			case 'procedures_add_arg':
			case 'procedures_sub_arg':
				break;
			default:
				return;
		}
		const block = workspace.getBlockById(evt.blockId);
		block._updateShape(block.getFieldValue('NAME'), evt.newValue, block._hasOutput());
	});
	function fireEvent(block, evtType, newValue){
		const oldRecordUndo = Blockly.Events.recordUndo;
		Blockly.Events.recordUndo = false;
		Blockly.Events.fire(new Blockly.Events.BlockChange(
			block, evtType, null, null, newValue
		));
	...
	this.getField("sub_btn").setOnClickHandler(function(){
		...
		argList.pop();
		fireEvent(block, 'procedures_sub_arg', argList);
	});
	this.getField("add_btn").setOnClickHandler(function(){
		//const name = prompt('变量名', 'ARG' + block.arguments_.length);
		const argList = block._getArgList();
		const name = 'ARG' + argList.length;
		fireEvent(block, 'procedures_add_arg', [...argList, name]);
	});
});
Blockly.Extensions.registerMutator('procedures_call_arg', {
	...
	_onUpdateShape(funcName, argList, argInputList, hasOutput){
		...
		if(hasOutput != !this.outputConnection)return;
		this.setPreviousStatement(!hasOutput);
		this.setNextStatement(!hasOutput);
		this.setOutput(hasOutput);
	}
});
Blockly.Extensions.registerMutator('procedures_ifreturn', {
	...
	onchange(){
		...
		if(legal){
			this._updateShape(block.getInput('OUTPUT') != null);
...
overrideMethod(Blockly.InsertionMarkerManager.prototype, 'maybeShowPreview_', oldFn => function(candidate){
	if(this.wouldDeleteBlock_)return;
	const {closest, local} = candidate;
	if(!closest
		|| closest.getSourceBlock().type == "procedures_definition"
		&& closest.type == Blockly.INPUT_VALUE
		&& closest.targetConnection
		&& closest.targetConnection.getSourceBlock().type == "procedures_param"
	)return;
...
Blockly.Gesture.prototype.handleBlockStart = function(e, block) {
	...
	if(block.type == 'procedures_param' && block.getParent()){
		const valueBlock = Blockly.Xml.domToBlock(
			Blockly.Xml.blockToDom(block),
			block.workspace
		);
		const xy = block.getRelativeToSurfaceXY();
		valueBlock.moveBy(xy.x, xy.y);
		this.setStartBlock(valueBlock);
	}else{
...
void function(){
	const fn = oldFn => function(){
		oldFn.call(this);
		this.setInputsInline(true);
	};
	overrideMethod(Blockly.Blocks.procedures_callnoreturn, 'init', fn);
	overrideMethod(Blockly.Blocks.procedures_callreturn, 'init', fn);
}();
...
`<block type="${type}">
	<mutation name="${block.getFieldValue('NAME')}" hasOutput="${block._hasOutput()}">
...
Blockly.Variables.flyoutCategory = function(){
	...
	const result = [
		`<button text="创建变量" callbackKey="CREATE_VARIABLE"></button>`,
...
const {connectBLE} = require('./ble_app');
require('./generators');
...
class MyDrawer extends React.Component
{
	...
	async onUploadCode(){
		...
		let data;
		try{
			data = await loadData('https://www.weeemake.com.cn:444/compile', 'arraybuffer', 'POST', code);
		}catch(error){
		...
		return React.createElement('div', null,
			//React.createElement('div', null,
				React.createElement('div', {
					...
					React.createElement('img', {onClick: this.onUploadFactoryFirmware,width:'36',height:'36',src:'./icons/firmware.png'}),
					React.createElement('div', {style:{height: '20px'}}),
					React.createElement('img', {onClick: this.onUploadFirmware,width:'36',height:'36',src:'./icons/restore.png'}),
					React.createElement('div', {style:{height: '20px'}}),
					React.createElement('img', {onClick: this.onUploadCode,width:'36',height:'36',src:'./icons/upload.png'}),
					React.createElement('div', {style:{height: '30px'}}),
					React.createElement('img', {onClick: this.onRunCode,width:'64',height:'64',src:'./icons/play.png'}),
				),
			//),
			React.createElement(antd.Modal, {
				maskClosable: step == 3,

...
Blockly.Extensions.registerMutator('procedures_definition', {
	...
	_onUpdateShape(funcName, argList, argInputList, hasOutput){
		...
		for(let name of argList){
			...
			input.setCheck('procedures_param');
			this.moveInputBefore(name, 'PARAMS');
			const valueBlock = Blockly.Xml.domToBlock(
				Blockly.Xml.textToDom(`<block type="procedures_param" movable="false"><field name="NAME">${name}</field></block>`),
				this.workspace
			);
...
overrideMethod(Blockly.Connection.prototype, 'isConnectionAllowed', oldFn => function(candidate){
	return oldFn.call(this, candidate) && !(candidate.type == Blockly.INPUT_VALUE && Array.isArray(candidate.check_) && candidate.check_[0] == 'procedures_param');
});
overrideMethod(Blockly.Gesture.prototype, 'handleBlockStart', oldFn => function(evt, block){
	const parent = block.getParent();
	if(block.type == 'procedures_param' && parent && parent.type == 'procedures_definition' && parent.getInputTargetBlock('OUTPUT') != block){

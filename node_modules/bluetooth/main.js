...
class BlocklyPanel extends React.Component
{
	...
	render(){
		return React.createElement('div', {style:{width: '100%', height: '100%'}, ref:this.onInit});
	}
}
...
const MainFrame = createElement(function(tag, end){
	...
	React.useEffect(() => {
		if(tabPanelKey == 'blockly'){
			blocklyPanel.current.onResize();
			console.log('use effect blockly')
		}else if(tabPanelKey == 'code_editor'){
			editorPanel.current.onResize();
		}
	})
	tag('div', {className:'frame', style:{width:'100%',height:'100%',backgroundColor:'aliceblue'}})
		...
		tag(TabPanel, {activeKey:tabPanelKey, onChange:setTabPanelKey})
			tag('div', {key:'blockly',title:'tab 1', style:{position:'relative'}})
				tag(BlocklyPanel, {ref:blocklyPanel, onChange:setCode}, true)
				tag('div', {id:'overlay'})
					tag(MyDrawer, {}, true)
				end('div')
			end('div')
			tag(MonacoEditor, {ref:editorPanel,key:'code_editor', title:'tab 2', value:editorCode, language:editorLang, onChange:setEditorCode}, true)
		...
		tag(Collapse, {style:{width:'400px',padding:'8px'}})
			tag(CodeView, {title:'代码预览',open:true, children: code}, true)

...
Blockly.defineBlocksWithJsonArray(require('./blocks_for_scratch'));
...
const result = Object.assign({net:{sendData(buffer){
	return window.serial?.sendBLERawData(buffer);
}}}, context, primitives);
return result;
}();
...
function regExt({blocks, toolbox, context:ctx, buttons}){
	...
	if(toolbox)defaultToolbox.contents.push(toolbox);
	for(let key in ctx){
...
const {init:initPort} = require('./serial');
require('./generators');
...
class BlocklyPanel extends React.Component
{
	...
	onInit(blocklyDiv){
		...
		this.workspace.addChangeListener(evt => {
			if(evt.type == Blockly.Events.SELECTED && !evt.group){
				console.log(evt.type, evt.newElementId, evt.oldElementId)
			}
		...
		regExt(require('./extensions/pinIO'))
		regExt(require('./extensions/sensor'))
		regExt(require('./extensions/elf-mini'))
		regExt(require('./extensions/elf-328p'))
		regExt(require('./extensions/tlink'))
		regExt(require('./extensions/tinywebdb'))
		regExt(require('./extensions/face++'))
...
function showNotify(title, time=0, {tag, onClose, onClick, ...args}){
	const result = new Notification(title, {tag, requireInteraction: time < 0, ...args});
	if(time > 0)setTimeout(() => result.close(), time);
	if(onClose)result.addEventListener('close', onClose);
...
*/
	...
	React.useEffect(() => {
		navigator.serial.addEventListener('disconnect', evt => {
			if(evt.target !== window.serial)return;
			onPortDisconnected();
		});
	...
	tag('div', {className:'frame', style:{width:'100%',height:'100%',backgroundColor:'aliceblue'}})
		tag('div', {id:'menu', style:{gridColumnEnd:'span 2'}})
			tag(MenuBar, {port:port ? '已连接' : '未连接', board, async onClick(key){
				if(key == 'choosePort'){
					if(port){
						port.close().then(onPortDisconnected)
						return;
					}
					navigator.serial.requestPort({filters:[
						...
						window.serial = serialport;
						initPort(serialport);
						setPort(serialport);
					}, error => {
						if(error.code == DOMException.NOT_FOUND_ERR)return;//取消
						message.error(error);
					})
		...
		tag(TabPanel)
			tag(BlocklyPanel, {key:'blockly',title:'积木', isCodeRun, onMaskClick:js_engine.stop, ref:blocklyPanel, updateCode:setCode, updateXML:setBlockXML, buttonList:[
				{title:isCodeRun?'停止':'运行', onClick(){
				...
				{title:'上传', async onClick(){
					...
					//return console.log(raw_hex)
					//let raw_hex = await fetch('compile', {method:'POST', body:code}).then(v => v.arrayBuffer())
					//let raw_hex = await fetch('./hex/weeebotmini_factory_firmware.hex.raw').then(v => v.arrayBuffer())
					await window.resetBLE(raw_hex, v => message.info({key:'upload',content:'正在上传:' + (100 * v).toFixed() + '%'}));
					message.info({key:'upload',content:'上传完成'})
				}},
			]}, true)
		...
		tag(Collapse, {style:{width:400}})
			...
			tag(Video, {title:'捕获画面', open:true, style:{margin:4}, buttonList:[

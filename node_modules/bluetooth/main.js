...
const showCode = require('./code_editor/code_editor');
require('./code_editor/code_editor.css');
require('./code_editor/collapse.css');
require('./code_editor/dialog.css');
const {message} = antd;
...
const createElement = require('../vdom/createElement').createElement(React);
const initTabPanel = require('./code_editor/tab-panel.js');
require('./code_editor/tab-panel.css');
require('./code_editor/menu-group.css');
const {Menu} = antd;
const {SubMenu} = Menu;

const AppMenu2 = createElement(function(tag, end){
	tag('nav', {className:'menu-group', style:{'--menu-width':'300px'}})
		tag('div')
			tag`文件`
		...
		tag('div')
			tag`固件`
			tag('button'), tag`上传在线固件`, end('button')
		end('div')
		tag('div')
			tag`帮助`
...
const TabPanel = createElement(function(tag, end, {children}){
	tag('div', {className:'tab-panel', ref:initTabPanel, style:{paddingTop:'12px'}})
		tag('div')
			React.Children.map(children, ({props:{title, className}}) => {
				tag('span', {className}), tag`${title}`, end('span')
			});
		end('div')
		tag('div')
			React.Children.map(children, child => {
				tag(React.cloneElement(child, {title:null}));
			});
		end('div')
	end('div')
});
...
const Collapse = createElement(function(tag, end, {style, children}){
	tag('div', {className:'collapse', style})
		React.Children.map(children, child => {
			const {title, open} = child.props;
			const children = React.cloneElement(child, {title:null, open:null});
			tag(Details, {title, open, children}, true)
		});
	end('div')
});

const Details = createElement(function(tag, end, {title, children, open}){
	tag('details', {open})
		tag('summary'), tag`${title}`, end('summary')
		tag(React.Fragment, {children}, true)
	end('details')
});

const Fieldset = createElement(function(tag, end, {title, children, disabled}){
	tag('fieldset', {disabled})
		tag('legend'), tag`${title}`, end('legend')
		tag(React.Fragment, {children}, true)
	end('fieldset')
});
...
let workspace;
function initWorkspace(blocklyDiv){
	...
	blocklyDiv.addEventListener('active', onResize);
}
...
const MainFrame = createElement(function(tag, end){
	...
	tag('div', {className:'frame', style:{width:'100%',height:'100%',backgroundColor:'aliceblue'}})
		...
		tag(TabPanel)
			tag('div', {title:'tab 1', className:'active', style:{position:'relative'}})
				tag('div', {style:{width:'100%',height:'100%'},ref:initWorkspace}, true)
				tag('div', {id:'overlay'})
					tag(MyDrawer, true)
				end('div')
			end('div')
			tag('div', {title:'tab 2', ref:initCodeMirror}, true)
			tag('div', {title:'tab 4',style:{backgroundColor:'pink'}})
				tag('div')
					tag('button', {onClick(){
						dialog.current.showModal(console.log)
					}}), tag`click`, end('button')
				end('div')
				tag(Dialog, {ref:dialog, style:{width:550,height:400}, buttonList:[
					{title:'确认', value:'ok',className:'ant-btn ant-btn-primary'},
					{title:'取消', value:'cancel',className:'ant-btn ant-btn-danger'},
					{title:'返回', value:'', className:'ant-btn ant-btn-default'},
				]})
					tag(Fieldset, {title:'test', disabled:true})
						tag`test info==1`
						tag`test info==2`
					end(Fieldset)
					tag('label')
						tag`name:`
						tag('input', {name:'myName', type:'text'}, true)
					end('label')
					tag('label')
						tag`choose:`
						tag(Select, {name:'testChoose', defaultValue:'option 2.3', children:[
							['Group 1',
								{label:'option 1.1'},
								{label:'option 1.2'},
							],['Group 3', true,
							...
							{label:'option 2.2', disabled:true},
							{label:'option 2.3'},
						]}, true)
					end('label')
				end(Dialog)
		...
		tag(Collapse, {style:{width:'400px',padding:'8px'}})
			tag('div', {id:'testEditor', title:'代码预览',className:'editor',open:true}, true)
			tag('p', {title:'aaa'}), tag`a`, end('p')
			tag('p', {title:'bbb'}), tag`b`, end('p')
		end(Collapse)
	end('div')
})

ReactDOM.createRoot(mainFrame).render(React.createElement(MainFrame));
...
class MyDrawer extends React.Component
{
	...
	async onUploadCode(){
		let code = getHeadFile();
		code += Blockly.Arduino.workspaceToCode(Blockly.getMainWorkspace());
		showCode(document.getElementById('testEditor'), code.repeat(40));
		return
		this.setState({uploadModalVisible: true, step: 0});
		let data;
		try{

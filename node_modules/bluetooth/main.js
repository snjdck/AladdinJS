...
class Dialog extends React.Component
{
	...
	componentDidUpdate({open}, prevState, snapshot){
		if(this.props.open == open || open)return;
		const dialog = this.dialogRef.current;
		dialog.addEventListener('close', () => this.props.onClose(false), {once:true})
		dialog.showModal();
	}
}
...
Dialog.prototype.render = createElement(function(tag, end){
	...
	tag('dialog', {style, ref:this.dialogRef})
		tag('form', {method:'dialog'})
			...
			tag('footer')
				for(let {name, className, onClick} of buttonList){
					tag('button', {className, onClick}), tag`${name}`, end('button')
				}
			end('footer')
		end('form')
	end('dialog')
	...
	tag('div', {className:'tab-panel', ref:initTabPanel, style:{paddingTop:'12px'}})
		tag('div')
			for(let i=0; i<children.length; ++i){
				const {title, className} = children[i].props;
				tag('span', {className}), tag`${title}`, end('span')
			}
		end('div')
		tag('div', {children}, true)
	end('div')
})
	...
	tag('div', {className:'collapse', style})
		for(let i=0; i<children.length; ++i){
			...
			tag('details', {open})
				...
				tag(React.cloneElement(child, {title:null,open:null}))
			end('details')
		}
	end('div')
})
...
const MainFrame = createElement(function(tag, end){
	...
	tag('div', {className:'frame', style:{width:'100%',height:'100%',backgroundColor:'aliceblue'}})
		...
		tag(TabPanel)
			tag('div', {title:'tab 1', ref:initWorkspace, className:'active'}, true)
			tag('div', {title:'tab 2', ref:initCodeMirror}, true)
			tag('div', {title:'tab 4',style:{backgroundColor:'pink'}})
				tag('div')
					tag('button', {onClick:()=>setDialogOpen(true)}), tag`click`, end('button')
					//tag(tt, true)
				end('div')
				tag(Dialog, {open:isDialogOpen, onClose:setDialogOpen,style:{width:550,height:400}, buttonList:[
					{name:'确认', className:'primary', onClick(e){console.log('ok')}},
					{name:'取消', className:'danger', onClick(e){console.log('cancel')}},
					{name:'返回', onClick(e){console.log('return')}},
				]})
					tag('div')
						tag`test info`
					end('div')
...
//ReactDOM.createRoot(document.getElementById('overlay')).render(React.createElement(MyDrawer))

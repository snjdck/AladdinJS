...
Blockly.Extensions.registerMutator('procedures_definition_arg', {
	...
	domToMutation(xmlElement){
		console.log('domToMutation',this)
		this.updateShape_(xmlElement.getAttribute('mode'));
	},
	updateShape_(mode){
	}
	...
	workspace.addChangeListener(function(evt){
		if(evt.type == 'ui' && evt.element == 'dragStart'){
			console.log(evt)
		}
	})
	const block = this;
	...
	this.getField("sub_btn").setOnClickHandler(function(){
		if(block.arguments_.length <= 0)return;
		block.removeInput(block.arguments_.pop());
	});
	this.getField("add_btn").setOnClickHandler(function(){
		//const name = prompt('变量名', 'ARG' + block.arguments_.length);
		const name = 'ARG' + block.arguments_.length;
		block.arguments_.push(name);
		const input = block.appendValueInput(name);
	//	block.moveInputBefore(name, 'PARAMS');
		Blockly.Xml.domToBlock(
			Blockly.Xml.textToDom(`<block type="procedures_param"></block>`),
			workspace
...
Blockly.Variables.flyoutCategory = function(){
	...
	const numberVarList = workspace.getVariablesOfType('Number');
	const stringVarList = workspace.getVariablesOfType('String');
	const boolVarList = workspace.getVariablesOfType('Boolean');
	const arrayVarList = workspace.getVariablesOfType('Array');

	result.push(...workspace.getVariablesOfType().map(variables_get));
	
	if(numberVarList.length > 0){
		const mostRecentVariable = numberVarList[numberVarList.length - 1];
		numberVarList.sort(Blockly.VariableModel.compareByName);
		result.push(
`<label text="数字"></label>`,
...
`<block type="math_change">
	...
	<value name="DELTA">
		...
		...numberVarList.map(variables_get)
		);
	}
	if(stringVarList.length > 0){
		const mostRecentVariable = stringVarList[stringVarList.length - 1];
		stringVarList.sort(Blockly.VariableModel.compareByName);
		result.push(
`<label text="文本"></label>`,
`<block type="variables_set">
	<field name="VAR" id="${mostRecentVariable.getId()}" variabletype="${mostRecentVariable.type}">${mostRecentVariable.name}</field>
	<value name="VALUE">
		...
		...stringVarList.map(variables_get)
		);
	}
	if(boolVarList.length > 0){
		const mostRecentVariable = boolVarList[boolVarList.length - 1];
		boolVarList.sort(Blockly.VariableModel.compareByName);
		result.push(
`<label text="布尔值"></label>`,
`<block type="variables_set">
	<field name="VAR" id="${mostRecentVariable.getId()}" variabletype="${mostRecentVariable.type}">${mostRecentVariable.name}</field>
	<value name="VALUE">
		...
		...boolVarList.map(variables_get)
		);
	}
	if(arrayVarList.length > 0){
		const mostRecentVariable = arrayVarList[arrayVarList.length - 1];
		arrayVarList.sort(Blockly.VariableModel.compareByName);
		result.push(
`<label text="列表"></label>`,
`<block type="variables_set">
	<field name="VAR" id="${mostRecentVariable.getId()}" variabletype="${mostRecentVariable.type}">${mostRecentVariable.name}</field>
	<value name="VALUE">
		<shadow type="lists_create_empty"></shadow>
	</value>
</block>`,
`<block type="data_deletealloflist"></block>`,
`<block type="lists_addtolist"></block>`,

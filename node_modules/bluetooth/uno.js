...
const checkerFactory = castFn => composeFactory(
	(value, buffer) => buffer.push(...castFn(value)),
...
async function doRead(length, sendFn, progressFn){
	...
	for(let address=0; address < length;){
		...
		recvData.push(...data);
		address += bytesSend;
		if(progressFn)progressFn(address / length);
	}
	return recvData;
}
...
stepFn () => Void
//*/
async function upload(payload, resetFn, sendFn, progressFn, stepFn){
	await resetFn();
	await delay(0.4);
	...
	if(stepFn)stepFn();
	const recvData = await doRead(payload.length, sendFn, progressFn);

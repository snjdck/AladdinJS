
exports.blocks = require('./blocks.json');
exports.toolbox = require('./toolbox.json');

exports.context = {
	elf_328p_mpin:function(){
		const list = [null, [6, 7], [5, 8], [19,1], [19, 2], [18,1], [18, 2], [17,1], [17, 2], [16,1], [16, 2]];
		return ({value}) => `328p:${value}:${list[value]}`;
	}(),
	elf_328p_buzzer_pin(){
		return 11;
	},
	elf_328p_rgb_pin(){
		return 3;
	},
	elf_328p_rgb_ring_pin(){
		return 13;
	},
	elf_328p_button_pin(){
		return 2;
	}
}

exports.arduino = function(){
	const pinMap = {9:'PORT_A', 10:'PORT_B', 12:'PORT_C', 4:'PORT_D', 14:'PORT_1', 15:'PORT_2', 19:'PORT_3', 18:'PORT_4', 17:'PORT_5', 16:'PORT_6'};
	return {
		elf_328p_port(block, {value}){
			this.addInclude('#include<WeELF328P.h>');
			return [pinMap[value], this.ORDER_ATOMIC];
		},
		elf_328p_empin:'elf_328p_port',
		elf_328p_mpin(block, {value}){
			this.addInclude('#include<WeELF328P.h>');
			return ['M' + value, this.ORDER_ATOMIC];
		},
		elf_328p_buzzer_pin(){
			this.addInclude('#include<WeELF328P.h>');
			return ['OnBoard_Buzzer', this.ORDER_ATOMIC];
		},
		elf_328p_rgb_pin(){
			this.addInclude('#include<WeELF328P.h>');
			return ['OnBoard_RGB', this.ORDER_ATOMIC];
		},
		elf_328p_rgb_ring_pin(){
			this.addInclude('#include<WeELF328P.h>');
			return ['PORT_0', this.ORDER_ATOMIC];
		},
		elf_328p_button_pin(){
			this.addInclude('#include<WeELF328P.h>');
			return ['OnBoard_Button', this.ORDER_ATOMIC];
		}
	};
}();
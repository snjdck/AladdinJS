...
async function connectBLE(){
	...
	function sendAndWait(value, checker){
		writeWritableStream(port.writable, new Uint8Array(value));
		return listenReadableStream(port.readable, checker);
	}

	window.sendBLERawData = value => sendAndWait(value, replyFn([]));

	const sendFn = (array, recvCount=0) => sendAndWait(array, composeFn([], recvCount));

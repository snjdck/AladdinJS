...
class Serial extends ChromeSerial
{
	...
	async upload(data, offset=0x1000, progressFn=null){
		const info = Buffer.alloc(36, 0x55);
		try{
			await this.update(460800);
			await retry(() => this.sync(info), 30);
	...
	async write_stub(){
		...
		await this.send_cmd(5, this.ints_to_buffer(codeText.length, Math.ceil(codeText.length / ESP_RAM_BLOCK), ESP_RAM_BLOCK, 0x400be000));
		await this.write_dataframe(7, ESP_RAM_BLOCK, codeText);
		await this.send_cmd(5, this.ints_to_buffer(codeData.length, Math.ceil(codeData.length / ESP_RAM_BLOCK), ESP_RAM_BLOCK, 0x3ffdeba8));
	...
	async write_flash(data, offset, progressFn=null){
		...
		await this.send_cmd(0x10, this.ints_to_buffer(old_size, Math.ceil(data.length / FLASH_WRITE_SIZE), FLASH_WRITE_SIZE, offset));
...
Serial.prototype.send_cmd = function(){
	...
	return function(op, data, timeout=1000){
		const head = this.ints_to_buffer(op << 8 | data.length << 16, checksum(data, 16));
...
Serial.prototype.recv = function(){
	function concatFn(value, buffer){
		...
		while(buffer.length > 0 && buffer[0] != 0xc0)buffer.shift();
	}

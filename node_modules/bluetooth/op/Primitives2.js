...
const pinyin = require('../../utils/text/pinyin.js');

const db = Object.assign({board:'mini'}, getSearchInfo());
const board_elf = 'elf';
...
exports.sensor_port=exports.board_port = ({VALUE}) => {
	const board = boards.find(v => v.name == db.board);
	let pin = board.pins[VALUE];
	if(pin != null){
		return pin;
...
const weeemake_dc_motor = exports.weeemake_dc_motor = function({pin, speed}){
	const [name, index, value] = pin.split(':');
	const [p0, p1] = value.split(',');
	if(name == 'mini'){
		return this.net.sendData(createPacket(
			analogWrite(p0, speed >= 0 ? 0 : -speed),
			analogWrite(p1, speed >= 0 ? speed : 0)
		));
	}else if(name == '328p'){
		if(index < 3){
			return this.net.sendData(createPacket(
				analogWrite(p0, speed >= 0 ? speed : -speed),
				digitalWrite(p1, speed >= 0 ? 0 : 1)
			));
		}else{
			return this.net.sendData(createPacket(
				onewireSet(p0, 2, p1, calc_rj11_speed(speed))
			));
		}
	}
}
...
exports.weeemake_pin_rgb = function({pin, index, color}){
	return send_packet_rgb.call(this, pin, 32, index, colour_picker(color));
}
...
const weeemake_buzzer = exports.weeemake_buzzer = function({pin, note, beat}){
	//let pin = boards.find(v => v.name == db.board).buzzer_pin;
	return this.net.sendData(createPacket(
		base_op.buzzer(pin, note, beat)
	));
}
...
const image_recognition_color = ['#e17373','#e08649','#5450e8','#37b80c','#24716b'];
const weeemake_image_recognition_color_position = exports.weeemake_image_recognition_color_position = async function(argValues){
	...
	IMAGE_RECOGNITION_TYPE = image_recognition_color.indexOf(IMAGE_RECOGNITION_TYPE) + 1;
...
const weeemake_image_recognition_set_fast_mode = exports.weeemake_image_recognition_set_fast_mode = function(argValues){
	...
	ON_OFF = ON_OFF.toLowerCase() == 'true' ? 1 : 0;
...
const weeemake_adapter_rgb = exports.weeemake_adapter_rgb = function(argValues){
	const {SENSOR_PORT, ADAPTER_INDEX, PIXEL, COLOUR} = castPort2Pin(argValues);
	const {r, g, b} = colour_picker(COLOUR);
...
exports.weeemake_wifi_init_mode = function(argValues){
	switch(argValues.MODE){
		case 'sta':return weeemake_wifi_init_sta(argValues);
		case 'ap':return weeemake_wifi_init_ap(argValues);
		case 'tcp':return weeemake_wifi_init_tcp(argValues);
		case 'slave':return weeemake_wifi_init_slave(argValues);
	}
	console.error('weeemake_wifi_init_mode', argValues);
}

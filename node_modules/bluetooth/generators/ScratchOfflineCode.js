...
const Arduino = {
	...
	*/,
	...
	control_repeat_until(block, {SUBSTACK}, {CONDITION}){
		...
		if(!SUBSTACK){
			return `while(${condition})` + (this.needGenLoopFunc ? 'update()' : '') + this.END;
		}
		let case1Code = this.insertUpdateCode() + SUBSTACK;
	...
	*/
	...
	procedures_defreturn(block, {NAME, STACK, RETURN}){
		const name = this.adjustVarName(NAME);
		this.addFuncDef('double', name, [], (STACK ?? '') + (`\treturn ${RETURN ?? 0};\n`));
		this.markNameUsed(name);
		return '';
	}
};

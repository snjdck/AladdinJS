...
const Arduino = {
	...
	*/,
	...
	colour_picker(block){
		return [block.field['#text'].replace('#', '0x'), this.ORDER_ATOMIC];
	},
	text(block, {TEXT=''}){
	...
	control_wait(block){
		...
		let value = this.valueToCode(block, 'DURATION', this.ORDER_MULTIPLICATIVE);
	...
	math_constrain(block){
		const VALUE = this.valueToCode(block, 'VALUE', this.ORDER_NONE);
		const LOW = this.valueToCode(block, 'LOW', this.ORDER_NONE);
		const HIGH = this.valueToCode(block, 'HIGH', this.ORDER_NONE);
		return [`constrain(${VALUE}, ${LOW}, ${HIGH})`, this.ORDER_HIGH];
	},
	math_round: (dict => function(block){
		const OP = this.getFieldValue(block,'OP');
	...
	math_modulo(block){
		const DIVIDEND = this.valueToCode(block, 'DIVIDEND', this.ORDER_NONE);
		const DIVISOR = this.valueToCode(block, 'DIVISOR', this.ORDER_NONE);
	...
	math_random_int(block){
		...
		const TO = this.valueToCode(block, 'TO', this.ORDER_ADDITIVE);
		return [`random(${FROM}, 1 + ${TO})`, this.ORDER_ATOMIC];
	},
	...
	math_constant(block){
		const CONSTANT = this.getFieldValue(block,'CONSTANT');
	...
	colour_rgb(block){
		const RED = this.valueToCode(block, 'RED', this.ORDER_ADDITIVE);
		const GREEN = this.valueToCode(block, 'GREEN', this.ORDER_ADDITIVE);
		const BLUE = this.valueToCode(block, 'BLUE', this.ORDER_ADDITIVE);
		return [`((${RED} & 0xFF) * 0x10000 | (${GREEN} & 0xFF) << 8 | ${BLUE} & 0xFF)`, this.ORDER_ATOMIC];
	},

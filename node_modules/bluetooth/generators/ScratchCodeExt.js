...
const repeatFnFactory = (keyCond, keyDo) => function(block, {[keyCond]:count, [keyDo]:subCode}){
	let name = 'i';
	return `for(int ${name}=${count}; ${name}>0; --${name}){\n${tab(subCode ?? '')}}\n`;
}

const dict = {
	procedures_ifreturn:{
		arduino: ({VALUE=0, CONDITION='false'}) => `if(${CONDITION})return ${VALUE};\n`,
		python:  ({VALUE=0, CONDITION='False'}) => `if ${CONDITION}:return ${VALUE}\n`,
	},
	logic_boolean:{
		arduino({BOOL}){return [BOOL == 'TRUE' ? 'true' : 'false', this.ORDER_ATOMIC]},
		python ({BOOL}){return [BOOL == 'TRUE' ? 'True' : 'False', this.ORDER_ATOMIC]},
	},
	logic_null:{
		arduino(){return ['NULL', this.ORDER_ATOMIC]},
		python (){return ['None', this.ORDER_ATOMIC]},
	},
	logic_ternary:{
		arduino({IF, THEN, ELSE}){return [`${IF} ? ${THEN} : ${ELSE}`, this.ORDER_HIGH]},
		python ({IF, THEN, ELSE}){return [`${THEN} if ${IF} else ${ELSE}`, this.ORDER_HIGH]},
	},
	log:{
		arduino({value=0}){
	...
	sensing_timer:{
		arduino(){
			this.addVarDef(`unsigned long lasttime;\n`, 'lasttime');
			return ['(millis() - lasttime) * 0.001', this.ORDER_MULTIPLICATIVE];
		},
		python (){
			this.addInclude('import time');
			this.addVarDef(`lasttime = time.ticks_ms()` + this.END, 'lasttime');
			return ['time.ticks_diff(time.ticks_ms(), lasttime) * 0.001', this.ORDER_MULTIPLICATIVE];
		},
	},
	sensing_resettimer:{
		arduino(){
			this.addVarDef(`unsigned long lasttime;\n`, 'lasttime');
			return `lasttime = millis()` + this.END;
		},
		python (){
			this.addInclude('import time');
			this.addVarDef(`lasttime = time.ticks_ms()` + this.END, 'lasttime');
			return `lasttime = time.ticks_ms()` + this.END;
		},
	},
	control_wait:{
		arduino(_, {DURATION}){
			...
			return `delay(${text})` + this.END;
		},
		python ({DURATION}){
			this.addInclude('import time');
			return `time.sleep(${DURATION})` + this.END;
		},
	},
	control_stop:{
		arduino: () => `while(true);\n`,
	...
	control_repeat_until:{
		arduino({SUBSTACK}, {CONDITION}){
			let condition = CONDITION.fixBrackets(this.ORDER_UNARY, true);
			return `while(!${condition}){\n${tab(SUBSTACK ?? '')}}\n`;
		},
		python ({SUBSTACK}, {CONDITION}){
			let condition = CONDITION.fixBrackets(this.ORDER_NOT, true);
			return `while not ${condition}:\n${tab(SUBSTACK ?? 'pass\n')}`;
		},
	},
	control_if:{
		arduino({CONDITION, SUBSTACK}){
			return `if(${CONDITION}){\n${tab(SUBSTACK ?? '')}}\n`;
		},
		python ({CONDITION, SUBSTACK}){
			return `if ${CONDITION}:\n${tab(SUBSTACK ?? 'pass\n')}`;
		},
	},
	control_forever:{
	...
	controls_for:{
		arduino({VAR, FROM, TO, BY, DO}){
			const i = this.adjustVarName(VAR);
			return `for(int ${i}=${FROM}; ${i}<${TO}; ${i}+=${BY}){\n${tab(DO ?? '')}}\n`;
		},
		python ({VAR, FROM, TO, BY, DO}){
			const i = this.adjustVarName(VAR);
			return `for ${i} in range(${FROM}, ${TO}, ${BY}):\n${tab(DO ?? 'pass\n')}`;
		},
	},
	control_if_else:{
		arduino({CONDITION, SUBSTACK, SUBSTACK2}){
			if(/^if\(.+\)\{\n((\t|\}else).+\n)*\}\n$/.test(SUBSTACK2)){
				return `if(${CONDITION}){\n${tab(SUBSTACK ?? '')}}else ${SUBSTACK2}`;
			}
			return `if(${CONDITION}){\n${tab(SUBSTACK ?? '')}}else{\n${tab(SUBSTACK2 ?? '')}}\n`;
		},
		python ({CONDITION, SUBSTACK, SUBSTACK2}){
			if(/^if\x20.+:\n(\t.*\n|else:\n|elif\x20.+:\n)+$/.test(SUBSTACK2)){
				return `if ${CONDITION}:\n${tab(SUBSTACK ?? 'pass\n')}el${SUBSTACK2}`;
			}
			return `if ${CONDITION}:\n${tab(SUBSTACK ?? 'pass\n')}else:\n${tab(SUBSTACK2 ?? 'pass\n')}`;
	...
	math_single:{
		arduino({OP}, {NUM}){
			NUM = NUM.fixBrackets(OP == 'NEG' ? this.ORDER_UNARY : this.ORDER_NONE);
			if(OP == 'ROOT')return [`sqrt(${NUM})`, this.ORDER_ATOMIC];
			if(OP == 'ABS')return [`abs(${NUM})`, this.ORDER_ATOMIC];
			if(OP == 'NEG')return [`- ${NUM}`, this.ORDER_UNARY];
		...
		python({OP}, {NUM}){
			...
			if(OP == 'ROOT')return [`math.sqrt(${NUM})`, this.ORDER_ATOMIC];
			if(OP == 'ABS')return [`math.fabs(${NUM})`, this.ORDER_ATOMIC];
			if(OP == 'NEG')return [`- ${NUM}`, this.ORDER_UNARY];
			if(OP == 'LN')return [`math.log(${NUM})`, this.ORDER_ATOMIC];
			if(OP == 'LOG10')return [`math.log10(${NUM})`, this.ORDER_ATOMIC];
			if(OP == 'EXP')return [`math.exp(${NUM})`, this.ORDER_ATOMIC];
			if(OP == 'POW10')return [`math.pow(10, ${NUM})`, this.ORDER_ATOMIC];
		},
	}，
	math_constant:{
		arduino({CONSTANT}){
	...
	procedures_defreturn:{
		...
		python({NAME, STACK, RETURN, arg=[]}){
			if(!Array.isArray(arg)){arg = [arg];}
			const name = this.adjustVarName(NAME);
			this.addFuncDef(`def ${name}(${arg.map(v => this.adjustVarName(v.name)).join(', ')})`, (STACK ?? '') + `return ${RETURN ?? 0}\n`);
		}
	},
	procedures_defnoreturn:{
		arduino({NAME, STACK, arg=[]}){
			if(!Array.isArray(arg)){arg = [arg];}
			const name = this.adjustVarName(NAME);
			this.addFuncDef(`void ${name}(${arg.map(v => `double ` + this.adjustVarName(v.name)).join(', ')})`, STACK ?? '');
		},
		python({NAME, STACK, arg=[]}){
			if(!Array.isArray(arg)){arg = [arg];}
			const name = this.adjustVarName(NAME);
			this.addFuncDef(`def ${name}(${arg.map(v => this.adjustVarName(v.name)).join(', ')})`, STACK ?? '');
		}
	},
	logic_negate:{
		arduino(_, {BOOL}){
		...
		python(_, {BOOL}){
			const order = this.ORDER_NOT;
			return [`not ${BOOL?.fixBrackets(order, true) ?? 'False'}`, order];
		},
	},
}

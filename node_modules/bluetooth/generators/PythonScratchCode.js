...
const Python = {
	...
	*/
	...
	procedures_defreturn(block, {NAME, STACK, RETURN, arg=[]}){
		...
		const stageVar = this._stage[this.STAGE_BLOCK_VAR];
		const globalDef = stageVar ? `global ${Object.keys(stageVar).join(', ')}\n` : '';
		this.addFuncDef(`def ${name}(${arg.map(v => this.adjustVarName(v.name)).join(', ')})`, (STACK ? globalDef + STACK : '') + `return ${RETURN ?? 0};\n`);
	},
	procedures_defnoreturn(block, {NAME, STACK, arg=[]}){
		...
		const stageVar = this._stage[this.STAGE_BLOCK_VAR];
		const globalDef = stageVar ? `global ${Object.keys(stageVar).join(', ')}\n` : '';
		this.addFuncDef(`def ${name}(${arg.map(v => this.adjustVarName(v.name)).join(', ')})`, STACK ? globalDef + STACK : '');
	}
};

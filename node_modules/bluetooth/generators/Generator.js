...
class Generator extends Stage {
	...
	collectVarDefs(vars){
		...
		for(let {'#text':name, type} of castToArray(vars)){
			...
			this.addStage(this.STAGE_BLOCK_VAR, this.commentStatement(this.onGenVarDef(new_name, type), name), new_name);
		}
	}
	blockToCode(block){
		if(!block)return '';
...
const adjustVarName = v => '_' + (isNormalName(v) ? v : castChar(removeBlanks(v)));

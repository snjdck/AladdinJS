...
module.exports = function(){
	...
	Arduino.onGenVarDef = (new_name, name, type) => `${Arduino.varType[type]} ${new_name}` + `;\t//${name}`;
	...
	Arduino.lists_deleteoflist = function(block){
	...
	Arduino.lists_addtolist = function(block){
		let raw_name = block.getField('LIST').getText();
		let name = this.adjustVarName(raw_name);
		let item = this.valueToCode(block, 'VALUE', ORDER_NONE);
		this.markNameUsed(name);
		return this.commentStatement(`${name}.push(${item})`, raw_name);
	}
	Arduino.lists_deletealloflist = function(block){
		let raw_name = block.getField('LIST').getText();
		let name = this.adjustVarName(raw_name);
		this.markNameUsed(name);
		return this.commentStatement(`${name}.clear()`, raw_name);
	}
	Arduino.lists_insertatlist = function(block){
		let raw_name = block.getField('LIST').getText();
		let name = this.adjustVarName(raw_name);
		let item = this.valueToCode(block, 'VALUE', ORDER_NONE);
		let index = this.calcListIndex(block);
	...
	Arduino.lists_replaceitemoflist = function(block){
		let raw_name = block.getField('LIST').getText();
		let name = this.adjustVarName(raw_name);
		let item = this.valueToCode(block, 'VALUE', ORDER_NONE);
		let index = this.calcListIndex(block);
	...
	Arduino.lists_itemoflist = function(block){
		let raw_name = block.getField('LIST').getText();
		let name = this.adjustVarName(raw_name);
		let index = this.calcListIndex(block);
		this.markNameUsed(name);
	...
	Arduino.lists_listcontainsitem = function(block){
		let raw_name = block.getField('LIST').getText();
		let name = this.adjustVarName(raw_name);
		let item = this.valueToCode(block, 'VALUE', ORDER_NONE);
		this.markNameUsed(name);

...
const Arduino = {};
	...
	Arduino.elf_uno_mpin =  function(block){
		return [this.getFieldValue(block,'value'), this.ORDER_HIGH]
	}

	Arduino.weeemake_dc_motor = function(block){
		const pin = this.valueToCode(block, 'pin', this.ORDER_NONE).split(',');
		const speed = this.valueToCode(block, 'speed', this.ORDER_NONE);
		this.addSetupCode(`pinMode(${pin[0]}, OUTPUT)`);
		this.addSetupCode(`pinMode(${pin[1]}, OUTPUT)`);

		if(this.getInputTargetBlock(block, 'pin').type == 'elf_328p_mpin'){
			return [`{`,
			`\tint16_t speed = ${speed};`,
	...
	Arduino.weeemake_buzzer = function(block){
		...
		this.addVarDef(`WeBuzzer buzzer(OnBoard_Buzzer)`, 'buzzer');
...
*/
	...
	Arduino.weeemake_image_recognition_color_position = function(block){
		const SENSOR_PORT = this.valueToCode(block, 'SENSOR_PORT', this.ORDER_NONE);
		const IMAGE_RECOGNITION_TYPE = this.getFieldValue(block,'IMAGE_RECOGNITION_TYPE');

		let key = `imageRec_` + SENSOR_PORT.split('_')[1];

		this.addVarDef(`WeImageRecognition ${key}(${SENSOR_PORT})`, key);

		return [`${key}.getColorPosition(${IMAGE_RECOGNITION_TYPE})`, this.ORDER_HIGH];
	}
/*
...
*/
	...
	Arduino.weeemake_image_recognition_value = (nameList => function(block){
		const SENSOR_PORT = this.valueToCode(block, 'SENSOR_PORT', this.ORDER_NONE);
		const IMAGE_RECOGNITION_VALUE = this.getFieldValue(block,'IMAGE_RECOGNITION_VALUE');

		let key = `imageRec_` + SENSOR_PORT.split('_')[1];

		this.addVarDef(`WeImageRecognition ${key}(${SENSOR_PORT})`, key);

		return [`${key}.${nameList[IMAGE_RECOGNITION_VALUE]}`, this.ORDER_HIGH];

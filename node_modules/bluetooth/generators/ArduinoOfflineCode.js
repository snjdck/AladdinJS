...
module.exports = function(Arduino){
	...
	Arduino.weeemake_weeebot_board_rgb = function(block){
		...
		let result = [
			`${key}.setColor(1, ${COLOR.replace('#', '0x')})`,
			`${key}.show()`
		];
	...
	const genRGB = (key, port) => function(block){
		...
		let result = [
			`${key}.setColor(${PIXEL}, ${COLOR.replace('#', '0x')})`,
			`${key}.show()`
		];
	...
	Arduino.weeemake_dc_motor = function(block){
		...
		Arduino.addVarDef(`WeDCMotor ${key}(M${DC_MOTOR_INDEX})`, key);
	...
	Arduino.weeemake_servo = function(block){
		...
		if(db.board === board_elf_mini){
			Arduino.addVarDef(`WeServo ${key}(${SERVO_PORT}, ${this.irFlag})`, key);
		}else{
	...
	Arduino.weeemake_servo_360 = function(block){
		...
		Arduino.addVarDef(`WeServo360 ${key}(${SERVO_PORT}, ${this.irFlag})`, key);
	...
	Arduino.buzzer_note = function(block){
		return [block.getFieldValue('VALUE'), ORDER_HIGH];
	}

	Arduino.buzzer_beat = function(block){
		return [block.getFieldValue('VALUE'), ORDER_HIGH];
	}
	...
	Arduino.weeemake_ultrasonic_led = function(block){
		const SENSOR_PORT = block.getFieldValue('SENSOR_PORT');
		const ULTRASONIC_LED_INDEX = block.getFieldValue('ULTRASONIC_LED_INDEX');
		const ON_OFF = block.getFieldValue('ON_OFF');
		...
		return Arduino.tab() + `${key}.setLed(${ULTRASONIC_LED_INDEX}, ${INT_2_BOOL[ON_OFF]})` + END;
	}
	...
	Arduino.weeemake_led_matrix_time = function(block){
		...
		return Arduino.tab() + `${key}.showClock(${HOUR}, ${MINUTE}, ${INT_2_BOOL[SHOW_COLON]})` + END;
	}
...
*/
	...
	Arduino.weeemake_led_button_light = function(block){
		...
		return Arduino.tab() + `${key}.setLed(${BUTTON_INDEX}, ${INT_2_BOOL[ON_OFF]})` + END;
	}
	...
	Arduino.weeemake_relay = function(block){
		...
		let method = ON_OFF == '1' ? 'openNC' : 'closeNC';
		return Arduino.tab() + `${key}.${method}()` + END;
	}
	...
	Arduino.weeemake_water_atomizer = function(block){
		...
		let method = ON_OFF == '1' ? 'start' : 'stop';
		return Arduino.tab() + `${key}.${method}()` + END;
	}
	...
	Arduino.weeemake_color_sensor_light = function(block){
		...
		let method = ON_OFF == '1' ? 'turnOnLight' : 'turnOffLight';
		return Arduino.tab() + `${key}.${method}()` + END;
	}
...
*/
	...
	Arduino.weeemake_image_recognition_set_fast_mode = function(block){
		...
		return Arduino.tab() + `${key}.fastMode(${INT_2_BOOL[ON_OFF]})` + END;
	}
	...
	Arduino.weeemake_led_single_line_follower_led = (list => function(block){
		const SENSOR_PORT = block.getFieldValue('SENSOR_PORT');
		const ON_OFF = block.getFieldValue('ON_OFF');
		...
		return Arduino.tab() + `${key}.${list[ON_OFF]}()` + END;
	})(['closeLED', 'openLED']);
	...
	Arduino.weeemake_adapter_digitalWrite = function(block){
		...
		return Arduino.tab() + `${key}.digitalWrite(${ADAPTER_INDEX}, ${INT_2_HIGH_LOW[HIGH_LOW]})` + END;
	}
	...
	Arduino.weeemake_adapter_rgb = function(block){
		...
		return Arduino.tab() + `${key}.RGBshow(${ADAPTER_INDEX}, ${PIXEL}, ${R}, ${G}, ${B})` + END;
	}

...
function help3(stepList, data, a, b, x, y, offset){
	if(data[a+offset] == A && data[b+offset] == A){
		...
		help4(stepList, data, a, b, offset);
		/*
		for(let value of findItem(data, height, x+offset, width, B, 2)){
			if(value != y)continue;
			help4(stepList, data, a, b, offset);
			break;
		}
...
function*findItem(data, count, offset, step, itemSize, itemValue){
	let index = 0;
	for(let i=0; i<count; ++i){
		if(data[offset+step*i] != itemValue)continue;
		if(++index % itemSize == 1){
			yield i;
		}
	}
...
function main(init){
	...
	while(stepList.length > 0){
		...
		if(sign in checkedSignDict)continue;
		checkedSignDict[sign] = null;

...
function help3(stepList, data, a, b, y, offset){
	...
	if(test == data[b+offset] && (test == A || test == B)){
		if(!(test == B && ((y == 1 && data[a-4+offset] == B) || (y == 2 && data[b+4+offset] == B)))){
			const result = cloneBuffer(data);
			result[a] = result[b] = test;
			if(test == A)offset *= 2;
			result[a+offset] = result[b+offset] = 0;
...
function checkTwo(stepList, data, a, b){
	...
	if(x < 3 && a + 1 == b){
		...
		if(y > 0){
			help2(stepList, data, a, b, -4);
			help(stepList, data, a, b, -4);
		}
		if(y < 4){
			help2(stepList, data, a, b, 4);
			help(stepList, data, a, b, 4);
		...
		if(y > 0){
			help1(stepList, data, a, b, -4, B);
		}
		if(y < 3){
			help1(stepList, data, b, a, 4, B);
		}
		if(x > 0){

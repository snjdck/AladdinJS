			...
			function onMouseMove(evt){
				...
				if(type == 'output'){
					if(slot.dataset.statement == '1'){
						query = `my-card > [slot="input"][data-statement="1"]:not([data-connected="1"])`;
					}else{
						query = `my-card > [slot="input"]:not([data-statement="1"]):not([data-connected="1"])`;
					}
				}else{//input
					if(slot.dataset.statement == '1'){
						query = `my-card > [slot="output"][data-statement="1"]:not([data-connected="1"])`;
					}else{
						query = `my-card > [slot="output"]:not([data-statement="1"])`;//输出表达式可以输出多个值
					}
				}

			...
			if(linkTarget){
				...
				showMenu('#blockList-menu', evt, async key => {
					...
					case 'aa':
						...
						let block = blockDict['aa'].cloneNode(true);
						...
						if(type == 'output'){
							if(slot.dataset.statement == '1'){
								query = `[slot="input"][data-statement="1"]`;
							}else{
								query = `[slot="input"]:not([data-statement="1"])`;
							}
						}else{//input
							if(slot.dataset.statement == '1'){
								query = `[slot="output"][data-statement="1"]`;
							}else{
								query = `[slot="output"]:not([data-statement="1"])`;
							}
						}
...
document.addEventListener('DOMContentLoaded', async()=>{
	...
	document.querySelector('.blueprint-root').addEventListener('contextmenu', evt => {
		...
		showMenu('#blockList-menu', evt, async key => {
			...
			block.style.left = evt.x + 'px';
			block.style.top = evt.y + 'px';
			document.querySelector('.blueprint-root > .layerBlock').appendChild(block);
		});
	});

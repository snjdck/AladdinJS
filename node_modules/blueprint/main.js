...
function calcLinkPath(outputSlot, inputSlot){
	...
	const dx = inputX - outputX;
	const dy = inputY - outputY;
	const distance = Math.hypot(dx, dy);
	
	if(inputX > outputX){
		const offsetX = Math.max(dx * 0.3, 50);
	...
	const offsetX = Math.max(dx * -0.05, 20);
	const offsetY = interpolate(inputY, outputY, Math.abs(dy) / distance);
	return `path("M${outputX},${outputY} C${outputX+offsetX},${outputY} ${outputX+offsetX},${offsetY} ${centerX},${centerY} S${inputX-offsetX},${inputY} ${inputX},${inputY}")`;
}
...
function listenMouseEvent(target){
	function onMouseDown(evt){
		...
		function onMouseUp(evt){
			...
			if(linkTarget){
				if(type == 'input'){
					setLinkLineInOut(line, linkTarget, slot);
				}else{
					setLinkLineInOut(line, slot, linkTarget);
				}
				...
				showMenu('blockList-menu', evt, async key => {
					if(!key){
						...
						let block = createBlockByKey(key);
					...
					updateBlockMenuItemLabel(menu);
					for(;;){
						let result = menu.querySelectorAll('ul:not(:has(>li))');
...
function updateBlockMenuItemLabel(menu){
	for(let item of menu.querySelectorAll('[data-key]')){
		let block = blockDict[item.dataset.key];
		item.textContent = block.querySelector('[slot="title"]').textContent;
	}
}

function calcAnotherSlotQuery(slot){
	const otherSlotType = slot.getAttribute('slot') == 'output' ? 'input' : 'output';
	if(slot.dataset.type == 'statement'){
		return `[slot="${otherSlotType}"][data-type="statement"]`;
	}else if(slot.dataset.valuetype){
		return `[slot="${otherSlotType}"][data-type="expression"][data-valuetype=${slot.dataset.valuetype}]`;
	}else{
		return `[slot="${otherSlotType}"][data-type="expression"]`;
	}
}
...
function listenContextMenu(target, menuID, handler, filter){
	function onContextMenu(evt){
		evt.preventDefault();
		evt.stopPropagation();
		showMenu(menuID, evt, handler, filter);
	}
	target.addEventListener('contextmenu', onContextMenu);
	return () => target.removeEventListener('contextmenu', onContextMenu);
}

function newBlockMenuHandler(block){
	return key => {
		if(key == 'delete'){
			const {name} = block.dataset;
			for(let line of document.querySelectorAll('#svgContainer > g.blueprint-linkline')){
				const {input_block, output_block} = line.dataset;
				if(input_block != name && output_block != name)continue;
				deleteLinkLine(line);
			}
			block.remove();
		}else if(key == 'copy'){
			let rect = block.getBoundingClientRect();
			let newBlock = createBlockByKey(block.dataset.id);
			newBlock.style.left = rect.right + 10 + 'px';
			newBlock.style.top = rect.bottom + 10 + 'px';
			block.parentNode.appendChild(newBlock);
		}
	}
}

function createBlockByKey(key){
	let block = blockDict[key].cloneNode(true);
	let list = Array.from(document.querySelectorAll('blueprint-block[data-name]'), v => parseInt(v.dataset.name));
	for(let i=1;;++i){
		if(list.includes(i))continue;
		block.dataset.name = i;
		break;
	}
	return block;
}

function setLinkLineInOut(line, slotIn, slotOut){
	line.dataset.input_block = slotIn.parentNode.dataset.name;
	line.dataset.input_slot = slotIn.dataset.id;
	line.dataset.output_block = slotOut.parentNode.dataset.name;
	line.dataset.output_slot = slotOut.dataset.id;
}

function deleteLinkLine(line){
	const {input_block, output_block, input_slot, output_slot} = line.dataset;
	document.querySelector(`blueprint-block[data-name='${input_block}'] > [data-id='${input_slot}']`).dataset.connected = 0;
	document.querySelector(`blueprint-block[data-name='${output_block}'] > [data-id='${output_slot}']`).dataset.connected = 0;
	line.remove();
}
	...
	new MutationObserver(function(mutationList, observer){
		for(let mutation of mutationList){
			...
			for(let block of mutation.addedNodes){
				...
				block.signalContextMenu = listenContextMenu(block, 'block-menu', newBlockMenuHandler(block));
			}
		}
...
function updateLinkLines(){
	...
	for(let line of document.querySelectorAll('#svgContainer > g.blueprint-linkline')){
		const {input_block, output_block} = line.dataset;
		if(input_block != name && output_block != name)continue;
		let rectA = document.querySelector(`blueprint-block[data-name='${input_block}']`).getBoundingClientRect();
		let rectB = document.querySelector(`blueprint-block[data-name='${output_block}']`).getBoundingClientRect();
		line.style.setProperty('--d', calcLinkPath(rectA, rectB));
	}
}

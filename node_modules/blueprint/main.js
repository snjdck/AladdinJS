			...
			if(linkTarget){
				...
				showMenu('#blockList-menu', evt, async key => {
					if(!key){
						...
						if(type == 'output'){
							if(slot.dataset.type == 'statement'){
								query = `[slot="input"][data-type="statement"]`;
							}else{
								query = `[slot="input"][data-type="expression"]`;
							}
						}else{//input
							if(slot.dataset.type == 'statement'){
								query = `[slot="output"][data-type="statement"]`;
							}else{
								query = `[slot="output"][data-type="expression"]`;
							}
						}
						linkTarget = block.querySelector(query);
...
function showMenu(menuID, {x, y}, handler){
	const menu = document.querySelector(menuID);
	menu.style.left = 2 + x + 'px';
	menu.style.top = y + 'px';
	let key;
	function onToggle(evt){
		if(evt.newState == 'closed'){//closed
			menu.removeEventListener('pointerdown', onEvent);
	...
	menu.addEventListener('pointerdown', onEvent);
...
document.addEventListener('DOMContentLoaded', async()=>{
	...
	document.querySelector('.blueprint-root').addEventListener('contextmenu', evt => {
		evt.preventDefault();
		showMenu('#blockList-menu', evt, async key => {
			if(!key)return;
			let block = blockDict[key].cloneNode(true);
	...
	const response = await fetch('assets/blocks.html');
	const text = await response.text();
	const parser = new DOMParser();
	const blocksDom = parser.parseFromString(text, 'text/html');
	for(let block of blocksDom.querySelectorAll('body > [data-id]')){
		blockDict[block.dataset.id] = block;
	}
	new MutationObserver(function(mutationList, observer){
		for(let mutation of mutationList){
			...
			for(let block of mutation.addedNodes){
				...
				block.signalCancelDrag = doDrag(block.shadowRoot.querySelector(block.dataset.title == '0' ? '.body' : '.title'), block, updateLinkLines);
			}
		}

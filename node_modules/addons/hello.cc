...
void onSendMouseInput(const FunctionCallbackInfo<Value>& args){
...
void onGetCursorPos(const FunctionCallbackInfo<Value>& args){
	POINT point;
	GetCursorPos(&point);
...
void onGetSystemMetrics(const FunctionCallbackInfo<Value>& args){
	Isolate* isolate = args.GetIsolate();
	Local<Context> context = isolate->GetCurrentContext();
...
void onFindWindow(const FunctionCallbackInfo<Value>& args){
	...
	if(args[1]->IsString()){
		...
		lpTitleName = new uint16_t[arg1->Length() + 1];
		arg1->Write(isolate, lpTitleName);
	}
	HWND hWin = FindWindowW((LPCWSTR)lpClassName, (LPCWSTR)lpTitleName);
	args.GetReturnValue().Set(Number::New(isolate, (unsigned long)hWin));
	if(lpClassName)delete[] lpClassName;
	if(lpTitleName)delete[] lpTitleName;
}

void Initialize(Local<Object> exports)
{
	NODE_SET_METHOD(exports, "SendMouseInput", onSendMouseInput);
	NODE_SET_METHOD(exports, "GetCursorPos", onGetCursorPos);
	NODE_SET_METHOD(exports, "GetSystemMetrics", onGetSystemMetrics);

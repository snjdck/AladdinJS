...
const pattern = /(?<=^|\W)require\s*\(\s*("|')(.+?)\1\s*\)\s*(;?)/g;
const fileList = [];
...
function load(file){
	...
	if(file.endsWith('.js')){
		...
		info.data = data.replace(pattern, (_0, _1, _2, _3) => {
			const name = _require.resolve(_2);
			if(name.endsWith('.css')){
				if(!cssMap.has(name)){
					cssMap.set(name, fs.readFileSync(name, 'utf8'));
				}
				return '';
			}
			if(builtinModules.includes(name))return _0;
			return debugFlag
				? `__webpack_require__(/* ${_2} */ ${load(name)})${_3}`
				: `__webpack_require__(${load(name)})${_3}`;
		});
...
(function(modules){
	...
	return function __webpack_require__(moduleId){
...
fs.writeFileSync('bundle.css', Array.from(cssMap.values()).join('\n'));

...
const {existsSync, statSync, createReadStream, createWriteStream, readdirSync, mkdirSync} = require('fs');
...
function StaticFile(directory, prefix){
	...
	return next => (request, response, url) => {
		...
		if(request.method != 'GET' || !existsSync(filePath))
			return next(request, response, url);
		response.setHeader('Content-Type', findContentType(extname(filePath).slice(1)));
		const {range} = request.headers;
		const totalLength = statSync(filePath).size;
		if(range){
...
function StaticDirectory(root, useCache=true, headFn, nameFn){
	const createPage = name => Buffer.from(headFn(name) + readdirSync(join(root, name)).map(v => nameFn(join(name, v))).join(''));
	const cacheFn = useCache ? createGetter(createPage) : createPage;
	return next => (request, response, url) => {
		const name = decodeURIComponent(url.pathname);
		if(!statSync(join(root, name), {throwIfNoEntry:false})?.isDirectory())
			return next(request, response, url);
		const data = cacheFn(name);
...
function PutFile(root){
	return next => (request, response, url) => {
		if(request.method != 'PUT')return next(request, response, url);
		const name = decodeURIComponent(url.pathname);
		const file = join(root, name);
		if(existsSync(file))return response.end();
		mkdirSync(dirname(file), {recursive:true});
		request.pipe(createWriteStream(file));

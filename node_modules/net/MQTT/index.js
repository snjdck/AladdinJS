...
class Client extends EventEmitter
{
	...
	send(packet){
		...
		if(!packet.qos){
			...
			this.makeSureSend(buffer, packet.waitMsgType, packet.msgID, this.putMsgIDBack);
		}else if(packet.qos === 2){
			this.makeSureSend(buffer, PUBREC, packet.msgID, msgID => this.send({type: PUBREL, qos: 1, waitMsgType: PUBCOMP, msgID}));
		}
	}
	...
	makeSureSend(data, type, msgID, callback){
		const {socket} = this;
		const sendFn = (dup=true) => socket.write(markDupFlag(data, dup));

...
class Packet
{
	...
	static decode(buffer){
		...
		}else if(type == 5){//publish ack
			data.msgID = buffer.readUInt16BE(offset);
		}else if(type == 6){//publish ack
			data.msgID = buffer.readUInt16BE(offset);
		}else if(type == 7){//publish ack
			data.msgID = buffer.readUInt16BE(offset);
...
function createMsgIDManager(){
	return new ObjectCache((offset => () => {
		...
		console.log('create msg id ', offset)
		return offset++;
	})(1));
}
...
class Client extends EventEmitter
{
	...
	subscribe(topic){
		const msgID = this.msgIDMgr.getOut();
		this.makeSureSend(packet_subscribe(topic, msgID), 9, msgID, this.putMsgIDBack);
	}

	unsubscribe(topic){
		const msgID = this.msgIDMgr.getOut();
		this.makeSureSend(packet_unsubscribe(topic, msgID), 11, msgID, this.putMsgIDBack);
	}
	...
	publish(topic, message, qos=0, retain=false){
		...
		if(qos == 0){//most once
			...
			this.makeSureSend(dup => {
				packet[0] |= dup << 3;
				return packet;
			}, 4, msgID, this.putMsgIDBack);
		}else if(qos == 2){//exact once
			this.makeSureSend(dup => {
				packet[0] |= dup << 3;
				return packet;
			}, 5, msgID, () => this.makeSureSend(packet_pubrel(msgID), 7, msgID, this.putMsgIDBack));
		}
	}

	makeSureSend(data, type, msgID, callback){
		const {socket} = this;
...
connect('mqtt://test.mosquitto.org').on('connect', function () {
  ...
  setInterval(() => {
  	 this.publish('presence?', 'Hello mqtt', 2);
  	}, 1000);
  setTimeout(() =>{

...
*/
module.exports = handlerMap => (request, socket, url) => {
	const {pathname:path, searchParams:params} = url;
	const handler = handlerMap[path];
	if(!handler)return socket.end();
	socket.write(createResponse(request.headers['sec-websocket-key']));
	handler(new WebSocketClient(socket), params);
}

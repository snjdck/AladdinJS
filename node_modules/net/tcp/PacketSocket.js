...
function request(socket, packet, timeout=10){
	if(!socket.__reqId__){
...
let socket = net.connect(1234);
socket.on('connect', ()=>console.log('connect'))
//socket.on('packet', p => console.log(p));
listenPacket(socket, Packet);
(async function(){
	try{
		let response = await request(socket, new Packet(101, {'user':'alex'}), 2);

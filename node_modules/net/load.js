...
const readResponse = response => new Promise(resolve => readDataAsync(response, resolve));
/*
...
function load(url, {method, body, headers, query}={}){
	return new Promise(function(resolve, reject){
		...
		const request = http.request({method, headers,
			...
			if(!contentType)return readDataAsync(response, resolve);
			const index = contentType.indexOf(';');
			const responseType = castContentType(index > 0 ? contentType.slice(0, index) : contentType);
			readResponse(response).then(castType(responseType)).then(resolve);
		});
...
const castResponseTypePlugin = next => (url, args) => {
	return next(url, args).then(castType(args && args.responseType));
}

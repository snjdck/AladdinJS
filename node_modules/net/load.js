...
function load(url, {method, body, headers, query, autoRead, onProgress}={}){
	return new Promise(function(resolve, reject){
		...
		require(info.protocol.slice(0, -1)).request({
			...
			switch(response.statusCode){
				case 200: case 206: break;
				default: return reject(response);
			}
			if(autoRead === false){
				return resolve(response);
			}
			if(onProgress){
				response.on('data', (total => data => onProgress.call(response, total += data.length))(0));
			}

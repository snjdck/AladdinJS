...
Blockly.Extensions.register('syncUnoPinOutput', function(){
	const getType = pin => pin < 14 ? 'DPin' : 'APin';
	this.setOnChange(function(evt){
		const oldType = this.outputConnection.getCheck()[0];
		const newType = getType(this.getFieldValue('value'));
	...
	this.getField('value').setValidator(function(newValue){
		...
		if(typeList?.includes(getType(newValue)) === false)return null;
	});
});
...
workspace.addChangeListener(evt => outWin?.postMessage({type:'change', data:evt}))

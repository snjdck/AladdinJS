...
Blockly.Extensions.register('addDefaultIcons', function(){
	this.getField('value').iconList = [
		"011000000110:100100001001:000000000000:000011110000:000001100000",
		"111000000111:101000000101:111010010111:000001100000",
		"10000000000001:01000000000010:00100000000100:00010000001000:10001000010001",
		"111000000111:010001100010:010010010010",
		"1001000010010:1010101010101:1001000010010:1010001010100:1011100010111",
		"100101:100100:111101:100101:100101",
		"1111110:0010100:0100100:1000100:0000111",
		"111001001:101001010:101001100:101001010:111001001"
	];
});
Blockly.Extensions.register('syncUnoPinOutput', function(){
	function getType(pin){
		if(pin >= 14){
			return ['DPin', 'APin'];
		}
	...
	this.getField('value').setValidator(function(newValue){
		...
		const typeList = block.outputConnection.targetConnection?.getCheck();
		const newType = getType(newValue);
		if(typeList?.some(type => newType.includes(type)) === false)return null;
		const oldType = block.outputConnection.getCheck();
		if(oldType.toString() == newType.toString())return;
		block.setOutput(true, newType);
	});
});
...
Blockly.Extensions.register('markEventBlock', function(){
	this.data = 'isEventBlock';
	this.setNextStatement(true, null);
});
...
Blockly.Extensions.registerMutator('procedures_ifreturn', {
	...
	onchange(){
		...
		let block = this;
		do{
			if(block.type == 'procedures_definition' || block.type == 'procedures_defreturn'){
				legal = true;
				break;
			}
...
Blockly.Variables.flyoutCategory = function(){
	...
	const result = [
		{"kind": "button", "text": "创建数字变量", "callbackKey": "CREATE_VARIABLE_NUMBER"},
		{"kind": "button", "text": "创建文本变量", "callbackKey": "CREATE_VARIABLE_STRING"},
		{"kind": "button", "text": "创建布尔变量", "callbackKey": "CREATE_VARIABLE_BOOLEAN"},
		{"kind": "button", "text": "创建列表变量", "callbackKey": "CREATE_VARIABLE_ARRAY"}
	];
	...
	if(varList.length > 0){
		...
		result.push({
			...
			"fields":{
				"VAR":{
					"id":mostRecentVariable.getId(),
					"variabletype":mostRecentVariable.type,
					"#text":mostRecentVariable.name
				}
			}
		});
	}
	...
	if(arrayVarList.length > 0){
		result.push({
			...
			"inputs":{
				"INDEX":{"shadow":{"type":"math_lists_index","fields":{"NUM":1}}}
			}
		},{
			"kind":"block",
...
workspace.addChangeListener(evt => outWin?.postMessage({type:'change'}))
...
handleMsg({
	...
	setBlockEnabled({id, value}){
		workspace.getBlockById(id).setEnabled(value);
	},
	/*getAllVariables(){

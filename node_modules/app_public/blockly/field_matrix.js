...
const getByIndex = (list, i, w) => updateByIndex(list, i, w);

function updateByIndex(list, i, w, v){
	const y = Math.floor(i / w);
	const x = i % w;
	const oldValue = list[y][x];
	if(v === undefined)return oldValue;
	if(v instanceof Function){
		return list[y][x] = v(oldValue);
	}
	return list[y][x] = v;
}
...
Blockly.fieldRegistry.register('field_matrix', class extends Blockly.Field {
	...
	onMouseDown(evt){
		...
		const value = this._setValueByIndex(index, v => v == '0' ? '1' : '0');
		this.paintStyle_ = value === '1' ? 'fill' : 'clear';
	}
	...
	onMouseMove(evt){
		...
		const index = this.ledButtons_.indexOf(evt.target);
		if(index < 0)return;
		this._setValueByIndex(index, this.paintStyle_ === 'fill' ? '1' : '0');
	}

	_setValueByIndex(index, value){
		const list = str2list(this.getValue(), this._w, this._h);
		const result = updateByIndex(list, index, this._w, value);

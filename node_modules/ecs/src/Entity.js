'use strict';

const assert = require('assert');

class Entity
{
	constructor(id){
		Object.defineProperty(this, 'id', {value: id});
		Object.defineProperty(this, 'engine', {writable: true});
		Object.defineProperty(this, 'componentDict', {value: new Map()});
	}

	addComponent(componentType, args){
		assert(!this.hasComponent(componentType));
		assert(!(componentType.name in this));
		let {componentDict, engine} = this;
		let component = componentType.apply(null, args);
		Object.defineProperty(this, componentType.name, {value: component, configurable: true});
		componentDict.set(componentType, component);
		if(engine)engine.onComponentAdded(this, componentType);
		return this;
	}

	removeComponent(componentType){
		if(!this.hasComponent(componentType))return this;
		let {componentDict, engine} = this;
		delete this[componentType.name];
		componentDict.delete(componentType);
		if(engine)engine.onComponentRemoved(this, componentType);
		return this;
	}

	hasComponent(componentType){
		return this.componentDict.has(componentType);
	}

	getComponent(componentType){
		return this.componentDict.get(componentType);
	}

	match(componentTypes){
		return componentTypes.every(this.hasComponent, this);
	}

	addEntityToSet(entitySet, componentTypes){
		if(this.match(componentTypes)){
			entitySet.add(this);
		}
	}

	removeEntityFromSet(entitySet){
		entitySet.delete(this);
	}

	[Symbol.iterator](){
		return this.componentDict.keys();
	}

	get[Symbol.toStringTag](){
		let {id, componentDict} = this;
		let desc = Array.from(componentDict.entries(), ([type, data]) => `${type.name}:${JSON.stringify(data)}`);
		return `(id:${id}, ${desc.join(', ')})`;
	}
}

module.exports = Entity;